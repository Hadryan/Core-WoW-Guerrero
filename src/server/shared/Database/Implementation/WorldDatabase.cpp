...
void WorldDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(WORLD_SEL_SMART_SCRIPTS_BY_ENTRYORGUID, "SELECT entryorguid, source_type, id, link, event_type, event_phase_mask, event_chance, event_flags, event_param1, event_param2, event_param3, event_param4, event_param5, action_type, action_param1, action_param2, action_param3, action_param4, action_param5, action_param6, target_type, target_param1, target_param2, target_param3, target_x, target_y, target_z, target_o FROM smart_scripts WHERE entryorguid = ? ORDER BY entryorguid, source_type, id, link", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_SEL_WAYPOINT_DATA_BY_ID, "SELECT point, position_x, position_y, position_z, orientation, move_type, delay, action, action_chance, inverse_formation_angle FROM waypoint_data WHERE id = ? ORDER BY point", CONNECTION_SYNCH);
    ...
    PrepareStatement(WORLD_INS_CREATURE, "INSERT INTO creature (guid, id , map, zone, area, spawnMask, phaseMask, modelid, equipment_id, position_x, position_y, position_z, orientation, spawntimesecs, spawndist, currentwaypoint, curhealth, curmana, MovementType, npcflag, unit_flags, dynamicflags, walkmode, saiscriptflag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", CONNECTION_ASYNC);
    ...
    PrepareStatement(WORLD_SEL_PLAYER_TEXTS, "SELECT TextId, GroupId, ID, Text, Type, Language, Probability, Emote, Duration, Sound, BroadcastTextId, comment FROM player_text", CONNECTION_SYNCH);
}
