...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    // Account data
    PrepareStatement(CHAR_SEL_ACCOUNT_DATA, "SELECT type, time, data FROM account_data WHERE accountId = ?", CONNECTION_BOTH); // Change to _BOTH to handle a crash from bot testing
    ...
    PrepareStatement(CHAR_DEL_CHAR_ACHIEVEMENT_PROGRESS_AND_CRITERIA, "DELETE FROM character_achievement_progress WHERE guid = ? AND criteria = ?",                         CONNECTION_ASYNC);
