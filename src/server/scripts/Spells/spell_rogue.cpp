    ...
    class spell_rog_blade_flurry_AuraScript : public AuraScript
    {
        ...
        void OnProc(AuraEffect const* /*p_AurEff*/, ProcEventInfo& p_EventInfo)
        {
            ...
            if (Unit* l_Caster = GetCaster())
            {
                ...
                if (!l_Damage)
                    l_Damage = p_EventInfo.GetDamageInfo()->GetDamage();
                ...
                if (Unit* l_Target = l_Caster->SelectNearbyTarget(p_EventInfo.GetActionTarget()))
                    l_Caster->CastCustomSpell(l_Target, SPELL_ROGUE_BLADE_FLURRY_EXTRA_ATTACK, &l_Damage, NULL, NULL, true);
            }
        }
        ...
        void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
        {
            if (Unit* caster = GetCaster())
            {
                if (Unit* target = eventInfo.GetActionTarget())
                {
                    ...
                    damage += target->GetRemainingPeriodicAmount(caster->GetGUID(), SPELL_ROGUE_T12_2P_TRIGGERED, SPELL_AURA_PERIODIC_DAMAGE) / damage;
