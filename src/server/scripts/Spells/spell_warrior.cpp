...
public:
	...
	class spell_warr_sweeping_strikes_AuraScript : public AuraScript
	{
		...
		bool CheckProc(ProcEventInfo& eventInfo)
		{
            ...
            {
                Player* playerActor = actor->ToPlayer();
                Creature* creatureActor = actor->ToCreature();

			    _procTarget = actor->SelectNearbyTarget(eventInfo.GetProcTarget());
                if (playerActor)
			        WeaponCheck = playerActor->GetWeaponForAttack(BASE_ATTACK) || playerActor->GetWeaponForAttack(OFF_ATTACK);
                else if (creatureActor)
                    if (creatureActor->GetBotOwner())
                        WeaponCheck = creatureActor->GetBotOwner()->GetWeaponForAttack(BASE_ATTACK) || creatureActor->GetBotOwner()->GetWeaponForAttack(OFF_ATTACK); // Extra check for playerbots crash
            }
...
public:
	...
	class spell_warr_heroic_leap_build_SpellScript : public SpellScript
	{
			...
			SpellCastResult CheckElevation()
		{
			...
			if (dest->GetPositionZ() > caster->GetPositionZ() + 40.0f) // Cant jump to higher ground
