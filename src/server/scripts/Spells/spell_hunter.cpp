    ...
    class spell_hun_serpent_sting_SpellScript : public SpellScript
    {
        ...
        void HandleAfterHit()
        {
            if (Unit* caster = GetCaster())
            {
                ...
                else if (AuraEffect const* impSting = caster->GetAuraEffectOfRankedSpell(SPELL_HUNTER_IMPROVED_SERPENT_STING_R2, EFFECT_0, caster->GetGUID()))
                    {
                        SpellInfo const* sting = sSpellMgr->GetSpellInfo(SPELL_HUNTER_SERPENT_STING);
...
public:
    ...
    class spell_hun_focus_fire_AuraScript : public AuraScript
    {
        ...
        void OnApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                caster->RemoveAurasDueToSpell(SPELL_HUNTER_FOCUS_FIRE_ALERT);
                if (Pet* pet = caster->ToPlayer()->GetPet())
                {
                    if (Aura* aur = pet->GetAura(SPELL_HUNTER_PET_FRENZY, pet->GetGUID()))
                    {
                        basepoints0 *= aur->GetStackAmount();
                        caster->CastCustomSpell(pet, SPELL_HUNTER_FOCUS_FIRE_ENERGIZE, &basepoints0, NULL, NULL, true);
                        aur->Remove();
                    }
                }
            }
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
                if (Pet* pet = caster->ToPlayer()->GetPet())
                    if (!pet->HasAura(SPELL_HUNTER_PET_FRENZY, pet->GetGUID()))
                        caster->RemoveAura(SPELL_HUNTER_FOCUS_FIRE_ALERT);
        }
        ...
        void Register() override
        {
            ...
            AfterEffectApply += AuraEffectApplyFn(spell_hun_focus_fire_AuraScript::OnApply, EFFECT_1, SPELL_AURA_DUMMY, AuraEffectHandleModes(AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY));
            OnEffectRemove += AuraEffectRemoveFn(spell_hun_focus_fire_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };
...
public:
    ...
    class spell_hun_black_arrow_AuraScript : public AuraScript
    {
        ...
        void Register() override
        {
            OnEffectPeriodic += AuraEffectPeriodicFn(spell_hun_black_arrow_AuraScript::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
        }
    };
...
// 19615
class spell_hunter_pet_frenzy_effect : public SpellScriptLoader
{
public:
    spell_hunter_pet_frenzy_effect() : SpellScriptLoader("spell_hunter_pet_frenzy_effect") { }

    class spell_hunter_pet_frenzy_effect_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_hunter_pet_frenzy_effect_AuraScript);

        // Our amounts available on the rank
        int amount1 = 2;
        int amount2 = 4;
        int amount3 = 6;
    
    void SetHaste(AuraEffect const* aurEff, int32& amount, bool& /*canBeRecalculated*/)
    {
        Unit* caster = GetCaster();
        ...
        {
            if (owner->HasAura(19621))
                amount = amount1;
            else if (owner->HasAura(19622))
                amount = amount2;
            else if (owner->HasAura(19623))
                amount = amount3;
        }
    }
    ...
    void Register() override
    {
        DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_hunter_pet_frenzy_effect_AuraScript::SetHaste, EFFECT_0, SPELL_AURA_MOD_MELEE_HASTE_3);
    }
    };
    AuraScript* GetAuraScript() const override
    {
        return new spell_hunter_pet_frenzy_effect_AuraScript();
    }
...
public:
    ...
    class spell_hun_frenzy_AuraScript : public AuraScript
    {
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pet = GetCaster())
                if (Unit* owner = pet->GetOwner())
                    if (owner->HasAura(SPELL_HUNTER_FOCUS_FIRE_ALERT) && owner->HasSpell(SPELL_HUNTER_FOCUS_FIRE))
                        owner->RemoveAura(SPELL_HUNTER_FOCUS_FIRE_ALERT);
        }
        ...
        void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* pet = GetCaster())
                if (Unit* owner = pet->GetOwner())
                    if (GetAura()->GetStackAmount() >= 5 && owner->HasSpell(SPELL_HUNTER_FOCUS_FIRE))
                        owner->CastSpell(owner, SPELL_HUNTER_FOCUS_FIRE_ALERT, true);

        }

        void Register()
        {
            OnEffectApply += AuraEffectApplyFn(spell_hun_frenzy_AuraScript::OnApply, EFFECT_0, SPELL_AURA_MOD_MELEE_HASTE_3, AuraEffectHandleModes(AURA_EFFECT_HANDLE_REAL | AURA_EFFECT_HANDLE_REAPPLY));
            OnEffectRemove += AuraEffectRemoveFn(spell_hun_frenzy_AuraScript::OnRemove, EFFECT_0, SPELL_AURA_MOD_MELEE_HASTE_3, AURA_EFFECT_HANDLE_REAL);
...
void AddSC_hunter_spell_scripts()
{
    ...
    new spell_hunter_pet_frenzy_effect();
}