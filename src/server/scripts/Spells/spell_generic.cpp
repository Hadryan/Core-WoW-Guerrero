    ...
    class spell_gen_raid_SPELL_SpellScript : public SpellScript
    {
        ...
        void RemoveInvalidTargets(std::list<WorldObject*>& targets)
        {
            targets.remove_if([](WorldObject* target)
            {
                ...
                {
                    return creature->GetEntry() == 15438; // Fire elemental
                    // || creature->GetEntry() == XXX more to come?
                }
...
enum TauntSpells
{
	...
	SPELL_DEATH_KNIGHT_TAUNT = 56222,
	SPELL_PALADIN_TAUNT = 62124,
	SPELL_DRUID_TAUNT = 6795
};

class spell_gen_player_taunt : public SpellScriptLoader
{
public:
	spell_gen_player_taunt() : SpellScriptLoader("spell_gen_player_taunt") { }

	class spell_gen_player_taunt_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_gen_player_taunt_SpellScript);

		bool Validate(SpellInfo const* /*spellInfo*/) override
		{
			if (!sSpellMgr->GetSpellInfo(SPELL_WARRIOR_TAUNT)
				|| !sSpellMgr->GetSpellInfo(SPELL_DEATH_KNIGHT_TAUNT)
				|| !sSpellMgr->GetSpellInfo(SPELL_PALADIN_TAUNT)
				|| !sSpellMgr->GetSpellInfo(SPELL_DRUID_TAUNT))
				return false;
			return true;
		}

		// If the target is typeid_player it means that the target is immune to the taunt spell
		// and thats why we are gonna be settin the target in combat with the caster
		void HandleDummy()
		{
			Unit* caster = GetCaster();
			if (Unit* target = GetExplTargetUnit())
				// Check if our target is an actual player
				if (target->GetTypeId() == TYPEID_PLAYER)
				{
					// Set our target in combat with the caster
		...
		void Register() override
		{
			OnCast += SpellCastFn(spell_gen_player_taunt_SpellScript::HandleDummy);
		}
	};

	SpellScript* GetSpellScript() const override
	{
		return new spell_gen_player_taunt_SpellScript();
	}
};
...
// Herb, fixes: https://project-nightfall.org/bugtracker/issue/178/Bloodthistle#issue-178
class spell_gen_bloodthistle_herb : public SpellScriptLoader 
{
public:
    spell_gen_bloodthistle_herb() : SpellScriptLoader("spell_gen_bloodthistle_herb") { }

    class spell_gen_bloodthistle_herb_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_gen_bloodthistle_herb_AuraScript);
        ...
        void OnRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            if (GetTargetApplication()->GetRemoveMode() == AURA_REMOVE_BY_EXPIRE)
                if (Unit* caster = GetCaster())
                {
                    caster->CastSpell(caster, 28274, true);
                }
        }

        void Register()
        {
            AfterEffectRemove += AuraEffectRemoveFn(spell_gen_bloodthistle_herb_AuraScript::OnRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };
    AuraScript* GetAuraScript() const
    {
        return new spell_gen_bloodthistle_herb_AuraScript();
    }
};

class spell_gen_elune : public SpellScriptLoader
{
public:
    spell_gen_elune() : SpellScriptLoader("spell_gen_elune") { }

    class spell_gen_elune_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_elune_SpellScript);

        void OnCast()
        {
            if (Player* player = GetCaster()->ToPlayer())
            {
                GetCaster()->RemoveAurasDueToSpell(65602);
        ...
        void Register()
        {
            BeforeHit += SpellHitFn(spell_gen_elune_SpellScript::OnCast);
       }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_elune_SpellScript();
    }
};
...
void AddSC_generic_spell_scripts()
{
	...
	new spell_gen_player_taunt();
    new spell_gen_bloodthistle_herb(); // Recently Verified
    new spell_gen_elune(); // Recently Verified
}
