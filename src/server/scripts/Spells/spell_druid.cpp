...
enum DruidSpells
{
    ...
    SPELL_DRUID_T13_RESTORATION_2P_BONUS    = 105715,
    SPELL_DRUID_NATURAL_HARMONY             = 105713,
};
    ...
    class spell_dru_innervate_AuraScript : public AuraScript
    {
        ...
        void AfterApply(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                //T13 Restoration 2p
                if (caster->HasAura(SPELL_DRUID_T13_RESTORATION_2P_BONUS))
                    caster->CastSpell(caster, SPELL_DRUID_NATURAL_HARMONY, true);
            }
        }
...
class spell_dru_harmony : public SpellScriptLoader
{
    public:
        ...
        class spell_dru_harmony_SpellScript : public SpellScript
        {
            ...
            void OnHit()
            {
                // Dont proc on spell learning
                if (GetSpellInfo()->Id == 77495)
                    return;
            ...
            void HandleEffectPeriodic(AuraEffect const* aurEff)
            {
                if (DynamicObject* dyn = GetTarget()->GetDynObject(aurEff->GetId()))
                    if (Unit* target = GetTarget())
                        if (dyn->GetMapId() == target->GetMapId())
                            target->CastSpell(dyn->GetPositionX(), dyn->GetPositionY(), dyn->GetPositionZ(), 81269, true);
            }
        ...
        class spell_dru_lifebloom_AuraScript : public AuraScript
        {
            ...
            void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes mode)
            {
               ...
               else
               {
                   Unit::appliedAurasList const& auras = caster->appliedAuras;
                   ACE_Guard<ACE_Recursive_Thread_Mutex> g(caster->m_appliedAuraMutex);
                   for (Unit::appliedAurasList::const_iterator itr = auras.begin(); itr != auras.end();)
                   {
                       Aura* aura = (*itr);
                       if (aura == GetAura() || !aura->m_needsUnregister)
                       {
...
class spell_dru_typhoon : public SpellScriptLoader
{
    public:
        ...
        class spell_dru_typhoon_SpellScript : public SpellScript
        {
            ...
            {
                // Hack: State/effect immunities applied by aura expect full spell immunity by the core
                // Check target immunities here and simulate a knockback accordingly
                if (Unit* caster = GetCaster())
                    if (Unit* target = GetHitUnit())
                        if (!target->IsImmunedToSpellEffect(sSpellMgr->GetSpellInfo(69823), EFFECT_2)) // 61391 has effect_0 removed by spellmgr, using dummy id instead
                            if (!target->HasAura(SPELL_DRUID_GLYPH_OF_TYPHOON))
                            {
                                ...
                                float ratio = 0.1f;
                                float speedxy = float(GetSpellInfo()->Effects[EFFECT_0].MiscValue) * ratio;
                                float speedz = float(GetSpellInfo()->Effects[EFFECT_0].BasePoints) * ratio;
            ...
            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_dru_typhoon_SpellScript::HandleKnockBack, EFFECT_1, SPELL_EFFECT_SCHOOL_DAMAGE);
            }
        };
...
// Mainaz: Skull bash
class spell_dru_skull_bash_roots : public SpellScriptLoader
{
public:
    spell_dru_skull_bash_roots() : SpellScriptLoader("spell_dru_skull_bash_roots") { }

    class spell_dru_skull_bash_roots_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_dru_skull_bash_roots_SpellScript);

        SpellCastResult CheckElevation()
        {
            if (Unit* caster = GetCaster())
            {
                if (caster->HasUnitState(UNIT_STATE_ROOT) && !GetExplTargetUnit()->IsWithinMeleeRange(caster))
        ...
        void Register()
        {
            OnCheckCast += SpellCheckCastFn(spell_dru_skull_bash_roots_SpellScript::CheckElevation);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_dru_skull_bash_roots_SpellScript();
    }
};
...
void AddSC_druid_spell_scripts()
{
    ...
    new spell_dru_skull_bash_roots(); // Recently Verified
}
