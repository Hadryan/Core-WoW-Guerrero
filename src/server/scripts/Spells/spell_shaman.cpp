...
enum ShamanSpells
{
	...
	SPELL_SHAMAN_FEEDBACK_R2					= 86184,
	SPELL_SHAMAN_FEEDBACK_R3					= 86185,
...
class spell_sha_spirit_link : public SpellScriptLoader
{
    public:
        ...
        class spell_sha_spirit_link_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& unitList)
            {
                auto l_NewEnd = std::remove_if(unitList.begin(), unitList.end(), [](WorldObject* p_Object)
                {
                    if (auto l_Target = p_Object->ToUnit())
                        if (Aura* l_SmokeBomb = l_Target->GetAura(76577))
                            if (auto l_SmokeBombCaster = ObjectAccessor::GetUnit(*l_Target, l_SmokeBomb->GetCasterGUID()))
                                if (l_SmokeBombCaster->IsHostileTo(l_Target) || l_Target->HasAura(33786))
                                    return true;
                ...
                unitList.erase(l_NewEnd, unitList.end());

                uint32 l_Count = 0;

                for (auto l_Object : unitList)
                {
                    auto l_Target = l_Object->ToUnit();
                    if (!l_Target)
                        continue;

                    targets[l_Target->GetGUID()] = l_Target->GetHealthPct();
                    averagePercentage += l_Target->GetHealthPct();
                    ++l_Count;
                }

                averagePercentage /= l_Count;
            }
...
public:
    ...
    class spell_sha_feedback_AuraScript : public AuraScript
    {
        ...
        void HandleCooldown(ProcEventInfo& /*eventInfo*/)
        {
            ...
            {
				AuraEffect* feedback;
				if (caster->HasAura(SPELL_SHAMAN_FEEDBACK_R1) || caster->HasAura(SPELL_SHAMAN_FEEDBACK_R2) || caster->HasAura(SPELL_SHAMAN_FEEDBACK_R3))
                {
					if (caster->HasAura(SPELL_SHAMAN_FEEDBACK_R1))
						 feedback = caster->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_FEEDBACK_R1, EFFECT_0);
					if (caster->HasAura(SPELL_SHAMAN_FEEDBACK_R2))
						 feedback = caster->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_FEEDBACK_R2, EFFECT_0);
					if (caster->HasAura(SPELL_SHAMAN_FEEDBACK_R3))
						 feedback = caster->GetAuraEffectOfRankedSpell(SPELL_SHAMAN_FEEDBACK_R3, EFFECT_0);
