...
enum WarlockSpells
{
    ...
    SPELL_WARLOCK_IMMOLATION_AURA_METAMORPHOSIS     = 50589,
};
	...
	class spell_warl_eye_of_kilrogg_AuraScript : public AuraScript
	{
		...
		void HandleAuraRemove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
		{
			if (Player* player = GetTarget()->ToPlayer())
			{
				...
				// Re-initializing the pet to fix the petbar
				if (player->GetPet())
				{
...
// Maybe in the future we set a generic rule on aura handler to remove the periodic trigger aura once player no longer has the necessary form for it?
class spell_warlock_immolation_aura_metamorphosis : public SpellScriptLoader
{
public:
    spell_warlock_immolation_aura_metamorphosis() : SpellScriptLoader("spell_warlock_immolation_aura_metamorphosis") { }

    class spell_warlock_immolation_aura_metamorphosis_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_warlock_immolation_aura_metamorphosis_AuraScript);
        ...
        {
            if (aurEff->GetBase()->IsRemoved())
                return;

            if (Unit* owner = GetUnitOwner())
                if (!owner->HasAura(SPELL_WARLOCK_METHAMORPHOSIS_FORM))
                    Remove(AURA_REMOVE_BY_DEFAULT);
        }

        void Register() override
        {
            OnEffectUpdatePeriodic += AuraEffectUpdatePeriodicFn(spell_warlock_immolation_aura_metamorphosis_AuraScript::HandleUpdatePeriodic, EFFECT_0, SPELL_AURA_PERIODIC_TRIGGER_SPELL);
        }
    };

    AuraScript* GetAuraScript() const override
    {
        return new spell_warlock_immolation_aura_metamorphosis_AuraScript();
    }
};
...
void AddSC_warlock_spell_scripts()
{
    ...
    new spell_warlock_immolation_aura_metamorphosis(); // Verified
}
