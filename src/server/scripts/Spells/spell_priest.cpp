...
enum PriestSpells
{
    ...
    SPELL_PRIEST_T13_2P_HEALING = 105827,
    SPELL_PRIEST_T13_2P_HEALING_BUFF = 105826
};
    ...
    class spell_pri_mind_blast_SpellScript : public SpellScript
    {
        ...
        void HandleOnHit()
        {
            ...
            if (Unit* unitTarget = GetHitUnit())
            {
                ...
                if (GetHitUnit()->isSpellCrit(GetHitUnit(), GetSpellInfo(), SPELL_SCHOOL_MASK_SHADOW))
                {
                    if (GetCaster()->HasAura(87192))
                        GetCaster()->CastSpell(GetHitUnit(), 87193);
                    if (GetCaster()->HasAura(87195))
                        GetCaster()->CastSpell(GetHitUnit(), 87194);
                }
...
public:
    ...
    class spell_pri_shadow_word_death_SpellScript : public SpellScript
    {
        ...
        void HandleDamage()
        {
            ...
            if (caster->HasAura(55682) && !target->HasAura(95652)) // Glyph of Shadow Word: Death
            {
                if (target && target->GetHealthPct() <= 25)
                {
                    if (target->isAlive())
                    {
                        caster->ToPlayer()->RemoveSpellCooldown(32379, true);
                        caster->AddAura(95652, target); // Glyph of Shadow Word: Death - Marker
                    }
                }
            }
            // Glyph of Spirit Tap
...
public:
    ...
    class spell_pri_divine_hymn_SpellScript : public SpellScript
    {
        ...
        {
            if (Player* caster = GetCaster()->ToPlayer())
                if (caster->HasAura(SPELL_PRIEST_T13_2P_HEALING))
                    caster->CastSpell(caster, SPELL_PRIEST_T13_2P_HEALING_BUFF, true);
        }

        void Register()
        {
            AfterCast += SpellCastFn(spell_pri_divine_hymn_SpellScript::HandleCast);
        }
    };
...
class spell_pri_power_infusion : public SpellScriptLoader
{
public:
    spell_pri_power_infusion() : SpellScriptLoader("spell_pri_power_infusion") { }

    class spell_pri_power_infusion_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_pri_power_infusion_SpellScript);
        ...
        void Register()
        {
            AfterCast += SpellCastFn(spell_pri_power_infusion_SpellScript::HandleCast);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_pri_power_infusion_SpellScript();
    }
};
...
public:
    ...
    class spell_pri_mind_flay_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_pri_mind_flay_SpellScript);
                ...
                if (Player* player = caster->ToPlayer())
                    if (Aura* aura = GetHitAura())
                        if (AuraEffect* aurEff = aura->GetEffect(EFFECT_0))
                            if (player->GetMindFlayPeriodicTimer() > 0 && player->GetMindFlayRecastTimer() > 0)
                            {
                                aurEff->SetPeriodicTimer(player->GetMindFlayPeriodicTimer());
                                aurEff->SetExtraTick(true);
                            }
        }

        void Register()
        {
            AfterHit += SpellHitFn(spell_pri_mind_flay_SpellScript::HandleAfterHit);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_pri_mind_flay_SpellScript();
    }
};
...
void AddSC_priest_spell_scripts()
{
    ...
    new spell_pri_divine_hymn(); //Recently Verified
    new spell_pri_power_infusion(); // Recently Verified
    new spell_pri_mind_flay();  // Recently Verified
}
