        ...
        class spell_mage_ring_of_frost_AuraScript : public AuraScript
        {
            ...
            void HandleEffectPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                for (std::list<Creature*>::iterator itr = MinionList.begin(); itr != MinionList.end(); itr++)
                {
                    ...
                    {
                        if (!GetTarget()->HasAura(33786)) // Prevent the cast if the mage is cycloned
...
class spell_mage_mage_ward : public SpellScriptLoader
{
   public:
       ...
       class spell_mage_mage_ward_AuraScript : public AuraScript
       {
           ...
           void AfterRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
           {
               // AURA_REMOVE_BY_ENEMY_SPELL works for both dispels and damage absorb aura removal, check for remaining absorb amount for molten shields.
        ...
        class spell_mage_piercing_chill_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                if (AuraEffect* aurEff = GetCaster()->GetAuraEffect(SPELL_AURA_DUMMY, SPELLFAMILY_MAGE, 4625, EFFECT_0))
                {
                    ...
                    monster::Containers::RandomResizeList(targets, aurEff->GetAmount());
...
public:
    ...
    class spell_mage_ice_block_SpellScript : public SpellScript
    {
        ...
        void HandleCdRemove()
        {
            ...
            if (caster == nullptr || caster->GetTypeId() != TYPEID_PLAYER)
                return;

            if (Player* player = caster->ToPlayer())
            {
                if (caster->HasAura(56372)) /// Glyph
        ...
        void Register()
        {
            AfterCast += SpellCastFn(spell_mage_ice_block_SpellScript::HandleCdRemove);
        }
    };
...
class spell_mage_dragon_breath : public AuraScript
{
    ...
    bool CheckProc(ProcEventInfo& eventInfo)
    {
        ...
        if (spellInfo && (spellInfo->SpellIconID == 3000 || spellInfo->SpellIconID == 4943) && spellInfo->SpellFamilyName == SPELLFAMILY_MAGE)
            return false;
        return true;
    }
    void Register() override
...
class IgniteRollingEvent : public BasicEvent
{
    public:
        IgniteRollingEvent(Unit* caster, Unit* victim, int32 bank, uint32 newDuration)
            : _caster(caster), _bank(bank), _victim(victim), _newDuration(newDuration)
        {
        }
        ...
        {
            if (_victim && _caster)
                if (_caster->IsValidAttackTarget(_victim))
                    if (Aura* igniteRolled = _caster->AddAura(SPELL_MAGE_IGNITE, _victim))
                    {
                        igniteRolled->SetMaxDuration(_newDuration);
                        igniteRolled->SetDuration(_newDuration);
                        if (AuraEffect* aurEff = igniteRolled->GetEffect(EFFECT_0))
                        {
                            aurEff->SetAmount(_bank);
                            if (_newDuration > 4000)
                                aurEff->SetPeriodicTimer(_newDuration % 2000);
                        }
                    }
            return true;
        }
...
public:
    ...
    class spell_mage_ignite_AuraScript : public AuraScript
    {
        ...
        bool CheckProc(ProcEventInfo& eventInfo)
        {
            _procTarget = eventInfo.GetActionTarget();
            _caster = eventInfo.GetActor();

            if (_procTarget && _caster)
            {
                ...
                if (spellInfo->Id == 34913 || spellInfo->Id == 44461
                 || spellInfo->Id == 82739 || spellInfo->Id == 84721 || spellInfo->Id == 95969) // frostfire/flame orb cant trigger ignite
                    return false;

                // 300 munch timer
                if (_caster->HasCustomProcCooldown(SPELL_MAGE_IGNITE))
                    return false;

                if (Aura* igniteApplied = _procTarget->GetAura(SPELL_MAGE_IGNITE , _caster->GetGUID()))
                {
                    if (AuraEffect* aurEff = igniteApplied->GetEffect(EFFECT_0))
                    {
                        // No refreshing if aura duration is greater than 4s
                        if (aurEff->GetBase()->GetSpellInfo()->GetMaxDuration() < igniteApplied->GetDuration())
                            return false;

                        uint32 newDuration = 0;
                        uint32 currentIgniteDuration = igniteApplied->GetDuration();
                        if (currentIgniteDuration <= 2000)
                            newDuration = currentIgniteDuration + 4000;
                        else if (currentIgniteDuration > 2000 && currentIgniteDuration <= 4000)
                            newDuration = currentIgniteDuration + 2000;

                        // Rolling
                        if (aurEff->GetPeriodicTimer() <= 500)
                        {
                            int32 currentBank = _procTarget->GetRemainingPeriodicAmount(_caster->GetGUID(), SPELL_MAGE_IGNITE, SPELL_AURA_PERIODIC_DAMAGE);
                            float mastery = 0.0f;
                            if (Player* playerCaster = _caster->ToPlayer())
                                mastery = ceilf(playerCaster->GetMasteryAmount(76595, EFFECT_0));
                            int32 talentAmount = GetSpellInfo()->Effects[0].BasePoints;
                            AddPct(talentAmount, mastery);
                            currentBank += int32(CalculatePct(eventInfo.GetDamageInfo()->GetDamage(), talentAmount));
                            currentBank /= 3;
                            _caster->m_Events.AddEvent(new IgniteRollingEvent(_caster, _procTarget, currentBank, newDuration), _caster->m_Events.CalculateTime(aurEff->GetPeriodicTimer() * 2));
                            return false; // Dont proc, we are simulating a postponed proc using BasicEvent
                        }
                    }
                }
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            if (_procTarget && _caster)
            {
                ...
                int32 talentAmount = GetSpellInfo()->Effects[0].BasePoints;
                AddPct(talentAmount, mastery);
                int32 bank = int32(CalculatePct(eventInfo.GetDamageInfo()->GetDamage(), talentAmount));
                bank += _procTarget->GetRemainingPeriodicAmount(_caster->GetGUID(), SPELL_MAGE_IGNITE, SPELL_AURA_PERIODIC_DAMAGE);
                bank /= _procTarget->HasAura(SPELL_MAGE_IGNITE, _caster->GetGUID()) ? 3 : 2;

                _caster->CastCustomSpell(_procTarget, SPELL_MAGE_IGNITE, &bank, NULL, NULL, true, NULL, NULL, _caster->GetGUID());

                // 300ms munch timer
                _caster->AddCustomProcCooldown(SPELL_MAGE_IGNITE, 300);
            }
        }
...
// Ignite Dot - 12654 
class spell_mage_ignite_dot : public SpellScriptLoader
{
public:
    spell_mage_ignite_dot() : SpellScriptLoader("spell_mage_ignite_dot") { }

    class spell_mage_ignite_dot_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_mage_ignite_dot_SpellScript);

        bool Load() override
        {
            previousIgniteDuration = 0;
            return true;
        }
        ...
        void HandleEffectLaunchTarget(SpellEffIndex effIndex)
        {
            if (Unit* caster = GetCaster())
                if (Unit* target = GetHitUnit())
                    if (Aura* previousIgnite = target->GetAura(SPELL_MAGE_IGNITE, caster->GetGUID()))
                        previousIgniteDuration = previousIgnite->GetDuration();
        }
        ...
        void HandleEffectHitTarget(SpellEffIndex effIndex)
        {
            ...
            if (Unit* target = GetHitUnit())
            {
                // On rolling cases, the aura will not be found, this code here is fine.
                if (previousIgniteDuration != 0)
                {
                    PreventHitDefaultEffect(EFFECT_0);
                    PreventHitDefaultEffect(EFFECT_1);
                    uint32 newDuration = 0;
                    if (previousIgniteDuration <= 2000)
                        newDuration = previousIgniteDuration + 4000;
                    else if (previousIgniteDuration > 2000 && previousIgniteDuration <= 4000)
                        newDuration = previousIgniteDuration + 2000;
                    else if (previousIgniteDuration > 4000)
                        newDuration = previousIgniteDuration;

                    if (Aura* newIgniteDot = GetHitAura())
                    {
                        newIgniteDot->SetMaxDuration(newDuration);
                        newIgniteDot->SetDuration(newDuration);
                        if (newDuration > 4000)
                        {
                            if (AuraEffect* aurEff = newIgniteDot->GetEffect(EFFECT_0))
                                aurEff->SetPeriodicTimer(newDuration % 2000);
                        }
                    }
                }
            }
    ...
    private:
        uint32 previousIgniteDuration;
        void Register()
        {
            OnEffectLaunchTarget += SpellEffectFn(spell_mage_ignite_dot_SpellScript::HandleEffectLaunchTarget, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
            OnEffectHitTarget += SpellEffectFn(spell_mage_ignite_dot_SpellScript::HandleEffectHitTarget, EFFECT_0, SPELL_EFFECT_APPLY_AURA);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_mage_ignite_dot_SpellScript();
    }
};

// 84714 84765 
class spell_mage_orb_summon : public SpellScriptLoader
{
public:
    spell_mage_orb_summon() : SpellScriptLoader("spell_mage_orb_summon") { }

    class spell_mage_orb_summon_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_mage_orb_summon_SpellScript);
        ...
        void Register()
        {
            OnEffectHit += SpellEffectFn(spell_mage_orb_summon_SpellScript::ChangeSummonPos, EFFECT_0, SPELL_EFFECT_SUMMON);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_mage_orb_summon_SpellScript();
    }
};
...
void AddSC_mage_spell_scripts()
{
    ...
    new spell_mage_ice_block(); // Verified
    new spell_mage_dragon_breath(); // Verified
    new spell_mage_ignite(); // Verified
    new spell_mage_ignite_dot(); // Verified
    new spell_mage_orb_summon(); // Verified
}
