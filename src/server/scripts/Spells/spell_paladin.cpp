...
enum PaladinSpells
{
    ...
    SPELL_PALADIN_2H_WEAPON_SPECIALIZATION = 20113,
    ...
    SPELL_PALADIN_RESISTANCE_AURA = 19891,
};
...
public:
    ...
    class spell_pal_judgement_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            ...
            int32 basepoints0 = 0;
            int32 judgementSpell = 54158; // Combatlog purposes
            ...
            caster->CastCustomSpell(GetHitUnit(), judgementSpell, &basepoints0, NULL, NULL, true);
            // Long arm of the law
...
public:
    ...
    class spell_pal_divine_storm_SpellScript : public SpellScript
    {
        ...
        void CountTargets(std::list<WorldObject*>& targetList)
        {
            ...
            {
                if (Unit* caster = GetCaster())
                    caster->EnergizeBySpell(caster, GetSpellInfo()->Id, 1, POWER_HOLY_POWER);
            }
        }

        void HandleEnergizeEffect(SpellEffIndex effIndex)
        {
            // Prevent hit here and change the holypower on EFFECT_2 target select handler
            // Can only have the target count after EFFECT_2 hits, and energize effect is EFFECT_0
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_pal_divine_storm_SpellScript::HandleEnergizeEffect, EFFECT_0, SPELL_EFFECT_ENERGIZE);
...
// Anicent Fury
class spell_ancient_fury : public SpellScriptLoader
{
public:
	spell_ancient_fury() : SpellScriptLoader("spell_ancient_fury") { }

	class spell_ancient_fury_SpellScript : public SpellScript
	{
		PrepareSpellScript(spell_ancient_fury_SpellScript);

		void TargetsNum(std::list<WorldObject*>& targetList)
		{
			targetnum = targetList.size();
		}

		void ModifyDamage(SpellEffIndex)
		{
			if (GetCaster() && GetHitUnit() && targetnum)
			{
				int32 dmg = GetHitDamage();

				if (Aura* aura = GetCaster()->GetAura(86700))
				{
					dmg = GetCaster()->SpellDamageBonusDone(GetHitUnit(), GetSpellInfo(), dmg, SPELL_DIRECT_DAMAGE);
					dmg = (dmg * aura->GetStackAmount());
					dmg /= targetnum;
				}
				SetHitDamage(dmg);
			}
	...
	private:
		...
		void Register()
		{
			OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_ancient_fury_SpellScript::TargetsNum, EFFECT_0, TARGET_UNIT_SRC_AREA_ENEMY);
			OnEffectHitTarget += SpellEffectFn(spell_ancient_fury_SpellScript::ModifyDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
		}
	};

	SpellScript* GetSpellScript() const
	{
		return new spell_ancient_fury_SpellScript();
	}
};
    ...
    public:
        ...
        class spell_pal_guardian_triggered_AuraScript : public AuraScript
        {
            ...
            void Remove(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
				...
				int32 bp0 = sSpellMgr->GetSpellInfo(86704)->Effects[EFFECT_0].CalcValue(target);
...
public:
    ...
    class spell_pal_templar_s_verdict_SpellScript : public SpellScript
    {
        ...
        bool Load()
        {
            ...
            _holyPower = GetCaster()->GetPower(POWER_HOLY_POWER);
        ...
        void ChangeDamage(SpellEffIndex /*effIndex*/)
        {
            ...
            case 2: // 2 Holy Power
                damage *= 3;    // 3*30 = 90%
                break;
            case 3: // 3 Holy Power OR Divine Purpose
                damage *= 7.84;  // 7.84*30% = 225%
                break;
            }
        ...
        void HandleOnHit()
        {
            ...
            if (caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_PALADIN, 3022, EFFECT_0))
                for (Unit::AuraApplicationMap::iterator iter = caster->GetAppliedAuras().begin(); iter != caster->GetAppliedAuras().end();)
                {
                    ...
                    if (caster->HasAuraWithMechanic(1 << MECHANIC_ROOT))
                    {
                        caster->RemoveAurasWithMechanic(1 << MECHANIC_ROOT);
                        break;
                    }
                    else if (caster->HasAuraWithMechanic(1 << MECHANIC_SNARE))
                    {
                    ...
                    {
                        caster->RemoveAurasDueToSpellByDispel(aura->GetId(), GetSpellInfo()->Id, aura->GetCasterGUID(), caster, 0.5);
                        break;
                    }
                    ++iter;
                }
    ...
    class spell_pal_inquisition_SpellScript : public SpellScript
    {
        ...
        void HandleApply(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                // Inquiry of faith
                int32 baseDuration = 4000;
                if (AuraEffect const* aurEff = caster->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_PALADIN, 3025, EFFECT_1))
                    AddPct(baseDuration, aurEff->GetAmount());

                if (Aura* aura = GetHitAura())
                {
                    aura->SetMaxDuration(baseDuration * holyPower);
                    aura->SetDuration(baseDuration * holyPower);
                }
            }
        }
...
public:
    ...
    class spell_pal_seal_of_righteousness_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
        {
            ...
            // Hack: paladin 2h weapon spec has melee school mask only, but it should apply to seal of rightoussness aswell.
            if (AuraEffect* aurEff = caster->GetAuraEffect(SPELL_PALADIN_2H_WEAPON_SPECIALIZATION, EFFECT_0, caster->GetGUID()))
    ...
    class spell_pal_exorcism_SpellScript : public SpellScript
    {
        ...
        void HandleAfterHit()
        {
            ...
            if (glyph)
            {
                ...
                if (damage > 0)
                {
                    uint32 dotDamage = CalculatePct(damage, glyph->GetAmount());
                    Unit* caster = GetCaster();
                    Unit* target = GetHitUnit();
                    if (caster && target)
                    {
                        if (Aura* exoAura = caster->AddAura(GetSpellInfo(), 2, target))
                        {
                            if (AuraEffect* periodic = exoAura->GetEffect(EFFECT_1))
...
public:
    ...
    class spell_pal_divine_shield_AuraScript : public AuraScript
    {
        ...
        void Register()
        {
            AfterEffectApply += AuraEffectApplyFn(spell_pal_divine_shield_AuraScript::HandleAfterApply, EFFECT_0, SPELL_AURA_MOD_DAMAGE_PERCENT_DONE, AURA_EFFECT_HANDLE_REAL);
        }
    };
...
class spell_pal_resistance_aura : public SpellScriptLoader
{
public:
    spell_pal_resistance_aura() : SpellScriptLoader("spell_pal_resistance_aura") { }

    class spell_pal_resistance_aura_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_pal_resistance_aura_AuraScript);
        ...
        void HandleCalcAmount(AuraEffect const* aurEff, int32&amount, bool &canBeRecalculated)
        {
            if (Unit* owner = GetUnitOwner())
                ...
                {
                    canBeRecalculated = true;
                    uint32 level = playerOwner->getLevel();
                    int32 newAmount = 0;
                    if (level <= 70)
                        amount = level;
                    else if (level <= 80)
                        amount = level + (level - 70) * 5;
                    else
                        amount = level + (level - 70) * 5 + (level - 80) * 7;
                }
        }

        void Register()
        {
            DoEffectCalcAmount += AuraEffectCalcAmountFn(spell_pal_resistance_aura_AuraScript::HandleCalcAmount, EFFECT_0, SPELL_AURA_MOD_RESISTANCE_EXCLUSIVE);
        }
    };

    AuraScript* GetAuraScript() const
    {
        return new spell_pal_resistance_aura_AuraScript();
    }
};
...
public:
    ...
    class spell_pal_hammer_of_wrath_SpellScript : public SpellScript
    {
        ...
        void HandleDamage(SpellEffIndex effIndex)
        {
            ...
            if (damage > 0)
            {
                // Two handed weapon spec should also affect hammer of wrath
                if (Unit* caster = GetCaster())
                    if (AuraEffect* aurEff = caster->GetAuraEffect(20113, EFFECT_0, caster->GetGUID()))
        ...
        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_pal_hammer_of_wrath_SpellScript::HandleDamage, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
        }
    };
...
void AddSC_paladin_spell_scripts()
{
    ...
    new spell_pal_divine_shield(); // Verified
    new spell_pal_resistance_aura(); // Verified
    new spell_pal_hammer_of_wrath(); // Verified
