...
npc_moonwell_challice   100%    Mastery trinket
EndContentData */
...
public:
    ...
    struct npc_t12_mirror_imageAI : CasterAI
    {
        ...
        void InitializeAI() override
        {
            ...
            // Take care of Glyph of Mirror Image
            ...
            // Inherit Master's Threat List (not implemented atm)
        ...
        void UpdateAI(const uint32 diff) override
        {
            if (!me->getVictim())
            {
                if (Unit* owner = me->GetOwner())
                {
                    ...
                    {
                        CasterAI::EnterCombat(target);
                        CasterAI::AttackStartCaster(target, 40.0f);
                    }
                }
            }
...
public:
   ...
   struct npc_mage_orbAI : public ScriptedAI
   {
       ...
       void InitializeAI() override
       {
           ...
           me->SetReactState(REACT_PASSIVE);
           events.ScheduleEvent(EVENT_MOVE_FORWARD, 200);
           ...
           if (uint32 eventId = events.ExecuteEvent())
           {
               ...
               if (eventId == EVENT_MOVE_FORWARD)
               {
                   ...
                   if (Unit* owner = me->GetCharmerOrOwner())
                   {
                       ...
                       me->MovePositionToFirstCollision(destPos, 45.0f, 0.0f);
                       me->GetMotionMaster()->MovePoint(1, destPos, false);
                   }
               }
           }
       }
...
public:
	...
	struct npc_pet_dk_army_of_the_deadAI : public CombatAI
	{
		...
		void InitializeAI()
		{
			...
			// Heroism / Bloodlust / Time wrap/ Ancient hysteria immunity
			// TOOD: Find a better way to check for all of these spells
			me->ApplySpellImmune(0, IMMUNITY_ID, 32182, true);
			me->ApplySpellImmune(0, IMMUNITY_ID, 2825, true);
			me->ApplySpellImmune(0, IMMUNITY_ID, 80353, true);
			me->ApplySpellImmune(0, IMMUNITY_ID, 90355, true);
		}
	};
...
public:
	...
	struct npc_melee_guardianAI : CreatureAI
	{
		npc_melee_guardianAI(Creature* creature) : CreatureAI(creature) {}
...
class npc_defense_guardian : public CreatureScript
{
public:
    npc_defense_guardian() : CreatureScript("npc_defense_guardian") { }

    struct npc_defense_guardianAI : PassiveAI
    {
        npc_defense_guardianAI(Creature* creature) : PassiveAI(creature) {}

        void IsSummonedBy(Unit* owner)
        {
            me->CastSpell(owner, 86657, true); // link visual
    ...
    PassiveAI* GetAI(Creature* creature) const
    {
        return new npc_defense_guardianAI(creature);
    }
};
...
enum eTrainingDummy
{
	...
	QUEST_ARCANE_MISILES_GOBLIN                = 14008,
	QUEST_ARCANE_MISILES_ORC                   = 25149,
	QUEST_ARCANE_MISILES_BLOOD_ELF             = 10068,
	QUEST_ARCANE_MISILES_DWARF                 = 24526,
	QUEST_ARCANE_MISILES_GNOME                 = 26198,
	QUEST_ARCANE_MISILES_UNDEAD                = 24965,
	QUEST_ARCANE_MISILES_TROLL                 = 24752
};
...
public:
    ...
    struct npc_training_dummyAI : Scripted_NoMovementAI
    {
		 ...
		 void SpellHit(Unit* caster, const SpellInfo* spell)
        {
            if (spell->Id == 5143)
            {
                if (caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_GOBLIN) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_ORC) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_BLOOD_ELF) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_DWARF) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_GNOME) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_UNDEAD) == QUEST_STATUS_INCOMPLETE || 
				    caster->GetTypeId() == TYPEID_PLAYER && caster->ToPlayer()->GetQuestStatus(QUEST_ARCANE_MISILES_TROLL) == QUEST_STATUS_INCOMPLETE)
                {
                    caster->ToPlayer()->KilledMonsterCredit(44175, 0);
					caster->ToPlayer()->KilledMonsterCredit(44794, 0); // undead quest. todo: rework to an else statement just for the undead..
                }
            }
        }
                ...
                switch (eventId)
                {
                    case EVENT_SEARING_BOLT:
                    {
                        if (Unit* owner = me->GetOwner())
                        {
                            ...
                            // Look for flameshock or stormstrike
                            if (!auraTarget)
                            {
                                for (Unit* target : targets)
                                {
                            ...
                            // No target, pick whoever target is closest
                            if (!auraTarget)
                            {
                                if (!targets.empty())
                                    DoCast(targets.front(), SPELL_SEARING_BOLT, false);
                            }
                            else // found prio target
...
/*######
## npc_moonwell_challice
######*/

enum MoonWeelTrinket
{
   EVENT_BLESSING_OF_THE_MOONWELL  = 1,
   SPELL_BLESSING_OF_THE_MOONWELL  = 100403,
};

class npc_moonwell_challice : public CreatureScript
{
public:
    npc_moonwell_challice() : CreatureScript("npc_moonwell_challice") { }

    struct npc_moonwell_challiceAI : public PassiveAI
    {
        npc_moonwell_challiceAI(Creature* creature) : PassiveAI(creature) {}
        ...
        void IsSummonedBy(Unit* summoner) override
        {
            ...
            me->AddUnitState(UNIT_STATE_ROOT);
            _events.ScheduleEvent(EVENT_BLESSING_OF_THE_MOONWELL, 500);
        }
        ...
        void UpdateAI(const uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_BLESSING_OF_THE_MOONWELL:
                    {
                        Unit* target = NULL;
                        target = me->ToTempSummon()->GetSummoner();
                        if (!target)
                            target = me->GetCharmerOrOwner();
                        me->CastSpell(target, SPELL_BLESSING_OF_THE_MOONWELL, true);
                        break;
                    }
                }
            }
    ...
    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_moonwell_challiceAI(creature);
    }
};
