...
enum Texts
{
    SAY_NEFARIAN_INTRO              = 0,
    SAY_NEFARIAN_MAGMAW_DEATH       = 1,
    SAY_NEFARIAN_MAGMAW_LOW_HEALTH  = 2,
    SAY_BLAZING_INFERNO             = 3,
};
...
static const Position ejectPos = {-316.84f, -59.081f, 212.62f, 1.117f};
...
public:
    ...
    struct boss_magmawAI : public BossAI
    {
        boss_magmawAI(Creature* creature) : BossAI(creature, DATA_MAGMAW),  vehicle(creature->GetVehicleKit())
        {
            ...
            nefarianIntroDone = false;
            blazingInfernoIntroDone = false;
        }
        ...
        void EnterCombat(Unit* who)
        {
            ...
            if(IsHeroic())
            {
                if (Creature* nefarian = me->SummonCreature(NPC_HEROIC_NEFARIAN, nefarianPos))
                {
                    ...
                    if (!nefarianIntroDone)
                    {
                        nefarian->AI()->Talk(SAY_NEFARIAN_INTRO);
                        nefarianIntroDone = true;
                    }
                }
        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage)
        {
            if (!below30 && me->HealthBelowPctDamaged(30, damage))
            {
                ...
                if (Creature* nefarian = ObjectAccessor::GetCreature(*me, nefarianGUID))
                    nefarian->AI()->Talk(SAY_NEFARIAN_MAGMAW_LOW_HEALTH);
                ...
                switch(eventId)
                {
                    ...
                    case EVENT_BLAZING_INFERNO:
                        if (Creature* nefarian = me->GetCreature(*me, nefarianGUID))
                        {
                            ...
                            if (!blazingInfernoIntroDone)
                            {
                                blazingInfernoIntroDone = true;
                                nefarian->AI()->Talk(SAY_BLAZING_INFERNO);
                            }
                        }            
        ...
        private:
            ...
            Vehicle* vehicle;
            bool achievement;
            bool nefarianIntroDone;
            bool blazingInfernoIntroDone;
...
public:
    ...
    struct npc_drakonid_drudgeAI : public ScriptedAI
    {
        ...
        {
            instance = me->GetInstanceScript();
            DoCastAOE(SPELL_CHAIN_VISUAL, true);
...
class spell_magmaw_parasitic_infection_aura : public SpellScriptLoader
{
    public:
        ...
        class spell_magmaw_parasitic_infection_aura_AuraScript : public AuraScript
        {
            ...
            void Register()
            {
                OnEffectPeriodic += AuraEffectPeriodicFn(spell_magmaw_parasitic_infection_aura_AuraScript::HandleOnEffectPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DAMAGE);
            }
        };
...
class spell_magmaw_lava_parasite_ride_vehicle : public SpellScriptLoader
{
public:
    spell_magmaw_lava_parasite_ride_vehicle() : SpellScriptLoader("spell_magmaw_lava_parasite_ride_vehicle") { }

    class spell_magmaw_lava_parasite_ride_vehicle_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_magmaw_lava_parasite_ride_vehicle_SpellScript);
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_magmaw_lava_parasite_ride_vehicle_SpellScript::HandleDummy, EFFECT_2, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_magmaw_lava_parasite_ride_vehicle_SpellScript();
    }
};
...
void AddSC_boss_magmaw()
{
    ...
    new spell_magmaw_lava_parasite_ride_vehicle();
