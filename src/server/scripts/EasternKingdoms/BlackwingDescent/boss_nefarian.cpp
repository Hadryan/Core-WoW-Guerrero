    ...
    struct boss_bwd_onyxiaAI : public BossAI
    {
        ...
        void DoAction(const int32 act)
        {
            switch (act)
            {
                ...
                case ACTION_REVIVE_ONYXIA:
                {
                    enableTransportMovements(me, true);
                    if (Creature *c = me->FindNearestCreature(NPC_NEFARIUS_INTRO, 200, false))
                        c->SummonCreature(BOSS_NEFARIAN, c->GetPositionX(),  c->GetPositionY(),  c->GetPositionZ(), c->GetOrientation());
                    me->GetInstanceScript()->SetData(DATA_NEFARIAN_ELEVATOR, GO_STATE_TRANSPORT_READY);
                    events.ScheduleEvent(EVENT_ONYXIA_REVIVE, 1500);
                    break;
                }
            }
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_ONYXIA_REVIVE:
                        ...
                        me->SetHomePosition(me->GetPositionX(), me->GetPositionY(), 7.0f, me->GetOrientation());
...
class boss_bwd_nefarian : public CreatureScript
{
    ...
    struct boss_bwd_nefarianAI : public BossAI
    {
        ...
        void InitializeAI() override
        {
            ...
            me->CastSpell(me, SPELL_INTRO_SHRINK_AURA, true);
            achievement = false;
            p3Started = false;
        }
        ...
        void JustSummoned(Creature * summon)
        {
            ...
            switch (summon->GetEntry())
            {
                ...
                case NPC_DOMINION_TRACKER:
                    ...
                    break;
                case NPC_ANIMATED_BONE_WARRIOR:
                    enableTransportMovements(summon, true);
        ...
        void JustDied(Unit * /*killer*/)
        {
            Talk(9);
            summons.DespawnAll();
            instance->CompleteGuildCriteriaForGuildGroup(14006);
            ...
            if (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                   ...
                   case EVENT_NEFARIAN_INTRO_1:
                        me->RemoveAura(SPELL_INTRO_2_STALKER_TRANSFORM);
                        me->RemoveAura(SPELL_INTRO_SHRINK_AURA);
                    ...
                    case EVENT_NEFARIAN_INTRO_2:
                        ...
                        {
                            if (onyxia->HasAura(SPELL_ONYXIA_FEIGN_DEATH) && !onyxia->isInCombat())
                            {
                    ...
                    case EVENT_START_PHASE_1:
                        ...
                        me->GetMotionMaster()->MoveLand(POINT_NEFARIAN_LAND_P1, Position(-107.32f, -224.66f, 7.0f, 0.0f));
                        instance->SendEncounterUnit(ENCOUNTER_FRAME_ENGAGE, me);
                        changePhase(PHASE_1);
                        break;
...
#define GOSSIP_DRAGON_ORB "<Put your hand on the sphere.>"
...
public:
    ...
    bool OnGossipSelect(Player* player, GameObject* go, uint32 /*sender*/, uint32 action) override
    {
        ...
        if (Creature *c = go->FindNearestCreature(NPC_INVISIBLE_STALKER, 10, true))
            c->DespawnOrUnsummon();
        if (Creature* nefarian = go->FindNearestCreature(BOSS_NEFARIAN, 300.0f, false))
        {
            if (Creature* onixya = go->FindNearestCreature(BOSS_ONYXIA, 300.0f))
                onixya->AI()->DoAction(ACTION_REVIVE_ONYXIA);
        }
        else if (Creature *c = go->FindNearestCreature(NPC_NEFARIUS_INTRO, 200, true)) // Server crashed or first intro
            c->AI()->DoAction(START_INTRO);
        else if (Creature* onixya = go->FindNearestCreature(BOSS_ONYXIA, 300.0f))
            onixya->AI()->DoAction(ACTION_REVIVE_ONYXIA);
