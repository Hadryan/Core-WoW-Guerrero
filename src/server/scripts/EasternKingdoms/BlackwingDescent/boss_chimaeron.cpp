...
public:
    ...
    struct boss_chimaeronAI : public BossAI
    {
        ...
        void JustDied(Unit* /*killer*/)
        {
            ...
            instance->CompleteGuildCriteriaForGuildGroup(14005);
...
class npc_finkle_einhorn : public CreatureScript
{
    public:
        ...
        struct npc_finkle_einhornAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                if (instance->GetBossState(DATA_CHIMAERON) == NOT_STARTED && !me->HasFlag(UNIT_FIELD_FLAGS, UNIT_NPC_FLAG_GOSSIP))
                    me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                else if (me->HasFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP))
...
// This guy id is the one used to greet the players upon entering the instance (based on sniffs), weird stuff...
class npc_victor_nefarius_chimaeron : public CreatureScript
{
    public:
        ...
        enum
        {
            SAY_INSTANCE_INTRO_NORMAL = 0,
            SAY_INSTANCE_INTRO_HEROIC = 1,
            SAY_AGGRO                 = 2,
            SAY_FEUD                  = 3,
            SAY_DEATH                 = 4,
            SAY_MOCKING_SHADOWS       = 5,
            SAY_DEATH_NON_HEROIC      = 6,
        };
        ...
        struct npc_victor_nefarius_chimaeronAI : public ScriptedAI
        {
            ...
            void MoveInLineOfSight(Unit* who) override
            {
                ...
                if (who->IsWithinDist2d(me, 30.0f))
                {
                    if (InstanceScript* instance = me->GetInstanceScript())
                        if (instance->GetData(DATA_NEFARIOUS_INSTANCE_INTRO_STATE) != DONE)
                        {
                            instance->SetData(DATA_NEFARIOUS_INSTANCE_INTRO_STATE, DONE);
                            if (me->GetMap()->IsHeroic())
                                Talk(SAY_INSTANCE_INTRO_HEROIC);
                            else
                                Talk(SAY_INSTANCE_INTRO_NORMAL);
                            introDespawnTimer = 8000;
                        }
                }
            }
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (introDespawnTimer > 0)
                {
                    if (introDespawnTimer <= diff)
                    {
                        me->DisappearAndDie();
                    } else introDespawnTimer -= diff;
                }
            }

        private:
            uint32 deathTimer;
            uint32 introDespawnTimer;
        };
