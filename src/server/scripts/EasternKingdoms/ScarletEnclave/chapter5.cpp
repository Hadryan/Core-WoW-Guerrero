            ...
            if (!bIsBattle)
            {
                if (uiPhase_timer <= diff)
                {
                    ...
                    case 71:
                    {
                        ...
                        Creature* temp = me->SummonCreature(NPC_HIGHLORD_DARION_MOGRAINE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, TEMPSUMMON_TIMED_OR_CORPSE_DESPAWN, 180000);
                        temp->Dismount(); // Darion spawns with mount so we dismount him
                        JumpToNextStep(1000);
                        break;
                    }
                    case 72:
