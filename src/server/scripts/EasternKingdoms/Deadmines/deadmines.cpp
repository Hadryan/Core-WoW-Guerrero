...
public:
    ...
    struct npc_vanessa_lighting_patterAI : public ScriptedAI
    {
        ...
        void InitializeAI()
        {
            // replaced with hover movement because hover has lots of bugs at this core
            me->SetDisableGravity(true);
            me->NearTeleportTo(me->GetPositionX(), me->GetPositionY(), me->GetPositionZ() + 1.00f, me->GetOrientation());
...
public:
    ...
    struct npc_defias_cannonAI : public PassiveAI
    {
        npc_defias_cannonAI(Creature* creature) : PassiveAI(creature), TargetGUID(0)
        {
        ...
        void OnSpellClick(Unit* player, bool& result)
        {
            player->CastSpell(me, SPELL_RIDE_CANNON, true);
        }
        ...
        void UpdateAI(uint32 const uiDiff)
        {
            if (CannonBlast_Timer <= uiDiff)
            {
                if (!findNearAlly())
                {
                    ...
                    DoCast(target, SPELL_CANNONBALL, false);
...
enum GoblinOverseerSpells
{
    SPELL_THREATENING_SHOUT = 91034,
    SPELL_MOTIVATE          = 91036,
};

enum GoblinOverseerEvents
{
    EVENT_MOTIVATE          = 1,
};
...
class npc_goblin_overseer : public CreatureScript
{
    public:
        ...
        struct npc_goblin_overseerAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit * /*who*/)
            {
                DoCast(me, SPELL_THREATENING_SHOUT, true);
                _events.ScheduleEvent(EVENT_MOTIVATE, urand(10000, 15000));
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = _events.ExecuteEvent())
                {
                    ...
                    {
                        case EVENT_MOTIVATE:
                            if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0, 20.00f, true))
                                DoCast(target, SPELL_MOTIVATE, false);
                            _events.ScheduleEvent(EVENT_MOTIVATE, urand(10000, 15000));
                            break;
                        default:
                            break;
...
enum MiningPowderSpells
{
    SPELL_EXPLODE       = 90096,
    SPELL_EXPLODE_STUN  = 89769,
};
...
class npc_mining_powder : public CreatureScript
{
    public:
        ...
        struct npc_mining_powderAI : public PassiveAI
        {
            npc_mining_powderAI(Creature* creature) : PassiveAI(creature) {}
            ...
            void JustDied(Unit* killer)
            {
                ...
                DoCast(me, SPELL_EXPLODE_STUN, true);
...
enum OafLackeySpells
{
    SPELL_AXE_TO_THE_HEAD   = 90098,
    SPELL_ENRAGE            = 8599,
};

enum OafLackeyEvents
{
    EVENT_AXE_TO_THE_HEAD   = 1,
};
...
class npc_oaf_lackey : public CreatureScript
{
    public:
        ...
        struct npc_oaf_lackeyAI : public ScriptedAI
        {
            ...
            {
                if (me->HealthBelowPctDamaged(30, damage) && !me->HasAura(SPELL_ENRAGE))
                    DoCast(me, SPELL_ENRAGE, true);
            }

            void EnterCombat(Unit * /*who*/)
            {
                _events.ScheduleEvent(EVENT_AXE_TO_THE_HEAD, urand(10000, 15000));
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_AXE_TO_THE_HEAD:
                            DoCastVictim(SPELL_AXE_TO_THE_HEAD);
                            _events.ScheduleEvent(EVENT_AXE_TO_THE_HEAD, urand(10000, 15000));
                            break;
                        default:
                            break;
...
enum DefiasEnforcerSpells
{
    SPELL_BLOODBATH   = 90925,
    SPELL_CHARGE      = 90928,
};

enum DefiasEnforcerEvents
{
    EVENT_BLOODBATH   = 1,
};
    ...
    public:
        ...
        struct npc_defias_enforcerAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit * /*who*/)
            {
                ...
                _events.ScheduleEvent(EVENT_BLOODBATH, urand(10000, 15000));
            }
...
enum DefiasBloodwizardSpells
{
    SPELL_BLOOD_CHANNELING   = 90389,
    SPELL_BLOODBOLT          = 90938, // 3cast, 4 dura
    SPELL_RAGEZONE           = 90932, // 30 sec cd 1 sec cast
};

enum DefiasBloodwizardEvents
{
    EVENT_BLOODBOLT   = 1,
    EVENT_RAGEZONE    = 2,
};
...
class npc_defias_bloodwizard : public CreatureScript
{
    public:
        ...
        struct npc_defias_bloodwizardAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                DoCast(me, SPELL_BLOOD_CHANNELING, false);
                _events.Reset();
            }

            void EnterCombat(Unit * /*who*/)
            {
                _events.ScheduleEvent(EVENT_RAGEZONE, urand(5000, 15000));
                _events.ScheduleEvent(EVENT_BLOODBOLT, 3000);
            }
                ...
                if (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_BLOODBOLT:
                            DoCastRandom(SPELL_BLOODBOLT, 40.00f);
                            _events.ScheduleEvent(EVENT_BLOODBOLT, urand(3000, 5000));
                            break;
                        case EVENT_RAGEZONE:
                            DoCastVictim(SPELL_RAGEZONE);
                            _events.ScheduleEvent(EVENT_BLOODBATH, urand(30000, 35000));
                            break;
                        default:
                            break;
                    }
...
enum DefiasShadowguardEvents
{
    EVENT_SINISTER_STRIKE   = 1,
    EVENT_WHIRLING_BLADES   = 2,
};
    ...
    public:
        ...
        struct npc_defias_shadowguardAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit * /*who*/)
            {
                ...
                _events.ScheduleEvent(EVENT_SINISTER_STRIKE, 3000);
                _events.ScheduleEvent(EVENT_WHIRLING_BLADES, urand(5000, 10000));
            }

            void DamageTaken(Unit* attacker, uint32& damage)
            {
                if (!evasionCasted && me->HealthBelowPctDamaged(20, damage))
                {
                    evasionCasted = true;
                    DoCast(me, SPELL_EVASION, true);
                }
            }
                ...
                if (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_SINISTER_STRIKE:
                            ...
                            _events.ScheduleEvent(EVENT_SINISTER_STRIKE, urand(3000, 5000));
                            break;
                        case EVENT_WHIRLING_BLADES:
                            DoCastRandom(SPELL_WHIRLING_BLADES, 35.00f, true);
                            _events.ScheduleEvent(EVENT_WHIRLING_BLADES, urand(15000, 20000));
                            break;
                        default:
                            break;
                    }
...
enum OgreHenchmanSpells
{
    SPELL_UPPERCUT          = 91045,
    SPELL_OGRE_MOTIVATION   = 89652,
};

enum OgreHenchmanEvents
{
    EVENT_UPPERCUT          = 1,
    EVENT_OGRE_MOTIVATION   = 2,
};
...
class npc_ogre_henchman : public CreatureScript
{
    public:
        ...
        struct npc_ogre_henchmanAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit * /*who*/)
            {
                _events.ScheduleEvent(EVENT_OGRE_MOTIVATION, 1000);
                _events.ScheduleEvent(EVENT_UPPERCUT, urand(5000, 10000));
            }
            ...
            void JustDied(Unit* killer)
            {
                ...
                for (Unit* unit : targets)
                    if (Aura* ogreMotivation = unit->GetAura(SPELL_OGRE_MOTIVATION, me->GetGUID()))
                        ogreMotivation->Remove();
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_OGRE_MOTIVATION:
                            DoCast(me, SPELL_OGRE_MOTIVATION, true);
                            _events.ScheduleEvent(EVENT_OGRE_MOTIVATION, 30000);
                            break;
                        case EVENT_UPPERCUT:
                            DoCastVictim(SPELL_UPPERCUT);
                            _events.ScheduleEvent(EVENT_UPPERCUT, urand(15000, 20000));
                            break;
                        default:
                            break;
                    }
...
public:
    ...
    struct npc_goblin_engineerAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            if (me->FindNearestGameObject(17154, 20.0f)) // Door before Foe Reaper 5000
                me->AddAura(78087, me);
            else
                me->AddAura(57626, me);
        }
