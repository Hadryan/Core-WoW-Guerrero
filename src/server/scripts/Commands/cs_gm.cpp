...
public:
    ...
    static bool HandleGMVisibleCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
            handler->PSendSysMessage(LANG_YOU_ARE, handler->GetSession()->GetPlayer()->isGMVisible() ? handler->GetmonsterString(LANG_VISIBLE) : handler->GetmonsterString(LANG_INVISIBLE));
            return true;
        }
