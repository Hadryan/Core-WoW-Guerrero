...
public:
    ...
    static bool HandleBanCharacterCommand(ChatHandler* handler, char const* args)
    {
		...
		switch (sWorld->BanCharacter(name, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName().c_str() : "Console"))
        {
            case BAN_SUCCESS:
            {
    ...
    static bool HandleBanHelper(BanMode mode, char const* args, ChatHandler* handler)
    {
		...
		switch (sWorld->BanAccount(mode, nameOrIP, durationStr, reasonStr, handler->GetSession() ? handler->GetSession()->GetPlayerName().c_str() : "Console"))
        {
            case BAN_SUCCESS:
                if (atoi(durationStr) > 0)
    ...
    static bool HandleBanInfoHelper(uint32 accountId, char const* accountName, ChatHandler* handler)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetmonsterString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetmonsterString(LANG_BANINFO_YES) : handler->GetmonsterString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
    ...
    static bool HandleBanInfoCharacterCommand(ChatHandler* handler, char const* args)
    {
        ...
        do
        {
            ...
            std::string banTime = permanent ? handler->GetmonsterString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
            handler->PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
                fields[0].GetCString(), banTime.c_str(), active ? handler->GetmonsterString(LANG_BANINFO_YES) : handler->GetmonsterString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
        }
        while (result->NextRow());
    ...
    static bool HandleBanInfoIPCommand(ChatHandler* handler, char const* args)
    {
        ...
        handler->PSendSysMessage(LANG_BANINFO_IPENTRY,
            fields[0].GetCString(), fields[1].GetCString(), permanent ? handler->GetmonsterString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
            permanent ? handler->GetmonsterString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
