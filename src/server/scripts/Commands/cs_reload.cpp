...
public:
	...
	std::vector<ChatCommand> GetCommands() const override
	{
		...
		static std::vector<ChatCommand> reloadCommandTable =
		{
			...
			{ "spell_regulator", SEC_ADMINISTRATOR, true, &HandleReloadSpellRegulatorCommand, "" },
			{ "player_text", SEC_ADMINISTRATOR, true, &HandleReloadPlayerTextsCommand, "" },
		};
	...
	static bool HandleReloadPlayerTextsCommand(ChatHandler* p_Handler, char const* /*p_Args*/)
	{
		sPlayerTextMgr->LoadTextFromDB(true);
		p_Handler->SendSysMessage("Successfully reloaded player_text table");
		return true;
	}
