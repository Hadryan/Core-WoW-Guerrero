...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> commandTable =
        {
            ...
            { "getpos",             SEC_PLAYER,             false, HandleGetPosCommand,                 "" },
        };
        return commandTable;
    }

    static bool HandleGetPosCommand(ChatHandler* p_Handler, char const* args)
    {
        Unit* l_Selected = p_Handler->getSelectedUnit();
        if (!l_Selected)
            l_Selected = p_Handler->GetSession()->GetPlayer();
        ...
        Position l_Pos;
        l_Selected->GetPosition(&l_Pos);

        ss << l_Pos.GetPositionX() << "f, "
            << l_Pos.GetPositionY() << "f, "
            << l_Pos.GetPositionZ() << "f, "
            << l_Pos.GetOrientation() << "f };"
            ;
    ...
    static bool HandleGPSCommand(ChatHandler* handler, char const* args)
    {
        ...
        CellCoord cellCoord = monster::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
        ...
        GridCoord gridCoord = monster::ComputeGridCoord(object->GetPositionX(), object->GetPositionY());
    ...
    static bool HandleSummonCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            handler->PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), handler->GetmonsterString(LANG_OFFLINE));
    ...
    static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            {
                handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target == handler->GetSession()->GetPlayer() ? handler->GetmonsterString(LANG_YOU) : nameLink.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }

            target->RemoveSpellCooldown(spellIid, true);
            handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spellIid, target == handler->GetSession()->GetPlayer() ? handler->GetmonsterString(LANG_YOU) : nameLink.c_str());
        }
        return true;
    }
    ...
    static bool HandleNearGraveCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (graveyard)
        {
            ...
            std::string team_name = handler->GetmonsterString(LANG_COMMAND_GRAVEYARD_NOTEAM);

            if (team == 0)
                team_name = handler->GetmonsterString(LANG_COMMAND_GRAVEYARD_ANY);
            else if (team == HORDE)
                team_name = handler->GetmonsterString(LANG_COMMAND_GRAVEYARD_HORDE);
            else if (team == ALLIANCE)
                team_name = handler->GetmonsterString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
    ...
    static bool HandleAddItemCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG("misc", handler->GetmonsterString(LANG_ADDITEM), itemId, count);
    ...
    static bool HandleAddItemSetCommand(ChatHandler* handler, char const* args)
    {
        ...
        TC_LOG_DEBUG("misc", handler->GetmonsterString(LANG_ADDITEMSET), itemSetId);
    ...
    static bool HandlePInfoCommand(ChatHandler* handler, char const* args)
    {
        ...
        std::string userName    = handler->GetmonsterString(LANG_ERROR);
        std::string eMail       = handler->GetmonsterString(LANG_ERROR);
        std::string lastIp      = handler->GetmonsterString(LANG_ERROR);
        uint32 security         = 0;
        std::string lastLogin   = handler->GetmonsterString(LANG_ERROR);
        ...
        handler->PSendSysMessage(LANG_PINFO_ACCOUNT, (target ? "" : handler->GetmonsterString(LANG_OFFLINE)), nameLink.c_str(), GUID_LOPART(targetGuid), userName.c_str(), accId, eMail.c_str(), security, lastIp.c_str(), lastLogin.c_str(), latency);
    ...
    static bool HandleRespawnCommand(ChatHandler* handler, char const* /*args*/)
    {
        ...
        monster::RespawnDo u_do;
        monster::WorldObjectWorker<monster::RespawnDo> worker(player, u_do);

        TypeContainerVisitor<monster::WorldObjectWorker<monster::RespawnDo>, GridTypeMapContainer > obj_worker(worker);
