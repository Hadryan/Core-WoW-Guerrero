...
namespace monster
{
    class SaveAll
    {
        public:
            SaveAll() {}
            void operator()(Creature* u) const { u->SaveToDB(); }
...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> npcSetCommandTable =
        {
            ...
            { "randommove",     SEC_GAMEMASTER,     false, &HandleNpcSetRandomMoveCommand,     "" },
            //}
        };
        static std::vector<ChatCommand> npcCommandTable =
        {
            ...
            { "get",            SEC_GAMEMASTER,     false, &HandleNpcGetCommand,               "" },
            { "saveallzone",    SEC_GAMEMASTER,     false, &HandleNpcSaveAllZone,              "" },
    ...
    static bool HandleNpcSaveAllZone(ChatHandler* p_Handler, char const* /*p_Args*/)
    {
    ...
    static bool HandleNpcSetRandomMoveCommand(ChatHandler* p_Handler, char const* /*p_Args*/)
    {
        ...
        WorldDatabase.PQuery("UPDATE `creature` SET `MovementType` = 1, `spawndist` = 5.0 WHERE `guid` = %u", l_Creature->GetDBTableGUIDLow());

        p_Handler->PSendSysMessage("Set '%s' to use random movement with spawndist (5.0f) GUID: (%u)", l_Creature->GetName().c_str(), l_Creature->GetDBTableGUIDLow());

        l_Creature->GetMotionMaster()->MoveRandom(l_Distance);
    ...
    */
    ...
    static bool HandleNpcGetCommand(ChatHandler* p_Handler, char const* /*args*/)
    {
        Creature* l_Creature = p_Handler->getSelectedCreature();
        ...
        p_Handler->PSendSysMessage("Entry: %u Name: %s LootID: %u", p_Template->Entry, p_Template->Name.c_str(), p_Template->lootid);
