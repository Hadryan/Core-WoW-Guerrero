...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> debugCommandTable =
        {
			...
			{ "hasaura",        SEC_ADMINISTRATOR,  false, &HandleDebugHasAuraCommand,         "",},
            { "stabilitytest",  SEC_ADMINISTRATOR,  true,  &HandleDebugStabilityTestCommand,   "" },
        };
    ...
    static bool HandleDebugStabilityTestCommand(ChatHandler* handler, char const* args)
    {
        if (!*args)
        {
        ...
        std::string strVal = std::string(strtok((char*)args, " "));
        ...
        if (strVal == "on")
        {
            char* strNumber = strtok(NULL, " ");
            if (strNumber == nullptr)
                return false;

            uint32 number = atoi(strNumber);
            ...
            if (result)
            {
                do
                {
                    ...
                    // WorldSession* newSession = new WorldSession(accountId, name.c_str(), accountId, nullptr, SEC_ADMINISTRATOR, 4, 0, LOCALE_esES, 0, false);

                    WorldSession* newSession = new WorldSession(accountId, NULL, SEC_ADMINISTRATOR, 4, 0, LOCALE_esES, 0, false);

                    newSession->SetStabilityTest(true);
                    newSession->LoadGlobalAccountData();
                    // newSession->LoadTutorialsData();
                    sWorld->AddSession(newSession);

                    newSession->LoginPlayer(guid);
	...
	static bool HandleDebugHasAuraCommand(ChatHandler* handler, const char* args)
	{
        ...
        handler->PSendSysMessage("unit %s doesn't have aura %i", unit->GetName().c_str(), auraid);
