...
public:
    ...
    std::vector<ChatCommand> GetCommands() const override
    {
        ...
        static std::vector<ChatCommand> serverCommandTable =
        {
            ...
            { "shell",          SEC_ADMINISTRATOR,  true,  &HandleServerShellCommand,               "" },
        };
    ...
    static bool HandleServerShellCommand(ChatHandler* handler, char const* args)
    {
#if PLATFORM == PLATFORM_WINDOWS
        handler->SendSysMessage("This command is only usable on linux systems");
        return false;
#endif
        ...
        {
            system("./coreshell.sh restore");
            sWorld->SetServerUpdateState(SERVER_STATE_RESTORING);
            handler->SendSysMessage("Core restore started");
            return true;
        }

        handler->SendSysMessage("Wrong subcommand! Please use UPDATE for core update or RESTORE for restoring the last version!");
        return true;
    }
