...
// For quest: Sludge-Investigation
class npc_muddy : public CreatureScript
{
public:
    npc_muddy() : CreatureScript("npc_muddy") { }

    CreatureAI* GetAI(Creature* creature) const
    {
        return new npc_muddyAI(creature);
    }

    struct npc_muddyAI : public ScriptedAI
    {
        npc_muddyAI(Creature* creature) : ScriptedAI(creature)
        {
        }
        ...
        void OnSpellClick(Unit* clicker, bool& /*result*/) override
        {
            if (Player* player = clicker->ToPlayer())
            {
                if (player->GetQuestStatus(29087) == QUEST_STATUS_INCOMPLETE)
                {
                    const std::string monsterTexts[5] = { 
                        "Grass, sand, and sludge mix together to form the vague resemblance of a footprint.",
                        "The tracks are smeared, almost as if the creature drags its feet.",
                        "Mixed in with the mud are traces of oily sludge from the nearby rig.",
                        "This trail is bloody. You follow it to a nearby bush, where you find the remains of the creature's last meal.",
                        "These tracks are fresh, and they lead down into the water..." 
                    };
                    me->MonsterTextEmote(monsterTexts[urand(0, 4)].c_str(), 0, true);
                    me->DespawnOrUnsummon(1);
                    player->KilledMonsterCredit(52166, 0);
                }
            }
        }
    };
