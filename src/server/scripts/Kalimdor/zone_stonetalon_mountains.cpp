...
/*######
Quest: Ashes to Ashes, issues when handling in the database
######*/
class spell_78140 : public SpellScriptLoader
{
public:
    spell_78140() : SpellScriptLoader("spell_78140") { }

    class spell_78140_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_78140_SpellScript);

        void HandleActiveObject(SpellEffIndex /*effIndex*/)
        {
            GetHitGObj()->SetLootState(GO_ACTIVATED);
        }

        void Register() override
        {
            OnEffectHitTarget += SpellEffectFn(spell_78140_SpellScript::HandleActiveObject, EFFECT_1, SPELL_EFFECT_ACTIVATE_OBJECT);
        }
    };

    SpellScript* GetSpellScript() const override
    {
        return new spell_78140_SpellScript();
    }
};

class go_tent : public GameObjectScript
{
public:
    go_tent() : GameObjectScript("go_tent") { }
