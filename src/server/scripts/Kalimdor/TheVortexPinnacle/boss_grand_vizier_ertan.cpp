    ...
    public:
        ...
        struct boss_grand_vizier_ertanAI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/) override
            {
                ...
                for (uint8 i = 0; i < 8; i++)
                {
                    GetPositionWithDistInOrientation(me, 27.5f, Position::NormalizeOrientation(stOri), x, y);

                    if (TempSummon* m_vortex = me->SummonCreature(NPC_ERTAN_VORTEX, x, y, me->GetPositionZ()))
    ...
    void DoAction(int32 const actionId) override
    {
        switch (actionId)
        {
            case ACTION_VORTEX_PULL_FORWARD:
                ...
                {
                    GetPositionWithDistInOrientation(me, dist - 5.5f, me->GetAngle(ertan), x, y);
