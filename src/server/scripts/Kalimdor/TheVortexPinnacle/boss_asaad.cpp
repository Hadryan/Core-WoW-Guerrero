                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_FIELD_4:
                            ...
                            // Storm Targets Visual
                            for (uint8 i = 0; i < 15; i++)
                                me->SummonCreature(NPC_STORM_TARGET, me->GetHomePosition().GetPositionX() + frand(-35.0f, 35.0f), me->GetHomePosition().GetPositionY() + frand(-35.0f, 35.0f), me->GetHomePosition().GetPositionZ(), me->GetOrientation(), TEMPSUMMON_TIMED_DESPAWN, 6 * IN_MILLISECONDS);

                            events.ScheduleEvent(EVENT_FIELD_5, 1000);
                            break;
                        case EVENT_FIELD_5:
                            me->StopMoving();
...
// Unstable Grounding Field 46492
struct npc_unstable_grounding_field : public ScriptedAI
{
    npc_unstable_grounding_field(Creature* creature) : ScriptedAI(creature)
    {
...
// Supremacy of the Storm 87553, 93994
class spell_asaad_supremacy_of_the_storm : public SpellScript
{
    ...
    void HandleEffectHitTarget(SpellEffIndex /*effIndex*/)
    {
        ...
        if (GetCaster()->GetExactDist2d(GetHitUnit()) < 5.0f)
            SetHitDamage(0);
    }

    void Register() override
    {
        OnEffectHitTarget += SpellEffectFn(spell_asaad_supremacy_of_the_storm::HandleEffectHitTarget, EFFECT_0, SPELL_EFFECT_SCHOOL_DAMAGE);
    }
};

// Supremacy of the Storm Dummy 86715
class spell_asaad_supremacy_of_the_storm_dummy : public SpellScript
{
    PrepareSpellScript(spell_asaad_supremacy_of_the_storm_dummy);

    void SelectTarget(std::list<WorldObject*>& targets)
    {
        targets.remove_if([=](WorldObject* target) { return target->GetEntry() != NPC_STORM_TARGET; });
    }

    void Register() override
    {
        OnObjectAreaTargetSelect += SpellObjectAreaTargetSelectFn(spell_asaad_supremacy_of_the_storm_dummy::SelectTarget, EFFECT_0, TARGET_UNIT_SRC_AREA_ENTRY);
    }
};
...
{
    new boss_asaad();
    new creature_script<npc_unstable_grounding_field>("npc_unstable_grounding_field");
    new spell_script<spell_asaad_supremacy_of_the_storm>("spell_asaad_supremacy_of_the_storm");
    new spell_script<spell_asaad_supremacy_of_the_storm_dummy>("spell_asaad_supremacy_of_the_storm_dummy");
}