            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                if (instance)
                {
                    ...
                    for (uint8 i = 0; i < NUM_TORNADOS; ++i)
                    {
                        ...
                        if (Creature* Tornado = me->GetMap()->SummonCreature(NPC_SAND_VORTEX, Point, 0, 120000))
            ...
            void SpawnNextCrystal()
            {
                ...
                if (Creature* Trigger = me->GetMap()->SummonCreature(NPC_OSSIRIAN_TRIGGER, CrystalCoordinates[CrystalIterator]))
                {
                   ...
                   /* if (GameObject* Crystal = Trigger->SummonGameObject(GO_OSSIRIAN_CRYSTAL,
                                                       ...
                                                       0, 0, 0, 0, 0, uint32(-1)))*/
					if (GameObject* Crystal = Trigger->FindNearestGameObject(GO_OSSIRIAN_CRYSTAL, 10))
					{
                        CrystalGUID = Crystal->GetGUID();
						Crystal->SetPhaseMask(1, true);
