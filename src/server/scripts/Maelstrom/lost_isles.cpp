...
// Clyclone of Elements 36178
// Quest Warchief's Revenge 14243
class npc_tornade_gob : public CreatureScript
{
public:
	npc_tornade_gob() : CreatureScript("npc_tornade_gob") { }

	struct npc_tornade_gobAI : public VehicleAI
	{
		npc_tornade_gobAI(Creature* creature) : VehicleAI(creature) { Initialize(); }
		...
		void PassengerBoarded(Unit* passenger, int8 seatId, bool apply) override
		{
			if (Player* player = passenger->ToPlayer())
				...
				me->GetMotionMaster()->MovePath(3617800, true);
				}
				else
				{
			...
			if (Creature* thrall = me->FindNearestCreature(36161, 30.0f))
				thrall->AI()->Talk(0, passenger->GetGUID(), CHAT_MSG_RAID_BOSS_EMOTE);
		}

		void KilledUnit(Unit* victim) override
		{
			if (Player* player = ObjectAccessor::GetPlayer(*me, m_playerGUID))
				if (player->GetReqKillOrCastCurrentCount(14243, 36176) == 50 && !m_backToLostIsland)
				{	
				    m_backToLostIsland = true;
					me->GetMotionMaster()->MovePath(3617801, false);
					me->SetSpeed(MOVE_FLIGHT, 4.17f, true);
					Talk(0, CHAT_MSG_RAID_BOSS_EMOTE);				
				}
		}
		...
		{
			me->SetCanFly(true);
			if (m_backToLostIsland && type == WAYPOINT_MOTION_TYPE && id == 13)
    ...
    bool OnGossipHello(Player* player, GameObject* go)
    {
        if (player->GetQuestStatus(24817) == QUEST_STATUS_INCOMPLETE)
        {
            if (Creature *t = player->SummonCreature(38318, go->GetPositionX(), go->GetPositionY(),  go->GetPositionZ() + 2,  go->GetOrientation(), TEMPSUMMON_CORPSE_TIMED_DESPAWN, 30*IN_MILLISECONDS))
            {
                ...
                monster::CreatureListSearcher<SparkSearcher> searcher(t, temp, check);
...
public:
    ...
    bool OnQuestReward(Player* player, Creature* creature, const Quest *_Quest, uint32 )
    {
        if (_Quest->GetQuestId() == 24864)
        {
            ...
            monster::CreatureListSearcher<YoungNagaSearcher> searcher(creature, temp, check);
...
{
    // new npc_Zapnozzle(); // Using SAI instead
    // new npc_singe_bombe(); // Using SAI instead
    // new npc_mineur_gob(); // Using SAI instead
    // new npc_Mechumide(); // Using SAI instead
    new spell_68281();
    ...
    // new npc_megs_isle_gob(); // Using SAI instead
    new npc_crack_isle_gob();
    // new npc_canon_gob(); // Using SAI instead
    new spell_boot_damage();
    new spell_boot_gob();
    // new npc_oom_isle_gob(); // Using SAI instead
    new npc_ceint();
