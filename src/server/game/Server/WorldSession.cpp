...
WorldSession::WorldSession(uint32 id, WorldSocket* sock, AccountTypes sec, uint8 expansion, time_t mute_time, LocaleConstant locale, uint32 recruiter, bool isARecruiter):
    ...
    m_IsStabilityTestSession(false),
...
bool WorldSession::Update(uint32 diff, PacketFilter& updater)
{
	...
	uint32 accId = sWorld->getIntConfig(CONFIG_BOTS_ACC_GUID);
	/*if (IsConnectionIdle() && GetAccountId() != accId)
        m_Socket->CloseSocket();*/
            ...
            _recvQueue.next(packet, updater))
    {
        ...
        if (opcodeProcessTime >= opcodeMinTime)
        {
            ...
            stmt->setInt32(0, packet->GetOpcode());
            stmt->setInt32(1, opcodeProcessTime);
    ...
    if (updater.ProcessLogout() && !m_IsStabilityTestSession)
    {
...
{
    // log out any PlayerBots owned in this WorldSession
    /*PlayerBotMap::const_iterator it = m_playerBots.begin();
    for(PlayerBotMap::const_iterator itr = m_playerBots.begin(); itr != m_playerBots.end(); ++itr) 
	{
    	uint64 guid = itr->first;
    	LogoutPlayerBot(guid, Save);
    }*/
...
{
    char const* format = GetmonsterString(string_id);
    if (format)
    {
        va_list ap;
        char szStr[1024];
...
const char *WorldSession::GetmonsterString(int32 entry) const
{
    return sObjectMgr->GetmonsterString(entry, GetSessionDbLocaleIndex());
}
