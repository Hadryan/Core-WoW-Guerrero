...
/// World constructor
World::World() : m_retailDelayTicks(0)
{
    m_playerLimit = 0;
...
void World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && AccountMgr::IsPlayerAccount(s->GetSecurity()) && !HasRecentlyDisconnected(s) && s->GetAccountId() != 55203 && s->GetAccountId() != 2105 && s->GetAccountId() != 18482)
    {
        AddQueuedPlayer(s);
...
void World::LoadConfigSettings(bool reload)
{
    ...
    // Custom stuff
    rate_values[RATE_BONUS_XP_PVP_VALUE] = sConfigMgr->GetFloatDefault("Rate.XP.PvPBonus.Percent", 10.0f);
    m_bool_configs[CONFIG_BONUS_XP_PVP_ENABLE] = sConfigMgr->GetBoolDefault("Config.XP.PvPBonus.Enable", true);
    rate_values[RATE_BONUS_XP_DUNGEON_VALUE] = sConfigMgr->GetFloatDefault("Rate.XP.Dungeon.Percent", 10.0f);
    m_bool_configs[CONFIG_BONUS_XP_DUNGEON_ENABLE] = sConfigMgr->GetBoolDefault("Config.XP.Dungeon.Enable", true);
    m_bool_configs[CONFIG_BONUS_TITLE_REQUIREMENT] = sConfigMgr->GetBoolDefault("Config.No.Title.Requirements", true);
    // End of custom stuff
    ...
    m_int_configs[CONFIG_CREATURE_STOP_FOR_PLAYER]                   = sConfigMgr->GetIntDefault("Creature.MovingStopTimeForPlayer", 30000);
    ...
    // Move Spline
    m_bool_configs[CONFIG_MMAPS_SPLINE_LOCKING] = sConfigMgr->GetBoolDefault("mmap.enableSplineLocking", false);
    ...
    m_int_configs[CONFIG_MAX_WHO] = sConfigMgr->GetIntDefault("MaxWhoListReturns", 1000); // Custom changed
    ...
    m_bool_configs[CONFIG_DISCORD_LOG] = sConfigMgr->GetBoolDefault("Discord.Enable", true);
    ...
    m_int_configs[CONFIG_ANTICHEAT_REPORTS_AMOUNT] = sConfigMgr->GetIntDefault("Anticheat.ReportsForAction", 30);
    m_int_configs[CONFIG_ANTICHEAT_ACTION] = sConfigMgr->GetIntDefault("Anticheat.Action", 0);
    m_bool_configs[CONFIG_ANTICHEAT_ACTION_ANNOUNCE] = sConfigMgr->GetIntDefault("Anticheat.AnnounceAction", true);
    m_int_configs[CONFIG_ANTICHEAT_BAN_DURATION] = sConfigMgr->GetIntDefault("Anticheat.BanDuration", 86400);
    m_int_configs[CONFIG_MOVEMENT_CHECKS_ACCESSLEVEL] = sConfigMgr->GetIntDefault("Anticheat.SkipGMLevel", 1);
    m_int_configs[CONFIG_MOVEMENT_CHECKS_MASK] = sConfigMgr->GetIntDefault("MovementChecker.EnableMask", 0);
    ...
    m_int_configs[CONFIG_BOTS_ACC_GUID] = sConfigMgr->GetIntDefault("Bots.AccGuid", 0);
    m_int_configs[CONFIG_BOTS_MAX] = sConfigMgr->GetIntDefault("Bots.MaxCount", 10);
...
void World::SetInitialWorldSettings()
{
    ...
    TC_LOG_INFO("server.loading", "Loading Taxi Flight Data...");
    sObjectMgr->LoadTaxiFlightNodes();                            // must be after LoadCreatureTemplates() and LoadCreatures()
    ...
    m_timers[WUPDATE_LOGINB].SetInterval(30 * IN_MILLISECONDS);
    ...
    m_timers[WUPDATE_RETAILDELAY].SetInterval(400);
    TC_LOG_INFO("server.loading", "Retail delay timer set to 400ms...");
...
void World::Update(uint32 diff)
{
    ...
    if (m_timers[WUPDATE_RETAILDELAY].Passed())
    {
        m_timers[WUPDATE_RETAILDELAY].Reset();
        ++m_retailDelayTicks;
        //std::cout << "Tick count: " << m_retailDelayTicks << std::endl;

        for (auto unit : m_unitsRequiringVisibilityUpdates)
        {
            if (unit != nullptr && unit->IsInWorld())
            {
                //std::cout << "Updating unit visibility for " << unit->GetName() << std::endl;
                unit->UpdateObjectVisibility(false);
            }
        }
        m_unitsRequiringVisibilityUpdates.clear();
    }
    ...
    if (m_timers[WUPDATE_LOGINB].Passed())
    {
        uint32 count = 0;
        uint32 m_MaxPlayerbots = sWorld->getIntConfig(CONFIG_BOTS_MAX);
        m_timers[WUPDATE_LOGINB].Reset();
        ++count;
...
void World::LoginB()
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
        if (itr->second && itr->second->GetPlayer() && itr->second->GetPlayer()->IsInWorld())
        {
            std::list<std::string>* names = itr->second->GetPlayer()->GetCharacterList();
            ...
            for (std::list<std::string>::iterator iter = names->begin(); iter != names->end(); iter++)
            {
                ...
                /*if (itr->second->GetPlayer()->GetSession()->GetPlayerBot(guid) != NULL)
                    return;*/
                itr->second->GetPlayer()->GetSession()->AddPlayerBot(guid);
            }
        }
}
...
uint64 World::GetRetailDelayTickCount()
{
    return m_retailDelayTicks;
}
