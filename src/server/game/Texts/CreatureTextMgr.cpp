        ...
        for (CreatureTextHolder::iterator groupItr = mapItr->second.begin(); groupItr != mapItr->second.end(); ++groupItr)
        {
            for (CreatureTextGroup::iterator textIter = groupItr->second.begin(); textIter != groupItr->second.end(); ++textIter)
            {
                ...
                if (uint32 bctId = sObjectMgr->FindBroadcastTextId(textIter->text, textIter->sound, &soundIdMatch, &textMatch))
                {
                    ...
                    ++count;
                   // No need to get it spammed on console: //TC_LOG_INFO("server.loading", "CreatureTextMgr::FillMissingBroadcastTexts: Found broadcast text (Id: %u) for creature text (Entry: %u, Group: %u, Id: %u). Text %s. SoundIds %s.", bctId, textIter->entry, textIter->group, textIter->id, textMatch ? "match" : "not match", soundIdMatch ? "match" : "not match");
