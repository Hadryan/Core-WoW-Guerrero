...
void LoadDisables()
{
    ...
    do
    {
        ...
        switch (type)
        {
            ...
            case DISABLE_TYPE_MMAP_FOR_UNIT_ENTRY:
            {
                QueryResult resultDB = WorldDatabase.PQuery("SELECT entry FROM creature_template WHERE entry = %u;", entry);
                if (!resultDB)
                {
                    TC_LOG_ERROR("sql.sql", "Creature entry %u from `disables` doesn't exist in creature_template, skipped.", entry);
                    continue;
                }
                TC_LOG_INFO("misc", "Pathfinding disabled for creature id %u.", entry);
                break;
            }
            default:
                break;
...
bool IsDisabledFor(DisableType type, uint32 entry, Unit const* unit, uint8 flags)
{
    ...
    switch (type)
    {
        ...
        case DISABLE_TYPE_MMAP_FOR_UNIT_ENTRY:
            if (entry != 0)
            {
                // Pathfinding disabled for specific entry
                if (entry == itr->first)
                    return true;
            }
            break;
