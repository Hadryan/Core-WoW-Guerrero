...
bool Condition::Meets(ConditionSourceInfo& sourceInfo)
{
    ...
    switch (ConditionType)
    {
        ...
        case CONDITION_QUEST_STATUS:
        {
            ...
            {
                if (!sObjectMgr->GetQuestTemplate(ConditionValue1))
                    break;

                QuestStatus const& l_QuestStatus = l_Player->GetQuestStatus(ConditionValue1);

                /// There can only be 1 switch statement
                switch (l_QuestStatus)
                {
                    case QUEST_STATUS_NONE:                   ///< Player does not have or had quest at all. He could accept it, but he did not (yet)
                        condMeets = ConditionValue2 & 0x1;
                        break;
                    case QUEST_STATUS_COMPLETE:               ///< Player fulfilled objectives, but did not hand it in yet
                        condMeets = ConditionValue2 & 0x2;
                        break;
                    case QUEST_STATUS_INCOMPLETE:             ///< Player did not fulfill objectives yet
                        condMeets = ConditionValue2 & 0x8;
                        break;
                    case QUEST_STATUS_FAILED:                 ///< Player failed to fulfill objectives for any reason, e.g. time limit
                        condMeets = ConditionValue2 & 0x20;
                        break;
                    case QUEST_STATUS_REWARDED:               ///< Player handed quest in and this is sort of a post-quest interaction
                        condMeets = ConditionValue2 & 0x40;
                        break;

                        /// Impossible, we can never reach default here
                    default:
                        TC_LOG_ERROR("sql.sql", "CONDITION_QUEST_STATUS Player %s has invalid quest status!", l_Player->GetName().c_str());
                        break;
                }
            }
            break;
        ...
        case CONDITION_HAS_SPELL_COOLDOWN:
        {
            if (Player* l_Player = object->ToPlayer())
            {
                condMeets = l_Player->HasSpellCooldown(ConditionValue1);
            }
            break;
        }
...
uint32 Condition::GetSearcherTypeMaskForCondition()
{
    ...
    switch (ConditionType)
    {
		...
		case CONDITION_IN_WATER:
			...
			break;
        default:
            /// Only trigger on Player (because only players can have quests)
        case CONDITION_QUEST_STATUS:
        case CONDITION_HAS_SPELL_COOLDOWN: ///< In MoP creature is also supported
            mask |= GRID_MAP_TYPE_MASK_PLAYER;
            break;
...
bool ConditionMgr::isSourceTypeValid(Condition* cond)
{
    ...
    case CONDITION_QUEST_STATUS:
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "%s has non existing questID in value1 (%u), skipped.", cond->SourceType, cond->ConditionValue1);
            return false;
        }

        ///@todo add check for allowed statusmask ? (0x1, 0x2, 0x8, 0x20, 0x40) ?
        break;
    }
    ...
    case CONDITION_HAS_SPELL_COOLDOWN:
    {
        ...
        {
            TC_LOG_ERROR("sql.sql", "%s has non existing spellid in value1 (%u), skipped.", cond->SourceType, cond->ConditionValue1);
            return false;
        }
