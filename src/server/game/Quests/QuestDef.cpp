...
void Quest::BuildExtraQuestInfo(WorldPacket& data, Player* player) const
{
    ...
    data << 10 * monster::Honor::hk_honor_at_level(_session->GetPlayer()->getLevel(), quest->GetRewHonorMultiplier());
...
{
    return sWorld->getBoolConfig(CONFIG_QUEST_IGNORE_AUTO_ACCEPT) ? false : HasFlag(QUEST_FLAGS_AUTO_ACCEPT);
}

bool Quest::IsAutoTake(Object* questGiver /* = nullptr */, uint64 guid /* = 0 */) const
{
    if (questGiver)
        return questGiver->GetTypeId() == TYPEID_PLAYER && HasFlag(QUEST_FLAGS_AUTO_TAKE);

    if (!guid)
        return false;

    return GUID_HIPART(guid) == HIGHGUID_PLAYER && HasFlag(QUEST_FLAGS_AUTO_TAKE);
}
