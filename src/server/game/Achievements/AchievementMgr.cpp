...
{
    PreparedStatement* l_Stmt = nullptr;
    ...
    if (!m_completedAchievements.empty())
    {
        for (CompletedAchievementMap::iterator iter = m_completedAchievements.begin(); iter != m_completedAchievements.end(); ++iter)
        {
            ...
            /// Deletion
            l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_BY_ACHIEVEMENT); ///< character_achievement

            l_Stmt->setUInt32(0, iter->first);               ///< achievement
            l_Stmt->setUInt32(1, GetOwner()->GetGUIDLow());  ///< guid

            trans->Append(l_Stmt);

            /// Insertion

            l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT); ///< character_achievement

            l_Stmt->setUInt32(0, GetOwner()->GetGUIDLow());  ///< guid
            l_Stmt->setUInt32(1, iter->first);               ///< achievement
            l_Stmt->setUInt64(2, uint64(iter->second.date)); ///< date
    ...
    if (!m_criteriaProgress.empty())
    {
        for (CriteriaProgressMap::iterator iter = m_criteriaProgress.begin(); iter != m_criteriaProgress.end(); ++iter)
        {
            ...
            {
                /// Deletetion
                l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_ACHIEVEMENT_PROGRESS_AND_CRITERIA); ///< character_achievement_progress

                l_Stmt->setUInt32(0, GetOwner()->GetGUIDLow()); ///< guid
                l_Stmt->setUInt32(1, iter->first);              ///< criteria
            ...
            {
                /// Insertion
                l_Stmt = CharacterDatabase.GetPreparedStatement(CHAR_INS_CHAR_ACHIEVEMENT_PROGRESS); ///< character_achievement_progress

                l_Stmt->setUInt32(0, GetOwner()->GetGUIDLow());     ///< guid
                l_Stmt->setUInt32(1, iter->first);                  ///< criteria
                l_Stmt->setUInt32(2, iter->second.counter);         ///< counter
                l_Stmt->setUInt32(3, iter->second.date);            ///< date
    ...
    for (uint8 i = 0; i < MAX_ADDITIONAL_CRITERIA_CONDITIONS; ++i)
    {
        ...
        switch (AchievementCriteriaAdditionalCondition(reqType))
        {
            ...
            case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_TARGET_CREATURE_ENTRY: // 4
                 ...
                 // Hack fix for Achievement - Bros. Before Ho Ho Ho's
            ...
            case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_SOURCE_HAS_AURA: // 8
				// Hack for Fa-la-la-la-Ogri'la, there are wrong auras in the dbc
				if (criteria->achievement == 1282)
				{
					if (referencePlayer->HasAura(62061))
						break;
				}
				else if (criteria->achievement == 233) // Achievement Bloodthirsty Berserker
				{
					if (!referencePlayer)
						return false;
			...
			case ACHIEVEMENT_CRITERIA_ADDITIONAL_CONDITION_TARGET_HAS_AURA: // 10
				if (criteria->achievement == 1258) // Achievement Take a Chill Pill
				{
					if (!unit || !unit->IsInWorld())
						return false;
