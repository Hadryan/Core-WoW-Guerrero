...
/// SAI Scripting
void Map::ScriptsStart(SmartScriptType p_Type, uint32 p_Entry, Object* p_Source, Object* p_Target, uint32 p_EffIndex)
{
    /// Check if it exists in smart_scripts
    if (!sSmartScriptMgr->ScriptExist(p_Entry, p_Type))
        return;

    SmartScript* l_NewScript = new SmartScript(p_Type, p_Entry, p_Source, p_Target);

    uint32 l_EndTime = l_NewScript->GetEndTime();

    if (l_EndTime > 0)
        m_scriptScheduleSAI.insert(ScriptScheduleMapSAI::value_type(l_EndTime, l_NewScript));
    else
    {
        l_NewScript->OnUpdate(1);
    }
}

/// Process queued scripts
void Map::ScriptsProcess(const uint32 p_Diff)
{
    if (m_scriptScheduleSAI.empty())
        return;

    ScriptScheduleMapSAI::iterator l_Itr = m_scriptScheduleSAI.begin();

    while (l_Itr != m_scriptScheduleSAI.end())
    {
        SmartScript* l_Script = l_Itr->second;

        l_Script->m_TimePassed += p_Diff;
        l_Script->OnUpdate(p_Diff);

        uint32 l_EndTime = l_Itr->first;

        if (l_Script->m_TimePassed >= l_EndTime)
        {
            delete l_Script;
            l_Script = nullptr;
            l_Itr = m_scriptScheduleSAI.erase(l_Itr);
        }
        else
            ++l_Itr;
    }
    ...
    while (!m_scriptSchedule.empty() && (iter->first <= sWorld->GetGameTime()))
    {
        ...
        switch (step.script->command)
        {
            case SCRIPT_COMMAND_TALK:
                ...
                if (step.script->Talk.Flags & SF_TALK_USE_PLAYER)
                {
                    if (Player* player = _GetScriptPlayerSourceOrTarget(source, target, step.script))
                    {
                        ...
                        std::string text(sObjectMgr->GetmonsterString(step.script->Talk.TextID, loc_idx));
            ...
            case SCRIPT_COMMAND_CALLSCRIPT_TO_UNIT:
            {
                ...
                {
                    CellCoord p(monster::ComputeCellCoord(wSource->GetPositionX(), wSource->GetPositionY()));
                    Cell cell(p);

                    monster::CreatureWithDbGUIDCheck target_check(wSource, step.script->CallScript.CreatureEntry);
                    monster::CreatureSearcher<monster::CreatureWithDbGUIDCheck> checker(wSource, cTarget, target_check);
