...
void Object::BuildCreateUpdateBlockForPlayer(UpdateData* data, Player* target) const
{
    ...
    switch (GetGUIDHigh())
    {
        ...
        {
			// if (ToUnit()->ToTempSummon() && IS_PLAYER_GUID(ToUnit()->ToTempSummon()->GetSummonerGUID())) // NOTE: Disabled to prevent a crash
                updateType = UPDATETYPE_CREATE_OBJECT2;
            break;
        }
...
{
    std::vector<uint32> l_TransportFrames;
    bool hasTransportTime2 = false;
    bool hasTransportTime3 = false;

    /// @todo: this is wrong, and not an updateflag, currently we use it to determine a stopframe
    /// We need to properly implement stopframes, for example right now Halls of org elevator is not blizzlike
    /// There is also an updateflag we need, which is UPDATEFLAG_SERVER_TIME
    /// That flag will sync transport position of server to client, but currently we don't track static transport position
    /// @todo ^: port it from mop
    if (flags & UPDATEFLAG_UNK7)
    {
        GameObject const* self = ToGameObject();
        l_TransportFrames.emplace_back(self->GetGOInfo()->transport.pause);
    }
    ...
    data->WriteBits(l_TransportFrames.size(), 24);
    data->WriteBit(0);
    ...
    // Data
    for (uint32 l_Frame : l_TransportFrames)
        *data << l_Frame;
...
void Object::SetGuidValue(uint16 index, uint32 value)
{
	ASSERT(index + 1 < m_valuesCount || PrintIndexError(index, true));
	if (*((uint32*)&(m_uint32Values[index])) != value)
	{
		*((uint32*)&(m_uint32Values[index])) = value;
		_changesMask.SetBit(index);
		_changesMask.SetBit(index + 1);
...
m_phaseMask(PHASEMASK_NORMAL), m_notifyflags(0), m_executed_notifies(0), m_tickCountOnBeginStealth(0)
{
...
bool WorldObject::_IsWithinDist(WorldObject const* obj, float dist2compare, bool is3D) const
{
    ...
    {
        // Sometimes pet and owner might be on diff transports, or one of them inside a transport while the other is not
        if (GetTransGUID() != obj->GetTransGUID())
        {
            // Whoever is inside the transport needs to have their trans offsets converted to global pos for the sake of comparing 
            // the coords with the one guy/pet that is outside.
            Transport* petTransport = GetTransport(); // pet is always the caller of this function, owner is the obj
            Transport* playerTransport = obj->GetTransport();

            if (!petTransport && playerTransport) // Pet outside transport, owner inside, convert owner offsets
            {
                float x, y, z, o;
                x = obj->GetTransOffsetX();
                y = obj->GetTransOffsetY();
                z = obj->GetTransOffsetZ();
                o = obj->GetTransOffsetO();
                playerTransport->CalculatePassengerPosition(x, y, z, o);
            ...
            if (petTransport && !playerTransport) // Pet inside transport, owner outside, convert owner offsets
            {
                float x, y, z, o;
                x = GetTransOffsetX();
                y = GetTransOffsetY();
                z = GetTransOffsetZ();
                o = GetTransOffsetO();
                petTransport->CalculatePassengerPosition(x, y, z, o);
...
bool WorldObject::IsWithinLOSInMap(const WorldObject* obj, SpellInfo const* m_spellInfo, VMAP::ModelIgnoreFlags ignoreFlags) const
{
    if (!IsInMap(obj))
        return false;
    ...
    return IsWithinLOS(ox, oy, oz, m_spellInfo, ignoreFlags);
}
...
bool WorldObject::IsWithinLOS(float ox, float oy, float oz, SpellInfo const *m_spellInfo, VMAP::ModelIgnoreFlags ignoreFlags) const
{
    if (m_spellInfo && m_spellInfo->AttributesEx2 & SPELL_ATTR2_CAN_TARGET_NOT_IN_LOS)
        return true;
...
void WorldObject::UpdateAllowedPositionZ(float x, float y, float &z) const
{
    ...
    bool petIsInTransport = ToUnit() && ToUnit()->isPet() && ToUnit()->GetTransGUID();
    bool vehicleInWater = ToUnit() && ToUnit()->IsVehicle() && ToUnit() && ToUnit()->IsInWater();
    bool pathFindingDisabledForEntry = ToUnit() && !MMAP::MMapFactory::IsPathFindingEnabledForUnit(ToUnit());
    if (!MMAP::MMapFactory::IsPathfindingEnabled(mapId) || pathFindingDisabledForEntry || petIsInTransport || vehicleInWater )
        return;
...
float WorldObject::GetGridActivationRange() const
{
    ...
    {
        float l_Distance = l_Creature->GetVisibilityDistance();

        if(l_Distance != GetMap()->GetVisibilityRange())
            return l_Distance;

        return l_Creature->m_SightDistance;
    }
...
{
    if (Creature const* l_Creature = ToCreature())
        return l_Creature->GetVisibilityDistance();
...
bool WorldObject::CanDetectStealthOf(WorldObject const* obj, bool checkAlert) const
{
	...
	// Mainaz: Used to emulate retail stealth (don't hide us until next world tick)
	if (obj->GetTickCountOnBeginStealth() == sWorld->GetRetailDelayTickCount())
	{
		return true;
	}
...
TempSummon* Map::SummonCreature(uint32 entry, Position const& pos, SummonPropertiesEntry const* properties /*= NULL*/, uint32 duration /*= 0*/, WorldObject* summoner /*= NULL*/, uint32 spellId /*= 0*/, uint32 vehId /*= 0*/, bool visibleBySummonerOnly /*= false*/)
{
    ...
    {
        if (transport)
            transport->RemovePetPassenger(summon);
        delete summon;
        return NULL;
    }
...
TempSummon* WorldObject::SummonCreature(uint32 entry, const Position &pos, TempSummonType spwtype /*= TEMPSUMMON_MANUAL_DESPAWN*/, uint32 duration /*= 0*/, uint32 vehId /*= 0*/, bool visibleBySummonerOnly /*= false*/)
{
    if (Map* map = FindMap())
    {
        if (TempSummon* summon = map->SummonCreature(entry, pos, NULL, duration, ToUnit(), 0, vehId, visibleBySummonerOnly))
        {
...
void WorldObject::MovePosition(Position &pos, float dist, float angle, bool limitZValue)
{
    ...
    if (limitZValue)
    {
        ...
        // Some crazy Z values can cause the below loop to fuck up position relocation
        if (destz == 0 || std::fabs(destz) > MAX_HEIGHT)
...
void WorldObject::MovePositionToFirstCollision(Position &pos, float dist, float angle) const
{
    ...
    // Mage flame and frostfire orbs.
    if (Creature const* mageOrb = ToCreature())
        if (mageOrb->GetEntry() == 44214 || mageOrb->GetEntry() == 45322)
            destz = pos.m_positionZ;
...
uint64 WorldObject::GetTickCountOnBeginStealth() const
{
	return m_tickCountOnBeginStealth;
}

void WorldObject::SetTickCountOnBeginStealth()
{
	m_tickCountOnBeginStealth = sWorld->GetRetailDelayTickCount();
}
