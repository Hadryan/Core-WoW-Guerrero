...
enum TempSummonType
{
    ...
    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT_NO_RESET = 9,             ///< Despawns after a specific time after the creature is out of combat (no reset)
};
...
class Object
{
    public:
		...
		uint32 GetGuidValue(uint16 index) const
		{
			ASSERT(index + 1 < m_valuesCount || PrintIndexError(index, false));
			return *((uint32*)&(m_uint32Values[index]));
		};
		...
		void SetGuidValue(uint16 index, uint32 value);
...
class GridObject
{
    public:
        ...
        void AddToGrid(GridRefManager<T>& m) { _gridRef.link(&m, (T*)this); }
...
class WorldObject : public Object, public WorldLocation
{
    ...
    public:
        ...
        bool GetClosePointJC(float &x, float &y, float &z, float size, float distance2d = 0, float angle = 0, const WorldObject* forWho = NULL, bool force = false) const;
        ...
        bool IsWithinLOS(float x, float y, float z, SpellInfo const *sp = NULL, VMAP::ModelIgnoreFlags ignoreFlags = VMAP::ModelIgnoreFlags::Nothing) const;
        bool IsWithinLOSInMap(const WorldObject* obj, SpellInfo const *sp = NULL, VMAP::ModelIgnoreFlags ignoreFlags = VMAP::ModelIgnoreFlags::Nothing) const;
        ...
        TempSummon* SummonCreature(uint32 id, const Position &pos, TempSummonType spwtype = TEMPSUMMON_MANUAL_DESPAWN, uint32 despwtime = 0, uint32 vehId = 0, bool visibleBySummonerOnly = false);
		...
		uint64 GetTickCountOnBeginStealth() const;
		void SetTickCountOnBeginStealth();
		protected:
			uint64 m_tickCountOnBeginStealth;
