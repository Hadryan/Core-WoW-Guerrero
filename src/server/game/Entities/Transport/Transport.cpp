...
bool Transport::GenerateWaypoints(const GameObjectTemplate* goInfo, std::set<uint32> &mapsUsed)
{
    ...
    for (auto l_Data : path)
        allPoints.push_back(G3D::Vector3(l_Data->LocX, l_Data->LocY, l_Data->LocZ));
    ...
    for (size_t i = 0; i < path.size(); ++i)
    {
        if (!mapChange)
        {
            ...
            if (i != path.size() - 1 && (node_i->Flags & 1 || node_i->MapID != path.at(i + 1)->MapID))
            {
...
bool Transport::TeleportTransport(uint32 newMapid, float x, float y, float z, float orientation)
{
    ...
    //for (PetSet::iterator itr = _petPassengerSet.begin(); itr != _petPassengerSet.end(); ++itr)
    //    (*itr)->FarTeleportTo(newMap, x, y, z, (*itr)->GetOrientation());
...
void Transport::RemovePetPassenger(Creature* passenger)
{
    ...
    TC_LOG_INFO("entities.transport", "Creature %s removed from transport %s.", passenger->GetName().c_str(), GetName().c_str());
}
    ...
    if (_positionChangeTimer.Passed())
    {
        ...
        else
        {
            ...
            */
            ...
            // Make pets follow their owners again, if not already following
            EnablePetMovementOnStop();
        }
    }
...
void Transport::EnablePetMovementOnStop()
{
    if (!m_passengers.empty())
    {
        for (std::map<uint64, WorldObject *>::iterator itr = m_passengers.begin(); itr != m_passengers.end(); ++itr)
        {
            if (WorldObject *passenger = ObjectAccessor::GetWorldObject(*this, itr->first))
            {
                ...
                {
                    if (Pet* pet = passenger->ToUnit()->ToPet())
                    {
                        if (PetAI* ai = dynamic_cast<PetAI*>(pet->GetAI()))
                        {
                            if (!pet->GetCharmInfo()->IsFollowing() && !pet->GetCharmInfo()->IsReturning())
                            {
                                ai->ClearCharmInfoFlags();
