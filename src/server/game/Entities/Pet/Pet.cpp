...
void Pet::AddToWorld()
{
    ...
    if (Player* owner = GetOwner())
    {
        ...
        {
            if (trans->GetGUID() != GetTransGUID())
            {
                owner->SetTransport(trans);
                owner->m_movementInfo.t_guid = trans->GetGUID();
                owner->m_movementInfo.t_pos.Relocate(owner->GetTransOffsetX(), owner->GetTransOffsetY(), owner->GetTransOffsetZ(), owner->GetTransOffsetO());
...
bool Pet::LoadPet(Player* owner, PetData* t_pet, bool current)
{
    ...
    // Set pet's position after setting level, its size depends on it, unless its on a transport
    Transport *transport = owner->GetTransport();
    if (transport)
    {
        SetTransport(transport);
        m_movementInfo.t_guid = transport->GetGUID();
        m_movementInfo.t_pos.Relocate(owner->GetTransOffsetX(), owner->GetTransOffsetY(), owner->GetTransOffsetZ(), owner->GetTransOffsetO());
        transport->AddPassenger(this);
    }
    else
    {
    ...
    {
        // Warlocks and hunters need control demon/pet in order to activate pet action bar
        if (owner->getClass() == CLASS_WARLOCK || owner->getClass() == CLASS_HUNTER)
        {
            if (owner->HasSpell(80388) || owner->HasSpell(93321))
    ...
    // Warlocks need control demon in order to activate pet action bar
    if (owner->getClass() == CLASS_WARLOCK || owner->getClass() == CLASS_HUNTER)
    {
        if (owner->HasSpell(80388) || owner->HasSpell(93321))
...
void Pet::SavePet(PetSlot mode, bool logout)
{
    ...
    t_pet->reactstate          = t_pet->level < 10 ? REACT_ASSIST : GetReactState(); // Sub level 10 pets need to assist their owner by default
...
void Pet::Update(uint32 diff)
{
    ...
    if (getPetType() == SUMMON_PET && GetOwner() && GetOwner()->getClass() == CLASS_WARLOCK)
    {
        ...
        {
            if (!HasAura(53646) && !GetOwner()->HasAura(53646)) // Demonic Pact raid buff
    ...
    switch (petType)
    {
        case SUMMON_PET:
        {
            switch (GetEntry())
            {
				...
				case 26125: // Deathknight's Ghoul
				{
...
bool Pet::addSpell(uint32 spellId,PetSpellData& m_spell)
{
    ...
    {
        ToggleAutocast(spellInfo, true);
        m_charmInfo->SetSpellAutocast(spellInfo, true);
    }
...
bool Pet::learnSpell(uint32 spell_id)
{
    ...
    if (Unit* owner = GetOwner())
        if (owner->ToPlayer())
            if (owner->ToPlayer()->getClass() == CLASS_WARLOCK && !owner->HasSpell(80388) || 
                owner->ToPlayer()->getClass() == CLASS_HUNTER && !owner->HasSpell(93321))
                return true;
...
void Guardian::RecalculatePetScalingAttackSpeed(WeaponAttackType /*att*/)
{
    ...
    else if (GetEntry() == 27893) // dancing rune weapon
    {
        auraId = 51906;
        effIndex = EFFECT_1;
    }
