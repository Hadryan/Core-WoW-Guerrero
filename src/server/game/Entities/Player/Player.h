...
// 2^n values, Player::m_isunderwater is a bitmask. These are monster internal values, they are never send to any client
...
#define MAX_MONEY_AMOUNT               (UI64LIT(9999999999999999)) // TODO: Move this restriction to worldserver.conf, default to this value, hardcap at uint64.max
...
enum PlayerCheatStates
{
    CHEAT_STATE_NONE			                     = 0x00,
    CHEAT_STATE_GODMODE			                     = 0x01,
    CHEAT_STATE_NO_CAST_TIME	                     = 0x02,
    CHEAT_STATE_NO_COOLDOWN		                     = 0x04,
    CHEAT_STATE_INFINITE_POWER	                     = 0x08,
    CHEAT_STATE_WATERWALK		                     = 0x10,
    CHEAT_STATE_ALL_SPELLS		                     = 0x20,
};
...
class Player : public Unit, public GridObject<Player>
{
    ...
    public:
        ...
        void GrantArcheologyXp(bool Solve);
		...
		DirectionMovement GetDirectionMovement();
		void GetDistAngleForPet(float &dist, float &angle);
        ...
        void SetCheatStatusOn(uint32 cheat) { _activeCheats |= cheat; }
        void SetCheatStatusOff(uint32 cheat) { _activeCheats &= ~cheat; }
        ...
        void BuildPlayerChat(WorldPacket* data, uint32 msgtype, const std::string& text, uint32 language, const std::string& addonPrefix, Unit* receiver = NULL) const;
        ...
        Item* StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update, int32 randomPropertyId = 0, AllowedLooterSet const& allowedLooters = AllowedLooterSet(), bool isSoulBound = false);
        ...
        */
        ...
        /// Gives credit to whole group within reward distance
        /// @p_Entry - Credit Entry
        /// @p_Guid - guid of credit, if needed
        void KilledMonsterCreditGroupNear(uint32 p_Entry, uint64 p_Guid = 0);
        ...
        static void DeleteFromDB(uint64 playerguid, uint32 accountId, bool deleteFinally = false);
        ...
        void SetMindFlayRecastTimer(uint32 timer) { m_mindFlayRecastTime = timer; }
        uint32 GetMindFlayRecastTimer() { return m_mindFlayRecastTime; }
        void SetMindFlayPeriodic(uint32 timer) { m_mindFlayPeriodic = timer; }
        uint32 GetMindFlayPeriodicTimer() { return m_mindFlayPeriodic; }
		...
		bool HasClientControl(Unit* target);
...
//        Spell* m_spellModTakingSpell;  // Spell for which charges are dropped in spell::finish
        ...
        uint32 m_mindFlayRecastTime;
        uint32 m_mindFlayPeriodic;
...
template <class T> T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T &basevalue, Spell* spell)
{
    ...
    for (SpellModList::iterator itr = m_spellMods[op].begin(); itr != m_spellMods[op].end(); ++itr)
    {
        ...
        // Custom charge drop for specific spells that need the mod applied but to not have a charge drop.
        switch (spellId)
        {
            // Zealotry, dont drop Divine Purpose charge
            case 85696:
            {
                if (mod->spellId == 90174)
                    dropCharge = false;
                break;
            }
            default:
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            // Except for cost spellmods, need to drop charge regardless
            if (basevalue == T(0))
            {
                if (mod->op == SPELLMOD_COST && dropCharge)
