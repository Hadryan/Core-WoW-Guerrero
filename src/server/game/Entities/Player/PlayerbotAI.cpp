	...
	if (!m_bot->isMoving())
	{
        ...
        if (i_nextMoveTime.Passed())
        {
            ...
            for (uint8 i = 0; i < MAX_MOVE_TYPE; ++i)
                if (m_bot->GetSpeedRate(UnitMoveType(i)))
                m_bot->SetSpeed(UnitMoveType(i), m_bot->GetSpeedRate(UnitMoveType(i)), true);
        }
	}
}
...
void PlayerbotAI::MoveCalculate()
{
    ...
    m_bot->GetMotionMaster()->MovePoint(m_bot->GetGUIDLow(), destX, destY, destZ, true);
}
...
void PlayerbotAI::UpdateAI(const uint32 diff)
{
	...
	chance;
    if (chance <= 20 && !m_bot->isDead() && (m_bot->getClass() != CLASS_WARRIOR || m_bot->getClass() != CLASS_DRUID))
		MoveUpdate(diff);
	
	if (!m_bot->isAlive() || m_bot->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_GHOST))
	{
		WorldSafeLocsEntry const* ClosestGrave = sObjectMgr->GetClosestGraveYard(m_bot->GetPositionX(), m_bot->GetPositionY(), m_bot->GetPositionZ(), m_bot->GetMapId(), m_bot->GetTeam());
		m_bot->BuildPlayerRepop();
		if (ClosestGrave)
			m_bot->TeleportTo(ClosestGrave->map_id, ClosestGrave->x, ClosestGrave->y, ClosestGrave->z, m_bot->GetOrientation());
		m_bot->ResurrectPlayer(50.0f);
	}
}
