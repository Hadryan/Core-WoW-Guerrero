...
void PlayerMenu::SendQuestGiverQuestDetails(Quest const* quest, uint64 npcGUID, bool activateAccept, bool p_DisplayPopup) const
{
    ...
    if (!(quest->HasFlag(QUEST_FLAGS_AUTO_SUBMIT) || quest->HasSpecialFlag(QUEST_SPECIAL_FLAGS_AUTO_ACCEPT)))
    ...
    data << uint8(0);                                       // IsFinished? value is sent back to server in quest accept packet - StartCheat
    data << uint8(ExtraQuestWindow);                        //  "0 = Normal, 1 = give extra ! quest window on right side" - DisplayPopup
