...
void TempSummon::Update(uint32 diff)
{
    ...
    switch (m_type)
    {
        ...
        case TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT_NO_RESET:
        {
            if (!isInCombat())
            {
                if (m_timer <= diff)
...
void TempSummon::InitStats(uint32 duration)
{
    ...
    if (owner)
    {
        owner->m_TempSummons.push_back(GetGUID());
...
void TempSummon::UnSummon(uint32 msTime)
{
    ...
    {
        auto l_Guid = GetGUID();
        auto & l_Summons = owner->m_TempSummons;

        l_Summons.erase(std::remove(l_Summons.begin(), l_Summons.end(), l_Guid), l_Summons.end());
    }
}
