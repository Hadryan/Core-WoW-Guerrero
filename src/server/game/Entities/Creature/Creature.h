...
#ifndef monsterCORE_CREATURE_H
#define monsterCORE_CREATURE_H
...
const float VISIBILITY_DISTANCE_TINY     = 22.5f;
const float VISIBILITY_DISTANCE_SMALL    = 45.0f;
const float VISIBILITY_DISTANCE_NORMAL   = 90.0f;
const float VISIBILITY_DISTANCE_LARGE    = 180.0f;
const float VISIBILITY_DISTANCE_GIGANTIC = 360.0f;
const float VISIBILITY_DISTANCE_INFINITE = 720.0f;
const float VISIBILITY_DISTANCE_ENTIRE_MAP = 9999.0f;
...
enum CreatureFlagsExtra
{
	...
	CREATURE_FLAG_EXTRA_TRAINING_DUMMY  = 0x00120000,       // creature is a training summy (SET DYNAMICALLY, DO NOT ADD IN DB)
...
#define CREATURE_NOPATH_EVADE_TIME  5000
...
struct CreatureAddon
{
    ...
    uint8 VisibilityDistanceType;
};

typedef UNORDERED_MAP<uint32, CreatureAddon> CreatureAddonContainer;

// `creature_sparring_template` table, handles creature fake combat system.
...
#define SUMMON_IDLE_TIMER 3000

enum class VictimAction : uint8
{
    None,
    Select,
    EvadeNoTargets,
...
class Creature : public Unit, public GridObject<Creature>, public MapObject
{
    public:
        ...
        {
            if (isPet())
                return (InhabitTypeValues)(INHABIT_GROUND | INHABIT_WATER);
        ...
        bool UpdateEntry(uint32 entry, uint32 team = ALLIANCE, const CreatureData* data = NULL, bool updateLevel = true);
        ...
        float GetSAIScriptFlag() const { return m_saiscriptflag; }
        ...
        Unit* QueryVictim(VictimAction& action);
        Unit* SelectVictim();
        ...
        uint32 GetBlockPercent() override;
		...
		////// IDLE / ACTIVE PET STATES //////
		// Aps: Used to define an idle state for pets while in follow move
		void InitIdleMovement();
		
		void SetIdle(bool idle) { m_idle = idle; }
		void StartIdleTimer() { m_idleTimer = SUMMON_IDLE_TIMER; m_idle = false; }
		uint32 GetIdleTimer() { return m_idleTimer; }
		bool IsIdle() { return m_idle; }
		
		// Aps: Used to re-position the pet while it's already idle
		void SetReIdle(bool re_idle) { m_re_idle = re_idle; }
		void StartReIdleTimer() { m_re_idleTimer = SUMMON_IDLE_TIMER; m_re_idle = false; }
		uint32 GetReIdleTimer() { return m_re_idleTimer; }
		bool IsReIdle() { return m_re_idle; }
		////// END //////
		
		// Aps: Used for special handling of movement on ropes
		bool OnBladesEdgeRopes();

        void ScheduleMovementInform(uint32 p_Type, uint32 p_Id);

        /// Set ignore sparring
        /// @p_Ignore - To ignore or not
        void SetIgnoreSparring(bool p_Ignore = true) { m_SparringIsCanceled = p_Ignore; }

        bool IsAlreadyIgnoringSparring() const { return m_SparringIsCanceled; }

        void SetIgnoreUpdateMovementFlag(bool p_Ignore) { m_IgnoreUpdateMovementFlag = p_Ignore; }

        void OnEvade(uint32 reason = 0);
    ...
    private:
		...
		/* Spell focus system */
		Spell const* m_focusSpell = NULL;   // Locks the target during spell cast for proper facing
		uint32 m_focusDelay;
		bool m_shouldReacquireTarget;
		uint32 m_suppressedTarget; // Stores the creature's "real" target while casting
		float m_suppressedOrientation; // Stores the creature's "real" orientation while casting

		// Aps: Pet idle / re-idle system
		uint32 m_idleTimer;
		bool m_idle;
		uint32 m_re_idleTimer;
		bool m_re_idle;
        ...
        /// Sparring
        bool m_SparringIsCanceled;

        std::list<std::pair<uint32, uint32>> m_ScheduledMovementInforms;

        bool m_IgnoreUpdateMovementFlag;
};
