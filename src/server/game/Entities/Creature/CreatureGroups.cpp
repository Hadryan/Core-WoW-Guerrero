...
void CreatureGroup::MemberAttackStart(Creature* member, Unit* target)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        if (((itr->first != m_leader && (groupAI & FLAG_MEMBERS_ASSIST_LEADER)) || (itr->first == m_leader && (groupAI & FLAG_LEADER_ASSISTS_MEMBER))) && itr->first->IsValidAttackTarget(target))
...
void CreatureGroup::LeaderMoveTo(float x, float y, float z, bool inverseAngle)
{
    ...
    for (CreatureGroupMemberType::iterator itr = m_members.begin(); itr != m_members.end(); ++itr)
    {
        ...
        monster::NormalizeMapCoord(dx);
        monster::NormalizeMapCoord(dy);
