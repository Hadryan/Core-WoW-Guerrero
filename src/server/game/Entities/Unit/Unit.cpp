...
Unit::Unit(bool isWorldObject): WorldObject(isWorldObject)
	...
	, m_earthShieldLastHitTimer(time(0))
    , lastUnit(nullptr)
{
#ifdef _MSC_VER
#pragma warning(default:4355)
    ...
    m_isHomePosLandNeeded = false;
}

////////////////////////////////////////////////////////////
// Methods of class GlobalCooldownMgr
bool GlobalCooldownMgr::HasGlobalCooldown(Unit* p_Caster, SpellInfo const* p_SpellInfo) const
{
    GlobalCooldownList::const_iterator l_Itr = m_GlobalCooldowns.find(p_SpellInfo->StartRecoveryCategory);
    if (l_Itr != m_GlobalCooldowns.end())
    {
        uint32 l_BaseGcd = p_SpellInfo->StartRecoveryTime;

        if (p_Caster->IsPlayer())
            p_Caster->ToPlayer()->ApplySpellMod(p_SpellInfo->Id, SPELLMOD_GLOBAL_COOLDOWN, l_BaseGcd);

        if (!l_BaseGcd)
            return false;
    }

    return l_Itr != m_GlobalCooldowns.end() && l_Itr->second.duration && getMSTimeDiff(l_Itr->second.cast_time, getMSTime()) < l_Itr->second.duration;
}
...
int32 GlobalCooldownMgr::GetGlobalCooldown(Unit* p_Caster, SpellInfo const* p_SpellInfo)
{
    if (HasGlobalCooldown(p_Caster, p_SpellInfo))
    {
...
void Unit::Update(uint32 p_time)
{
    ...
    // Update custom proc cooldowns
    UpdateCustomProcCooldowns(p_time);
    ...
    if (isGuardian()) //Update Speed for Guardians
    {
...
/// GetLeeWayBonusRange
/// @p_Target : Target we are checking player is in range for
float Unit::GetLeewayBonusRange(Unit const* p_Target) const
{
    /// Default melee range
    float l_Range = MELEE_RANGE;

    /// Check if player and target is moving, if so then add lee way bonus range
    if (IsPlayer() && p_Target)
        if (isMoving() && p_Target->IsPlayer() && p_Target->isMoving())
            l_Range += 2.6666667f; ///< From client
...
bool Unit::IsWithinRange(float x, float y, float z, float dist) const
{
	...
	float sizefactor = GetMeleeReach() + GetMeleeReach();
	float maxdist = dist + sizefactor;
	
	return distsq < maxdist * maxdist;
}

bool Unit::IsWithinRange(Unit* unit, float dist) const
{
	if (!unit)
		return false;
	return IsWithinRange(unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ(), dist);
}
...
bool Unit::GetRandomContactPointAC(const Unit* obj, float &x, float &y, float &z, bool force) const
{
    float combat_reach = GetCombatReach();
    if (combat_reach < 0.1f) // sometimes bugged for players
        combat_reach = DEFAULT_COMBAT_REACH;
    ...
    if (c)
        if (c->isWorldBoss() || c->IsDungeonBoss() || (obj->ToPet() && const_cast<Unit*>(obj)->ToPet()->isControlled()))
...
{
    // Creature sparring checks -> Creature Fake Combat
    ...
    else
    {
		...
		// Mainaz: Pets will now assist their owners when their target recieves damage
...
void Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, uint64 originalCaster)
{
    ...
    spell->prepare(&targets, triggeredByAura, false);
}
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->damage)
    {
        ...
        for (AuraEffectList::const_iterator dmgShieldItr = vDamageShieldsCopy.begin(); dmgShieldItr != vDamageShieldsCopy.end(); ++dmgShieldItr)
        {
            ...
            {
                int32 baseAmount = i_spellProto->Effects[EFFECT_0].CalcValue(victim);

                if (victim->HasAura(768)) // cat form
                    damage = baseAmount + victim->GetTotalAttackPowerValue(BASE_ATTACK) * 0.168f;
                else if (victim->HasAura(5487)) // bear form
                {
                    ...
                    damage = baseAmount + (victim->GetTotalAttackPowerValue(BASE_ATTACK) - vengeanceAP) * 0.168f;
                }
            }
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst (const Unit* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    else
    {
		...
		// Mainaz* if the player is casting or is stunned - then the player cant dodge
 		if (victim->IsNonMeleeSpellCasted(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED))
 			tmp = 0;
    ...
    else
    {
        ...
        if (victim->GetTypeId() == TYPEID_PLAYER || !(victim->ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY))
        {
			...
			// Mainaz* cant parry while casting or while stunned
 			if (victim->IsNonMeleeSpellCasted(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED))
 				tmp = 0;
...
uint32 Unit::OnUnitStateUpdate(uint32 oldState, uint32 newState)
{
	// End of charge
	if (oldState & UNIT_STATE_CHARGING && !(newState & UNIT_STATE_CHARGING))
	{
		for (auto iter : this->m_queuedCCState)
		{
			this->SetControlled(iter.first, iter.second, false);
		}
		this->m_queuedCCState.clear();
	}
...
{
	this->m_state = this->OnUnitStateUpdate(m_state, m_state | f);
}
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spell, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    if (canDodge)
    {
		...
		// Mainaz* cant dodge while casting or while stunned
		if (dodgeChance < 0 || victim->IsNonMeleeSpellCasted(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED))
			dodgeChance = 0;
    ...
    if (canParry)
    {
        ...
        // Mainaz* cant parry while casting or while stunned
 		if (parryChance < 0 || victim->IsNonMeleeSpellCasted(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED))
              parryChance = 0;
    ...
    if (canBlock)
    {
		...
		// Mainaz* cant block while casting or while stunned
		if (blockChance < 0 || victim->IsNonMeleeSpellCasted(false, false, true) || victim->HasUnitState(UNIT_STATE_CONTROLLED))
			blockChance = 0;
        tmp += blockChance;
...
bool Unit::CanMoveDuringChannel() const
{
	if (Spell* spell = m_currentSpells[CURRENT_CHANNELED_SPELL])
		if (spell->getState() != SPELL_STATE_FINISHED)
			return spell->GetSpellInfo()->HasAttribute(SPELL_ATTR5_CAN_CHANNEL_WHILE_MOVING) && spell->IsChannelActive();
	return false;
}
...
void Unit::_ApplyAura(AuraApplication * aurApp, uint8 effMask)
{
    ...
    if (Creature* l_Creature = ToCreature())
        if (l_Creature->AI())
            l_Creature->AI()->OnAuraApplied(aura->GetSpellInfo()->Id);
}
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator &i, AuraRemoveMode removeMode)
{
    ...
    //if (removedAuras != m_removedAurasCount) new aura may be added
        ...
        if (Creature* l_Creature = ToCreature())
            if (l_Creature->AI())
                l_Creature->AI()->OnAuraRemoved(aura->GetSpellInfo()->Id);
}
        ...
        case SPELLFAMILY_DRUID:
        {
            ...
            if (auraSpellInfo->Id == 79577)
            {
                ...
                switch (effIndex)
                {
                    case 0:
                    {
                        bool isLunarShowerProcAllowed = procSpell->Id == 8921 && GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_DRUID, 3698, EFFECT_0);
                        // Do not proc if proc spell isnt starfire, starsurge and monfire + lunar shower
                        if (procSpell->Id != 2912 && procSpell->Id != 78674 && !isLunarShowerProcAllowed)
                            return false;
                    ...
                    case 1:
                    {
                        bool isLunarShowerProcAllowed = procSpell->Id == 93402 && GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_DRUID, 3698, EFFECT_0);

                        // Do not proc if proc spell isnt wrath and starsurge and sunfire
                        if (procSpell->Id != 5176 && procSpell->Id != 78674 && !isLunarShowerProcAllowed)
                            return false;
...
bool Unit::HandleDummyAuraProc(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown, SpellInfo const* interruptedSpell, bool procSpellIsHeal, bool onCast, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    switch (dummySpell->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (dummySpell->Id == 44445 || dummySpell->Id == 44446 || dummySpell->Id == 44448)
            {
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            case 77485: // Echo of light
            {
                ...
                case 87100:
                {
                    ...
                    int32 summonShadowFiend = 34433;
                    Player* pl = ToPlayer();
                    int32 reducedCooldown = 1000 * triggeredByAura->GetAmount();
                    if (HasSpellCooldown(summonShadowFiend))
                    {
                        uint32 newCooldownDelay = pl->GetSpellCooldownDelay(summonShadowFiend);
                        if (newCooldownDelay < uint32(reducedCooldown / 1000))
                            newCooldownDelay = 0;
                        else
                            newCooldownDelay -= uint32(reducedCooldown / 1000);
                        AddSpellCooldown(summonShadowFiend, 0, uint32(time(NULL) + newCooldownDelay));
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            switch (dummySpell->Id)
            {
                case 77606: // Dark Simulacrum
                {
                    ...
                    if (!hasManaCost || procSpell->DmgClass != SPELL_DAMAGE_CLASS_MAGIC && !isException || procSpell->HasEffect(SPELL_EFFECT_SUMMON) && !isException
    ...
    switch (dummySpell->SpellFamilyName)
    {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 86172:
                {
                    *handled = true;
                    ...
                    if (procEx & PROC_EX_MISS)
                        return false;

                    CastWithDelay(100, this, 90174, true);
                    break;
                }
            }
        ...
        case SPELLFAMILY_MAGE:
        {
            switch (dummySpell->Id)
            {
                ...
                {
                    // Living bomb explosion doesn't break dragon's breath , or through Hand of Sacrifice
                    if (procSpell && procSpell->Id == 44461 || procSpell && procSpell->Id == 6940)
                    {
                        *handled = true;
                        return false;
                    }
		...
		case SPELLFAMILY_SHAMAN:
		{
			switch (dummySpell->Id)
			{
				// Earth shield hackfix
			case 974:
			{
				auto currentTime = time(0);
				const int cooldownTime = 3;
				if (currentTime - m_earthShieldLastHitTimer <= cooldownTime)
				{
					*handled = true;
					return false;
				}

				m_earthShieldLastHitTimer = currentTime;
				break;
			}
			}
		}
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
            ...
            case SPELLFAMILY_PALADIN:
            {
                switch (auraSpellInfo->Id)
                {
                    ...
                    case 76672:
                    {
                        ...
                        // Add player casted 8% spell damage taken auras
                        AuraEffectList const& bonusSpellDamageTakenAuras = victim->GetAuraEffectsByType(SPELL_AURA_MOD_DAMAGE_PERCENT_TAKEN);
                        for (AuraEffectList::const_iterator const_itr = bonusSpellDamageTakenAuras.begin(); const_itr != bonusSpellDamageTakenAuras.end(); const_itr++)
                        {
                            AuraEffect* const aurEff = (*const_itr);

                            // Only player spells..
                            if (aurEff->GetSpellInfo()->SpellFamilyName == SPELLFAMILY_GENERIC)
                                continue;

                            // Only damage increase ones
                            if (aurEff->GetAmount() < 0)
                                continue;

                            AddPct(bp0, aurEff->GetAmount());
                            break; // Apply only 1 spell
                        }
	...
	case 105552: // Item - Death Knight T12 Blood 2P Bonus
		...
		case 108002: // Normal Version
 		case 109823: // Raid Finder Version
 		case 109826: // Heroic Version
		{
			// Only procs off of critical heals
			if (!(procEx & PROC_EX_CRITICAL_HIT))
				return false;
		}
        ...
        case 20140:
        {
            ...
            if (procSpell)
            {
                if (procSpell->IsTargetingArea(effectMask))
                    if (procSpell->Id != 85222) // Light of Dawn
                        return false;

                if (procSpell->Id == 53652) // Beacon of Light heal
                    return false;
        ...
        case 55667:
        {
			...
			SpellInfo const* triggeredInfo2 = sSpellMgr->GetSpellInfo(73975); // Necrotic Strike is not listed in DBCs?
			if (victim->IsImmunedToSpell(triggeredInfo) || victim->IsImmunedToSpell(triggeredInfo2))
                return false;
            break;
        }
        case 16176: // Ancestral Healing
        ...
        case 105820: // Paladin t13 4p - Synch aura duration with zealotry
        {
            CastWithDelay(100, this, trigger_spell_id, true);
            return true;
        }
        ...
        case 79684: // Offensive State (DND) - Enables Arcane Missiles, Proc only when arcane missiles is learned
        {
            if (!HasSpell(5143))
                return false;
    ...
    switch (trigger_spell_id)
    {
        ...
        case 91342: // Shadow Infusion
        {
            ...
            if (Pet* pet = ToPlayer()->GetPet())
            {
				...
				// No procs before arena has started in preparation mode
				Battleground* bg = ToPlayer()->GetBattleground();
				if (bg && bg->isArena() && ToPlayer()->HasAura(SPELL_ARENA_PREPARATION))
					return false;
...
{
	if (HasUnitState(UNIT_STATE_ATTACK_PLAYER) ||
		HasUnitState(UNIT_STATE_MELEE_ATTACKING) ||
...
void Unit::ApplyAuraToEntry(uint32 entry, uint32 auraentry, bool apply, float range) const
{// apply = true -> add aura to targets, apply = false, remove aura from targets
    ...
    if (apply)
        ...
        {
            (*iter)->AddAura(auraentry, (*iter));
        }
    else
        for (std::list<Creature*>::const_iterator iter = targets.begin(); iter != targets.end(); ++iter)
        {
            (*iter)->RemoveAura(auraentry);
        }
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
    ...
    for (AuraEffectList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        switch (( *i )->GetMiscValue())
        {
			...
			{
				// effect 1 m_amount
				int32 maxPercent = ( *i )->GetMiscValue() == 4992 ? 9 : 18;
				...
				for (AuraApplicationMap::const_iterator itr = victimAuras.begin(); itr != victimAuras.end(); ++itr)
				{
					...
					if (aura->HasEffectType(SPELL_AURA_PERIODIC_DAMAGE) || aura->HasEffectType(SPELL_AURA_PERIODIC_DAMAGE_PERCENT))
						modPercent += ( *i )->GetMiscValue() == 4992 ? 3 : 6;
				}
    ...
    switch (spellProto->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_NONE:
            ...
            switch (spellProto->Id)
            {
                ...
                case 73685: // Unleash Life
                case 85222: // Light of Dawn
                case 94286: // Protector of the Innocent proc (Rank 1)
                case 94288: // Protector of the Innocent proc (Rank 2)
                case 94289: // Protector of the Innocent proc (Rank 3)
                    break;
                default:
                    return false;
            }
...
uint32 Unit::SpellCriticalDamageBonus(SpellInfo const* spellProto, uint32 damage, Unit* /*victim*/)
{
    ...
    switch (spellProto->DmgClass)
    {
            ...
            switch (spellProto->SpellFamilyName)
            {
                case SPELLFAMILY_HUNTER:
                {
                    if ((spellProto->SpellFamilyFlags[0] & 0x00004000) || // serpent sting
                        (spellProto->SpellFamilyFlags == flag96(0x00000080,  0x08000000, 0))) // black arrow
                        crit_bonus += damage / 2;
                    else
                        crit_bonus += damage;
                    break;
        ...
        default:
        {
            bool isWarlockOrMage = false;
            if (Player* player = ToPlayer())
                if (player->getClass() == CLASS_WARLOCK || player->getClass() == CLASS_MAGE)
                    isWarlockOrMage = true;

            if (spellProto->Id == 87532 || spellProto->Id == 63675) // Shadowy Apparition and Improved Devouring Plague should crit with 100%
                crit_bonus += damage;
            else
                crit_bonus += isWarlockOrMage ? 0 : damage / 2;                       // for non-mage and non-warlock spells, base spellcrit is is 50%, warlocks and mages use aura 85801
            break;
        }
    }
...
uint32 Unit::SpellHealingBonusDone(Unit* victim, SpellInfo const* spellProto, uint32 healamount, DamageEffectType damagetype, uint32 stack)
{
    ...
    {
        // Tree of life bonus by 15% on all spells
        if (victim && victim->HasAura(5420))
    ...
    for (AuraEffectList::const_iterator i = mDummyAuras.begin(); i != mDummyAuras.end(); ++i)
    {
        switch ((*i)->GetSpellInfo()->Id)
        {
            ...
            case 30869:
            {
                // Only increase if Direct heal or riptide & is affected by earth shield
                if ((damagetype != DOT || spellProto->Id == 61295) && victim->HasAura(974))
                    tmpMod += (*i)->GetAmount();
                break;
            }
...
MountCapabilityEntry const* Unit::GetMountCapability(uint32 mountType) const
{
    ...
    for (uint32 i = MAX_MOUNT_CAPABILITIES; i > 0; --i)
    {
        ...
        /// Artamedes: This seems like a bad check, Mounts can swim since 3.1
        /// This fixes https://project-nightfall.org/bugtracker/issue/742/Flying-Mounts-vs-Water
        ...
        /// According to this it says, if we don't have ground flags, and we don't have flying flags, skip?
...
void Unit::SetInCombatWith(Unit* enemy)
{
    ...
    if (Creature* l_Creature = ToCreature())
    {
        if (!l_Creature->IsAlreadyIgnoringSparring())
        {
            if (sObjectMgr->DoesCreatureHaveSparringInfo(l_Creature->GetEntry()) && !l_Creature->CanSparWith(enemy->ToCreature()))
            {
                ...
                for (std::list<HostileReference*>::const_iterator itr = threatList.begin(); itr != threatList.end(); ++itr)
                {
                    if (Unit* unit = (*itr)->getTarget())
                        if (Creature* l_RefCreature = unit->ToCreature())
                            if (l_RefCreature->CanSparWith(l_Creature))
                                l_RefCreature->SetIgnoreSparring(true);
                }

                l_Creature->SetIgnoreSparring(true);
                l_Creature->SetLootRecipient(this);
            }
        }
    }
...
void Unit::CombatStart(Unit* target, bool initialAggro)
{
    if (initialAggro && !target->HasUnitState(UNIT_STATE_EVADE))
    {
        ...
        /// Set owner in combat when we enter combat :)
        if (GetTypeId() == TYPEID_UNIT && isPet())
        {
            Unit* owner = GetCharmerOrOwner();
            if (owner && owner->isAlive() && !owner->isInCombat())
            {
                owner->SetInCombatState(target);
                target->SetInCombatState(owner);
                owner->AddThreat(target, 0.0f);
            }
        }
    }
...
void Unit::ClearInCombat()
{
    ...
    if (Creature* creature = ToCreature())
    {
        if (sObjectMgr->DoesCreatureHaveSparringInfo(creature->GetEntry()))
            creature->SetIgnoreSparring(false);
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if (Creature const* l_ThisCreature = ToCreature())
        if (Creature const* l_Creature = target->ToCreature())
            if (l_ThisCreature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY && l_Creature->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_PARRY)
                return true;

    if (Creature const* l_Creature = target->ToCreature())
    {
        if (l_Creature->AI())
            if (!l_Creature->AI()->CanAttackTarget(this))
                return false;

        Creature const* l_ThisCreature = ToCreature();
        if (l_ThisCreature)
            if (auto l_AI = l_ThisCreature->AI())
                if (!l_ThisCreature->AI()->CanAttackTarget(l_Creature))
                    return false;
    }
...
void Unit::AddThreat(Unit* victim, float fThreat, SpellSchoolMask schoolMask, SpellInfo const* threatSpell)
{
    ...
    // Dont add threat if the unit is evading, unless it is charming something (in that case the charmer need to have a threatlist or it will evade)
    if (CanHaveThreatList() && (!HasUnitState(UNIT_STATE_EVADE) || (HasUnitState(UNIT_STATE_EVADE) && isPossessing() || GetVehicle())))
...
Unit* Creature::QueryVictim(VictimAction& action)
{
    Unit* target = nullptr;
    ...
    if (!iAuras.empty())
    {
        for (Unit::AuraEffectList::const_iterator itr = iAuras.begin(); itr != iAuras.end(); ++itr)
        {
            ...
            {
                action = VictimAction::EvadeOther;
                break;
            }
        }
        return nullptr;
    ...
    action = VictimAction::EvadeNoTargets;
    return nullptr;
}
...
Unit* Creature::SelectVictim()
{
    ...
    Unit* target = QueryVictim(action);
    // First checking if we have some taunt on us

    switch (action)
    {
    case VictimAction::Select:
        SetInFront(target);
        break;
    case VictimAction::EvadeOther:
        OnEvade(); ///< CreatureAI::EVADE_REASON_OTHER
        break;
    case VictimAction::EvadeNoTargets:
        OnEvade(); ///< CreatureAI::EVADE_REASON_NO_HOSTILES
        break;
    default:
        break;
    }
...
bool Unit::isTargetNotAcceptableByMMaps(uint64 guid, uint32 currTime, const Position* t/* = NULL*/) const
{
    ...
    if (itr != m_targetsNotAcceptable.end() && (itr->second._endTime >= currTime || (t && !itr->second.PosChanged(*this, *t))))
        return true;
...
DiminishingLevels Unit::GetDiminishing(DiminishingGroup group)
{
    for (Diminishing::iterator i = m_Diminishing.begin(); i != m_Diminishing.end(); ++i)
    {
        ...
        if (i->stack == 0 && getMSTimeDiff(i->hitTime, getMSTime()) > 18000)
        {
...
Player* Unit::GetPlayer(uint64 guid) // Define so it does not need an object in this case like me, useful for vehicle related work eg. 
{
...
void CharmInfo::LoadPetActionBar(const std::string& data)
{
    ...
    for (; index < ACTION_BAR_INDEX_END; ++iter, ++index)
    {
		...
		// Mainaz, aggressive react state must be replayced by assist react state (4.2+ changes)
		if (type == ACT_REACTION && action == 2)
		{
			action = 3;
		}

		// Mainaz, stay comand should be replaced by petmoveto
		if (type == ACT_COMMAND && action == 0)
		{
			action = 4;
		}
                                 ...
                                 uint32 damage, uint32 absorbed, uint32 resisted, uint32 blocked, uint32 armorMitigated, SpellInfo const* procAura, SpellInfo const* interruptedSpell, bool procSpellIsHeal, bool onCast, const uint8 effectMask /*= MAX_EFFECT_MASK*/)
{
    ...
    for (AuraApplicationMap::const_iterator itr = GetAppliedAuras().begin(); itr!= GetAppliedAuras().end(); ++itr)
    {
        ...
        // only auras that has triggered spell should proc from fully absorbed damage (for both victim and attackers)
        ...
        // Dont proc if aura proc is on cooldown due to item application
        if (HasSpellCooldown(itr->first))
            if (m_spellCooldowns[itr->first].itemid != 0)
                continue;
        ...
        if (m_reactiveTimer[reactive] <= p_time)
        {
            ...
            switch (reactive)
            {
                case REACTIVE_DEFENSE:
                    if (HasAuraState(AURA_STATE_DEFENSE) && !HasAura(48263)) // Blood Presence
                        ModifyAuraState(AURA_STATE_DEFENSE, false);
                    break;
...
void Unit::Kill(Unit* victim, bool durabilityLoss, bool isRewardAllowed)
{
    ...
    if (Player* killerPlayer = GetCharmerOrOwnerPlayerOrPlayerItself())
         killerPlayer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_GET_KILLING_BLOWS, 1, 0);
...
void Unit::SetControlled(bool apply, UnitState state, bool allowCCQueueing)
{
    if (apply)
    {
		...
		// Queue CC for after charge
		if (allowCCQueueing && this->HasUnitState(UNIT_STATE_CHARGING))
		{
			this->m_queuedCCState.push_back(std::pair<bool, UnitState>(apply, state));
			return;
		}
		else
		{
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    if (GetTypeId() == TYPEID_UNIT && this->GetEntry() != 39294)
    {
...
void Unit::SendPlaySpellVisualKit(uint32 id, uint32 unkParam, uint32 p_Duration)
{
    ObjectGuid guid = GetGUID();

    WorldPacket data(SMSG_PLAY_SPELL_VISUAL_KIT, 4 + 4+ 4 + 8);
    data << uint32(p_Duration);
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
	...
	if (vehicle->GetBase()->GetEntry() == 30236 || vehicle->GetBase()->GetEntry() == 39759 || vehicle->GetBase()->GetEntry() == 27992 || 
        vehicle->GetBase()->GetEntry() == 43952 || vehicle->GetBase()->GetEntry() == 38526)
...
void Unit::_ExitVehicle()
{
    ...
    if (!vehicle->GetBase()->DisableChangeAI)
...
uint32 Unit::GetRemainingPeriodicAmount(uint64 caster, uint32 spellId, AuraType auraType, uint8 effectIndex) const
{
    ...
    for (AuraEffectList::const_iterator i = periodicAuras.begin(); i != periodicAuras.end(); ++i)
    {
        if ((*i)->GetCasterGUID() != caster || (*i)->GetId() != spellId || (*i)->GetEffIndex() != effectIndex || !(*i)->GetTotalTicks())
            continue;
        int32 MaxDuration = ceil(( *i )->GetBase()->GetMaxDuration() / pow(10, 3)) * pow(10, 3);
        int32 TotalTicks = ceil(MaxDuration / ( *i )->GetAmplitude());
        amount += uint32((*i)->GetAmount() * std::max<int32>(TotalTicks - int32((*i)->GetTickNumber()), 0 ));
        break;
    }
...
void Unit::PetSpellFail(const SpellInfo* spellInfo, Unit* target, uint32 result)
{
    ...
    if ((MMAP::MMapFactory::IsPathfindingEnabled(GetMapId()) || result != SPELL_FAILED_LINE_OF_SIGHT) && target)
    {
...
void Unit::SetFacingToObject(WorldObject* object, bool p_UseMonsterMove)
{
    // never face when already moving
    if (!IsStopped())
        return;

    if (p_UseMonsterMove && object->ToUnit())
    {
        Movement::MoveSplineInit l_Init(this);

        ///l_Init.Path().push_back(G3D::Vector3(GetPositionX(), GetPositionY(), GetPositionZ()));
        l_Init.SetFacing(object->ToUnit());
...
bool Unit::SetSwim(bool enable)
{
    ...
    if (enable)
        Movement::PacketSender(this, SMSG_SPLINE_MOVE_START_SWIM, NULL_OPCODE, &info, true).Send();
    else
        Movement::PacketSender(this, SMSG_SPLINE_MOVE_STOP_SWIM, NULL_OPCODE, &info, true).Send();
...
Unit* Unit::SelectNearestHostileUnitInAggroRange(bool useLOS) const
{
...
Player* Unit::SelectNearestPlayer(float distance, int32 aura) const
{
    Player* target = NULL;
