...
{
    BASE_ATTACK   = 0,          ///< Main hand attack
    OFF_ATTACK    = 1,          ///< Off hand attack
    RANGED_ATTACK = 2,          ///< Bow/Wand attack
    MAX_ATTACK                  ///< Max Enum (used for for loop)
};
...
public:
    int32 GetGlobalCooldown(Unit* p_Caster, SpellInfo const* p_SpellInfo);
...
typedef std::map<uint32, uint32> CustomProcCooldowns; 
...
class Unit : public WorldObject
{
    public:
        ...
        float GetLeewayBonusRange(Unit const* p_Target) const;

        bool IsWithinCombatRange(const Unit* obj, float dist2compare) const;
        bool IsWithinMeleeRange(const Unit* obj, float dist = MELEE_RANGE) const;
		bool IsWithinRange(float x, float y, float z, float dist = MELEE_RANGE) const;
		bool IsWithinRange(Unit* unit, float dist = MELEE_RANGE) const;
        bool IsWithinBoundaryRadius(const Unit* obj) const;
        void GetRandomContactPoint(const Unit* target, float &x, float &y, float &z, float distance2dMin, float distance2dMax) const;
        bool GetRandomContactPointAC(const Unit* target, float &x, float &y, float &z, bool force = false) const;
		...
		uint32 OnUnitStateUpdate(uint32 oldState, uint32 newState);
        ...
        bool isNotInCombat()  const { return !HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IN_COMBAT); }
        ...
        void SendPlaySpellVisualKit(uint32 id, uint32 unkParam, uint32 p_Duration = 0);
        ...
        */
        ...
        void SetFacingToObject(WorldObject* object, bool p_UseMonsterMove = false);
        ...
        void ApplyAuraToEntry(uint32 entry, uint32 auraentry, bool apply = true, float range = 300) const;
		...
		// Aps: Check if our current channel spell has attribute SPELL_ATTR5_CAN_CHANNEL_WHEN_MOVING
		bool CanMoveDuringChannel() const;

        uint32 m_addDmgOnce;
        uint64 m_SummonSlot[MAX_SUMMON_SLOT];
        uint64 m_ObjectSlot[MAX_GAMEOBJECT_SLOT];
        std::vector<uint64> m_TempSummons;
        ...
        void SetIsHomePosLandNeeded(bool value) { m_isHomePosLandNeeded = value; }
        bool IsHomePosLandNeeded() { return m_isHomePosLandNeeded; }
		...
		void SetControlled(bool apply, UnitState state, bool allowCCQueueing = true);
        ...
        bool IsAIEnabled, NeedChangeAI, DisableChangeAI;
        ...
        // Move Spline helper
        void saveSelectedUnit(Unit* target) { lastUnit = target; }
        Unit* getLastSaveSelectedUnit() const { return lastUnit; }
        ...
        // This is a hack, only way to have proc cooldowns with millisecond precisou without the implementation of SpellHistory class
        void UpdateCustomProcCooldowns(uint32 diff)
        {
            if (!_customProcCooldowns.empty())
            {
                for (CustomProcCooldowns::iterator itr = _customProcCooldowns.begin(); itr != _customProcCooldowns.end();)
                {
                    if (itr->second <= diff)
                        _customProcCooldowns.erase(itr++);
                    else
                    {
                        itr->second -= diff;
                        ++itr;
        ...
        bool HasCustomProcCooldown(uint32 spellId)
        {
            CustomProcCooldowns::iterator itr = _customProcCooldowns.find(spellId);
            if (itr != _customProcCooldowns.end())
                return itr->second > 0;

            return false;
        }
        void AddCustomProcCooldown(uint32 spellId, uint32 cooldown) { _customProcCooldowns.insert(std::make_pair(spellId, cooldown)); }
    ...
    protected:
		...
		std::vector<std::pair<bool, UnitState>> m_queuedCCState;
		time_t m_earthShieldLastHitTimer; // Used for earthshield ICD on units
    ...
    private:
        ...
        CustomProcCooldowns _customProcCooldowns;
