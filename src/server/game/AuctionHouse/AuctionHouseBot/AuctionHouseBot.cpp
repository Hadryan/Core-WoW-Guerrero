/*
* Copyright (C) 2008-2010 monster <http://www.monstercore.org/>
...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    if (!AHBSeller)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Disabled");
        return;
    }
    ...
    if (maxItems == 0)
    {
        //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHSeller: Auctions disabled");
        return;
    }
    ...
    {
        //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHSeller: Auctions above minimum");
        return;
    }
    if (auctions >= maxItems)
    {
        //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHSeller: Auctions at or above maximum");
        return;
    }
    ...
    if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Adding %u Auctions", items);
    ...
    case 7:
        ...
        break;
    default:
        if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: GetAHID() - Default switch reached");
        AuctioneerGUID = 23442; //default to neutral 7
        break;
    }

    if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Current Auctineer GUID is %u", AuctioneerGUID);
    ...
    */
    ...
    if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: %u items", items);

    // only insert a few at a time, so as not to peg the processor
    for (uint32 cnt = 1; cnt <= items; cnt++)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: %u count", cnt);
        uint32 itemID = 0;
        uint32 itemColor = 99;
        ...
        while (itemID == 0 && loopbreaker <= 50)
        {
            ...
            default:
            {
                if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: itemID Switch - Default Reached");
                break;
            }
            }
            ...
            if (itemID == 0)
            {
                if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Item::CreateItem() - ItemID is 0");
                continue;
            }
            ...
            if (prototype == NULL)
            {
                if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Huh?!?! prototype == NULL");
                continue;
            }
            ...
            if (item == NULL)
            {
                if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Item::CreateItem() returned NULL");
                break;
            }
            ...
            else
            {
                ...
                if (debug_Out) TC_LOG_DEBUG("misc", "AHBuyer: Quality %u not Supported", prototype->Quality);
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    if (!AHBBuyer)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBuyer: Disabled");        return;
    }

    if (!allowBuyFromNeutral && config->GetAHFID() == 120)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBuyer: Disabled for neutral auctionhouse");
        return;
    }
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        {
            //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: I have no items to bid on.");
        ...
        if (!pItem)
        {
            if (debug_Out) TC_LOG_DEBUG("misc", "AHBuyer: Item %u doesn't exist, perhaps bought already?", auction->itemGUIDLow);
            continue;
        }
        ...
        {
            TC_LOG_DEBUG("misc", "-------------------------------------------------");
            TC_LOG_DEBUG("misc", "AHBuyer: Info for Auction #%u:", auction->Id);
            TC_LOG_DEBUG("misc", "AHBuyer: AuctionHouse: %u", auction->GetHouseId());
            TC_LOG_DEBUG("misc", "AHBuyer: Auctioneer: %u", auction->auctioneer);
            TC_LOG_DEBUG("misc", "AHBuyer: Owner: %u", auction->owner);
            TC_LOG_DEBUG("misc", "AHBuyer: Bidder: %u", auction->bidder);
            TC_LOG_DEBUG("misc", "AHBuyer: Starting Bid: %u", auction->startbid);
            TC_LOG_DEBUG("misc", "AHBuyer: Current Bid: %u", currentprice);
            TC_LOG_DEBUG("misc", "AHBuyer: Buyout: %u", auction->buyout);
            TC_LOG_DEBUG("misc", "AHBuyer: Deposit: %u", auction->deposit);
            TC_LOG_DEBUG("misc", "AHBuyer: Expire Time: %u", uint32(auction->expire_time));
            TC_LOG_DEBUG("misc", "AHBuyer: Bid Rate: %f", bidrate);
            TC_LOG_DEBUG("misc", "AHBuyer: Bid Max: %Lf", bidMax);
            TC_LOG_DEBUG("misc", "AHBuyer: Bid Value: %Lf", bidvalue);
            TC_LOG_DEBUG("misc", "AHBuyer: Bid Price: %u", bidprice);
            TC_LOG_DEBUG("misc", "AHBuyer: Item GUID: %u", auction->itemGUIDLow);
            TC_LOG_DEBUG("misc", "AHBuyer: Item Template: %u", auction->itemEntry);
            TC_LOG_DEBUG("misc", "AHBuyer: Item Info:");
            TC_LOG_DEBUG("misc", "AHBuyer: Item ID: %u", prototype->ItemId);
            TC_LOG_DEBUG("misc", "AHBuyer: Buy Price: %u", prototype->BuyPrice);
            TC_LOG_DEBUG("misc", "AHBuyer: Sell Price: %u", prototype->SellPrice);
            TC_LOG_DEBUG("misc", "AHBuyer: Bonding: %u", prototype->Bonding);
            TC_LOG_DEBUG("misc", "AHBuyer: Quality: %u", prototype->Quality);
            TC_LOG_DEBUG("misc", "AHBuyer: Item Level: %u", prototype->ItemLevel);
            // Can just comment out as it's wotlk, hunter ammo, and removed in cata? TC_LOG_DEBUG("misc", "AHBuyer: Ammo Type: %u", prototype->AmmoType);
            TC_LOG_DEBUG("misc", "-------------------------------------------------");
...
void AuctionHouseBot::Update()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
    {
        ...
        {
            //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_a));
            //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: Bidding on Alliance Auctions");
        ...
        {
            //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_h));
            //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: Bidding on Horde Auctions");
    ...
    {
        //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_n));
        //if (debug_Out) TC_LOG_DEBUG("misc","misc", "AHBuyer: Bidding on Neutral Auctions");
...
void AuctionHouseBot::Initialize()
{
    ...
    if ((AHBplayerAccount != 0) || (AHBplayerGUID != 0))
    {
        ...
        if (!result)
        {
            TC_LOG_DEBUG("misc", "server.loading", "AuctionHouseBot: The account/GUID-information set for your AHBot is incorrect (account: %u guid: %u)", AHBplayerAccount, AHBplayerGUID);
            return;
        }
    }
    ...
    if (AHBSeller)
    {
        ...
        {
            if (debug_Out) TC_LOG_DEBUG("misc", "AuctionHouseBot: \"%s\" failed", npcQuery);
        }
        ...
        {
            if (debug_Out) TC_LOG_DEBUG("misc", "AuctionHouseBot: \"%s\" failed", lootQuery);
        }
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (PTR/Beta/Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Permanent Enchant Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Conjured Consumable)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Gem)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Money)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (MoneyLoot)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Lootable Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Quest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Has a Duration)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (BOP or BQI and Required Level is less than Item Level)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Warrior Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Paladin Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Hunter Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Rogue Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Priest Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (DK Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Shaman Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Mage Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Warlock Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Unused Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Druid Item)", itr->second.ItemId);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Item Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Trade Good %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (Trade Good Level = %u)", itr->second.ItemId, itr->second.ItemLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            {
                if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Trade Good %u disabled (RequiredLevel = %u)", itr->second.ItemId, itr->second.RequiredLevel);
                continue;
            }
            ...
            // {
            //    if (debug_Out_Filters) TC_LOG_DEBUG("misc","misc", "AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", itr->second.ItemId, itr->second.RequiredSkillRank);
            //    continue;
            // }
        ...
        {
            TC_LOG_DEBUG("misc", "server.loading", "AuctionHouseBot: No items");
            AHBSeller = 0;
        }

        TC_LOG_DEBUG("misc", "server.loading", "AuctionHouseBot:");
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u grey trade goods", uint32(greyTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u white trade goods", uint32(whiteTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u green trade goods", uint32(greenTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u blue trade goods", uint32(blueTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u purple trade goods", uint32(purpleTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u orange trade goods", uint32(orangeTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u yellow trade goods", uint32(yellowTradeGoodsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u grey items", uint32(greyItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u white items", uint32(whiteItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u green items", uint32(greenItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u blue items", uint32(blueItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u purple items", uint32(purpleItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u orange items", uint32(orangeItemsBin.size()));
        TC_LOG_DEBUG("misc", "server.loading", "loaded %u yellow items", uint32(yellowItemsBin.size()));
    }
    TC_LOG_DEBUG("misc", "server.loading", "AuctionHouseBot and AuctionHouseBuyer have been loaded.");
}
...
void AuctionHouseBot::InitializeConfiguration()
{
    ...
    allowBuyFromNeutral = sConfigMgr->GetBoolDefault("AuctionHouseBot.EnableBuyerNeutral", 0);
}
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    if (!pItem)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBot: Item %u doesn't exist, perhaps bought already?", ah->itemGUIDLow);
        return;
    }
    ...
    if (!u_entry)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBot: %u returned as House Faction. Neutral", ah->GetHouseFaction());
        config = &NeutralConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_ALLIANCE)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBot: %u returned as House Faction. Alliance", ah->GetHouseFaction());
        config = &AllianceConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_HORDE)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBot: %u returned as House Faction. Horde", ah->GetHouseFaction());
        config = &HordeConfig;
    }
    else
    {
...
{
    if (debug_Out)
        TC_LOG_DEBUG("misc", "Start Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
    if (AHBSeller)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG("misc", "minItems                = %u", config->GetMinItems());
            TC_LOG_DEBUG("misc", "maxItems                = %u", config->GetMaxItems());
            TC_LOG_DEBUG("misc", "percentGreyTradeGoods   = %u", config->GetPercentages(AHB_GREY_TG));
            TC_LOG_DEBUG("misc", "percentWhiteTradeGoods  = %u", config->GetPercentages(AHB_WHITE_TG));
            TC_LOG_DEBUG("misc", "percentGreenTradeGoods  = %u", config->GetPercentages(AHB_GREEN_TG));
            TC_LOG_DEBUG("misc", "percentBlueTradeGoods   = %u", config->GetPercentages(AHB_BLUE_TG));
            TC_LOG_DEBUG("misc", "percentPurpleTradeGoods = %u", config->GetPercentages(AHB_PURPLE_TG));
            TC_LOG_DEBUG("misc", "percentOrangeTradeGoods = %u", config->GetPercentages(AHB_ORANGE_TG));
            TC_LOG_DEBUG("misc", "percentYellowTradeGoods = %u", config->GetPercentages(AHB_YELLOW_TG));
            TC_LOG_DEBUG("misc", "percentGreyItems        = %u", config->GetPercentages(AHB_GREY_I));
            TC_LOG_DEBUG("misc", "percentWhiteItems       = %u", config->GetPercentages(AHB_WHITE_I));
            TC_LOG_DEBUG("misc", "percentGreenItems       = %u", config->GetPercentages(AHB_GREEN_I));
            TC_LOG_DEBUG("misc", "percentBlueItems        = %u", config->GetPercentages(AHB_BLUE_I));
            TC_LOG_DEBUG("misc", "percentPurpleItems      = %u", config->GetPercentages(AHB_PURPLE_I));
            TC_LOG_DEBUG("misc", "percentOrangeItems      = %u", config->GetPercentages(AHB_ORANGE_I));
            TC_LOG_DEBUG("misc", "percentYellowItems      = %u", config->GetPercentages(AHB_YELLOW_I));
            TC_LOG_DEBUG("misc", "minPriceGrey            = %u", config->GetMinPrice(AHB_GREY));
            TC_LOG_DEBUG("misc", "maxPriceGrey            = %u", config->GetMaxPrice(AHB_GREY));
            TC_LOG_DEBUG("misc", "minPriceWhite           = %u", config->GetMinPrice(AHB_WHITE));
            TC_LOG_DEBUG("misc", "maxPriceWhite           = %u", config->GetMaxPrice(AHB_WHITE));
            TC_LOG_DEBUG("misc", "minPriceGreen           = %u", config->GetMinPrice(AHB_GREEN));
            TC_LOG_DEBUG("misc", "maxPriceGreen           = %u", config->GetMaxPrice(AHB_GREEN));
            TC_LOG_DEBUG("misc", "minPriceBlue            = %u", config->GetMinPrice(AHB_BLUE));
            TC_LOG_DEBUG("misc", "maxPriceBlue            = %u", config->GetMaxPrice(AHB_BLUE));
            TC_LOG_DEBUG("misc", "minPricePurple          = %u", config->GetMinPrice(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "maxPricePurple          = %u", config->GetMaxPrice(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "minPriceOrange          = %u", config->GetMinPrice(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "maxPriceOrange          = %u", config->GetMaxPrice(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "minPriceYellow          = %u", config->GetMinPrice(AHB_YELLOW));
            TC_LOG_DEBUG("misc", "maxPriceYellow          = %u", config->GetMaxPrice(AHB_YELLOW));
            TC_LOG_DEBUG("misc", "minBidPriceGrey         = %u", config->GetMinBidPrice(AHB_GREY));
            TC_LOG_DEBUG("misc", "maxBidPriceGrey         = %u", config->GetMaxBidPrice(AHB_GREY));
            TC_LOG_DEBUG("misc", "minBidPriceWhite        = %u", config->GetMinBidPrice(AHB_WHITE));
            TC_LOG_DEBUG("misc", "maxBidPriceWhite        = %u", config->GetMaxBidPrice(AHB_WHITE));
            TC_LOG_DEBUG("misc", "minBidPriceGreen        = %u", config->GetMinBidPrice(AHB_GREEN));
            TC_LOG_DEBUG("misc", "maxBidPriceGreen        = %u", config->GetMaxBidPrice(AHB_GREEN));
            TC_LOG_DEBUG("misc", "minBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_DEBUG("misc", "maxBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_DEBUG("misc", "minBidPricePurple       = %u", config->GetMinBidPrice(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "maxBidPricePurple       = %u", config->GetMaxBidPrice(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "minBidPriceOrange       = %u", config->GetMinBidPrice(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "maxBidPriceOrange       = %u", config->GetMaxBidPrice(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "minBidPriceYellow       = %u", config->GetMinBidPrice(AHB_YELLOW));
            TC_LOG_DEBUG("misc", "maxBidPriceYellow       = %u", config->GetMaxBidPrice(AHB_YELLOW));
            TC_LOG_DEBUG("misc", "maxStackGrey            = %u", config->GetMaxStack(AHB_GREY));
            TC_LOG_DEBUG("misc", "maxStackWhite           = %u", config->GetMaxStack(AHB_WHITE));
            TC_LOG_DEBUG("misc", "maxStackGreen           = %u", config->GetMaxStack(AHB_GREEN));
            TC_LOG_DEBUG("misc", "maxStackBlue            = %u", config->GetMaxStack(AHB_BLUE));
            TC_LOG_DEBUG("misc", "maxStackPurple          = %u", config->GetMaxStack(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "maxStackOrange          = %u", config->GetMaxStack(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "maxStackYellow          = %u", config->GetMaxStack(AHB_YELLOW));
        }
        //AuctionHouseEntry const* ahEntry =  sAuctionMgr->GetAuctionHouseEntry(config->GetAHFID());
        AuctionHouseObject* auctionHouse = sAuctionMgr->GetAuctionsMap(config->GetAHFID());
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG("misc", "Current Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
            TC_LOG_DEBUG("misc", "Grey Trade Goods\t%u\tGrey Items\t%u", config->GetItemCounts(AHB_GREY_TG), config->GetItemCounts(AHB_GREY_I));
            TC_LOG_DEBUG("misc", "White Trade Goods\t%u\tWhite Items\t%u", config->GetItemCounts(AHB_WHITE_TG), config->GetItemCounts(AHB_WHITE_I));
            TC_LOG_DEBUG("misc", "Green Trade Goods\t%u\tGreen Items\t%u", config->GetItemCounts(AHB_GREEN_TG), config->GetItemCounts(AHB_GREEN_I));
            TC_LOG_DEBUG("misc", "Blue Trade Goods\t%u\tBlue Items\t%u", config->GetItemCounts(AHB_BLUE_TG), config->GetItemCounts(AHB_BLUE_I));
            TC_LOG_DEBUG("misc", "Purple Trade Goods\t%u\tPurple Items\t%u", config->GetItemCounts(AHB_PURPLE_TG), config->GetItemCounts(AHB_PURPLE_I));
            TC_LOG_DEBUG("misc", "Orange Trade Goods\t%u\tOrange Items\t%u", config->GetItemCounts(AHB_ORANGE_TG), config->GetItemCounts(AHB_ORANGE_I));
            TC_LOG_DEBUG("misc", "Yellow Trade Goods\t%u\tYellow Items\t%u", config->GetItemCounts(AHB_YELLOW_TG), config->GetItemCounts(AHB_YELLOW_I));
        }
    }
    if (AHBBuyer)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_DEBUG("misc", "buyerPriceGrey          = %u", config->GetBuyerPrice(AHB_GREY));
            TC_LOG_DEBUG("misc", "buyerPriceWhite         = %u", config->GetBuyerPrice(AHB_WHITE));
            TC_LOG_DEBUG("misc", "buyerPriceGreen         = %u", config->GetBuyerPrice(AHB_GREEN));
            TC_LOG_DEBUG("misc", "buyerPriceBlue          = %u", config->GetBuyerPrice(AHB_BLUE));
            TC_LOG_DEBUG("misc", "buyerPricePurple        = %u", config->GetBuyerPrice(AHB_PURPLE));
            TC_LOG_DEBUG("misc", "buyerPriceOrange        = %u", config->GetBuyerPrice(AHB_ORANGE));
            TC_LOG_DEBUG("misc", "buyerPriceYellow        = %u", config->GetBuyerPrice(AHB_YELLOW));
            TC_LOG_DEBUG("misc", "buyerBiddingInterval    = %u", config->GetBiddingInterval());
            TC_LOG_DEBUG("misc", "buyerBidsPerInterval    = %u", config->GetBidsPerInterval());
        }
    }
    if (debug_Out) TC_LOG_DEBUG("misc", "End Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
}