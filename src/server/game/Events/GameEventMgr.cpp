    ...
    TC_LOG_INFO("server.loading", "Loading Game Event GO Quest Data...");
    {
        ...
        else
        {
            ...
            do
            {
                ...
                // WorldDatabase.PExecute("DELETE FROM game_event_gameobject_quest WHERE `quest`= '%u'", questId);
...
void GameEventMgr::RunSmartAIScripts(uint16 event_id, bool activate)
{
    ...
    {
        monster_READ_GUARD(HashMapHolder<Creature>::LockType, *HashMapHolder<Creature>::GetLock());
    ...
    {
        monster_READ_GUARD(HashMapHolder<GameObject>::LockType, *HashMapHolder<GameObject>::GetLock());
