    ...
    if (creator->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for (EquipmentSlots const* itr = &itemSlots[0]; *itr != EQUIPMENT_SLOT_END; ++itr)
        {
            if (*itr == EQUIPMENT_SLOT_HEAD && player->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_HIDE_HELM))
                ...
                data << uint32(0);
           // Mainaz: Mirror images will now also support transmogrification 
			else if (Item const* item = player->GetItemByPos(INVENTORY_SLOT_BAG_0, *itr))
				if (uint32 transmog = item->GetEnchantmentId(TRANSMOGRIFY_ENCHANTMENT_SLOT))
				{
					// We gotta use the display id and not the entry id of the item
					const ItemTemplate* temp = sObjectMgr->GetItemTemplate(transmog);
					data << uint32(temp->DisplayInfoID);
				}
				else 
...
void WorldSession::HandleCancelQueuedSpellOpcode(WorldPacket& /*p_RecvPacket*/)
{
