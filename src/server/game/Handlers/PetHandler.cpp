    ...
    switch (flag)
    {
        case ACT_COMMAND:                                   //0x07
            switch (spellid)
            {
                ...
                {
                    if (PetAI* petAi = dynamic_cast<PetAI*>(pet->GetAI()))
                        petAi->SetIsEngageBySpell(false);
        ...
        case ACT_ENABLED:                                   // 0xC1    spell
        {
            ...
            if (pet->GetCharmInfo())
            {
                // Dont chase after a passive spellcast...
                PetAI* petAI = dynamic_cast<PetAI*>(pet->GetAI());
                if (petAI)
                {
                    // Unless the target is out of range....
                    SpellRangeEntry const* entry = spell->GetSpellInfo()->RangeEntry;
                    bool chase = false;
                    if (entry && unit_target)
                    {
                        float range = pet->GetSpellMaxRangeForTarget(unit_target, spell->GetSpellInfo());
                        chase = range > pet->GetDistance2d(unit_target);
                    }
                    if (pet->ToCreature()->HasReactState(REACT_PASSIVE) && !chase)
                        petAI->SetIsEngageBySpell(true);
                    else
                        petAI->SetIsEngageBySpell(false);
                }
