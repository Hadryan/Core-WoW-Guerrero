...
void WorldSession::SendTrainerList(uint64 guid, uint32 index)
{
    std::string str = GetmonsterString(LANG_NPC_TAINER_HELLO);
    SendTrainerList(guid, str, index);
}

void WorldSession::SendTrainerList(uint64 guid, const std::string& strTitle, uint32 index)
{
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvData)
{
    ...
    // For Achievement: Giddy Up!
    AchievementEntry const* giddy_up = sAchievementStore.LookupEntry(891);
   if (spellId == 33388 && !(_player->HasAchieved(891)))
       _player->CompletedAchievement(giddy_up);
...
void WorldSession::HandleGossipHelloOpcode(WorldPacket& recvData)
{
    ...
    if (unit->isArmorer() || unit->isCivilian() || unit->isQuestGiver() || unit->isServiceProvider() || unit->isGuard() || unit->isGossip())
    {
        ...
        bool l_ShouldStop = true;

        if (unit->AI())
            l_ShouldStop = unit->AI()->IsIgnoringStopOnInteraction();
