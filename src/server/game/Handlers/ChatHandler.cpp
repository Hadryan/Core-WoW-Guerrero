...
class DelayedPlayerWhisper : public BasicEvent
{
public:
	DelayedPlayerWhisper(Player* player, uint64 sender) : _player(player), _sender(sender) { }

	bool Execute(uint64, uint32) override
	{
		_player->GetPlayerbotAI()->HandleOnWhisperReact(_sender);
		return true;
	}
...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recvData)
{
    ...
    switch (type)
    {
        ...
        case CHAT_MSG_WHISPER:
        {
			...
			if (receiver->m_playerbotAI != NULL)
				receiver->m_Events.AddEvent(new DelayedPlayerWhisper(receiver, sender->GetGUID()), receiver->m_Events.CalculateTime(urand(2500, 8000)));
			//receiver->m_playerbotAI->HandleOnWhisperReact(sender->GetGUID());
