...
void WorldSession::HandleQuestgiverHelloOpcode(WorldPacket& recvData)
{
    ...
    if (creature->AI())
        l_ShouldStop = creature->AI()->IsIgnoringStopOnInteraction();
...
void WorldSession::HandleQuestgiverQueryQuestOpcode(WorldPacket& recvData)
{
    ...
    if (Quest const* quest = sObjectMgr->GetQuestTemplate(questId))
    {
        ...
        if (quest->IsAutoAccept() || quest->IsAutoTake(object, guid) && _player->CanAddQuest(quest, true))
        {
    ...
    while (quest)
    {
        if (_player->CanRewardQuest(quest, reward, true))
        {
            ...
            else
            {
                ...
                {
                    if (_player->CanAddQuest(nextQuest, true) && _player->CanTakeQuest(nextQuest, true) && (nextQuest->HasFlag(QUEST_FLAGS_AUTO_SUBMIT) || nextQuest->IsAutoTake(object, guid) || nextQuest->IsAutoAccept()))
