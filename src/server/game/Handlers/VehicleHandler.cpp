...
void WorldSession::HandleDismissControlledVehicle(WorldPacket &recvData)
{
    ...
    {
        if (Creature* l_Creature = vehicle->GetBase()->ToCreature())
        {
            l_Creature->DisableChangeAI = false;
        }
    }
...
void WorldSession::HandleRequestVehicleExit(WorldPacket& /*recvData*/)
{
    ...
    if (Vehicle* vehicle = GetPlayer()->GetVehicle())
    {
        if (Creature* l_Creature = vehicle->GetBase()->ToCreature())
        {
            ...
            if (l_Creature->AI())
            {
                l_Creature->AI()->OnLeaveVehicle(_player);
                l_Creature->AI()->OnVehicleExitRequested(_player);
            }
        }
