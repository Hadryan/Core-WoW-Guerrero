...
void WorldSession::LoginPlayer(uint64 guid)
{
    LoginQueryHolder* loginQueryHolder = new LoginQueryHolder(GetAccountId(), guid);

    if (!loginQueryHolder->Initialize())
    {
        delete loginQueryHolder;                                      // delete all unprocessed queries
        m_playerLoading = false;
        return;
    }

    _charLoginCallback = CharacterDatabase.DelayQueryHolder(((SQLQueryHolder*)loginQueryHolder));
}
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
	...
	{
		// On Warrior, start Battle Stance. On Death Knight create, start Frost Presence.
    ...
    if (pCurrChar->GetUInt32Value(UNIT_NPC_EMOTESTATE) != 0)
        pCurrChar->SetUInt32Value(UNIT_NPC_EMOTESTATE, EMOTE_ONESHOT_NONE);
...
void WorldSession::HandlePlayerBotLogin(SQLQueryHolder *holder)
{
	...
	WorldSession *botSession = new WorldSession(lqh->GetAccountId(), NULL, masterSession->GetSecurity(), masterSession->Expansion(), 0, LOCALE_enUS, 0, false);
	if (!botSession)
	{
		delete holder;
        delete botSession;
    ...
    botSession->_accountId = sWorld->getIntConfig(CONFIG_BOTS_ACC_GUID);
