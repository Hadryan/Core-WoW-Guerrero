...
void WorldSession::HandleMoveWorldportAckOpcode()
{
    ...
    if (!GetPlayer()->IsBeingTeleportedFar() && (GetPlayer()->m_playerbotAI == NULL))
        return;
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recvPacket)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_MMAPS_SPLINE_LOCKING) && _player && _player->IsSplineEnabled() && !_player->movespline->Finalized())
      {
          recvPacket.rfinish();
          return;
      }
