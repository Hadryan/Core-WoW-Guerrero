...
{
    // NOT USED BY ANY SPELL OR USELESS OR IMPLEMENTED IN DIFFERENT WAY IN monster
}
        ...
        switch (m_spellInfo->SpellFamilyName)
        {
            case SPELLFAMILY_GENERIC:
            {
                ...
                switch (m_spellInfo->Id)                     // better way to check unknown
                {
                    ...
                    case 99353:
                    {
                        damage = CalculatePct(unitTarget->GetMaxHealth(), m_spellInfo->Effects[EFFECT_0].CalcValue());

                        // Damage can't be more than 250k
                        int32 max = m_spellInfo->Effects[EFFECT_2].CalcValue();
...
void Spell::EffectDummy(SpellEffIndex effIndex)
{
	...
	case SPELLFAMILY_GENERIC:
        ...
        case 99633:
            ...
            m_caster->AddThreat(unitTarget, 1000000);
            m_caster->CastSpell(unitTarget, m_spellInfo->Effects[EFFECT_0].CalcValue(), false);
            break;
            // Leaping Flames				
        case 101165:
            m_caster->CastSpell(unitTarget, m_spellInfo->Effects[EFFECT_0].CalcValue(), false);
            if (unitTarget->ToPlayer() && unitTarget->GetMap()->IsHeroic())
                unitTarget->SetPower(POWER_ALTERNATE_POWER, -1);
            break;
			// spell Hot Rod - Radio
		...
		break;
    case SPELLFAMILY_WARLOCK: // Have to be spellfamily_warlock with the Have Group, Will Travel spell. Bad DBC data. So it's INTENDED that it's put here.
        switch (m_spellInfo->Id)
        {
    ...
    case SPELLFAMILY_PALADIN:
            ...
            case 19740:          // Blessing of Might
            {
                if (m_caster->GetTypeId() == TYPEID_PLAYER)
                {
                    ...
                    if (Continue == true)
                    {
                        for (std::list<Unit*>::iterator itr = PartyMembers.begin(); itr != PartyMembers.end(); ++itr)          // If caster is in party with a player
                        {
                            if ((*itr)->HasAura(79062, m_originalCasterGUID))
                                (*itr)->RemoveAurasDueToSpell(79062);
                            if ((*itr)->HasAura(79063, m_originalCasterGUID))
                                (*itr)->RemoveAurasDueToSpell(79063);
                            m_caster->CastSpell((*itr), 79102, true);          // Blessing of Might (Raid)
                        }
                    }
                    else
                    {
                        if (unitTarget->HasAura(79062, m_originalCasterGUID))
                            unitTarget->RemoveAurasDueToSpell(79062);
                        if (unitTarget->HasAura(79063, m_originalCasterGUID))
                            unitTarget->RemoveAurasDueToSpell(79063);
        ...
        case 20217:          // Blessing of King
        {
            if (m_caster->GetTypeId() == TYPEID_PLAYER)
            {
                ...
                if (Continue == true)
                {
                    for (std::list<Unit*>::iterator itr = PartyMembers.begin(); itr != PartyMembers.end(); ++itr)          // If caster is in party with a player
                    {
                        if ((*itr)->HasAura(79101, m_originalCasterGUID))
                            (*itr)->RemoveAurasDueToSpell(79101);
                        if ((*itr)->HasAura(79102, m_originalCasterGUID))
                            (*itr)->RemoveAurasDueToSpell(79102);
                        m_caster->CastSpell((*itr), 79063, true);          // Blessing of Kings (Raid)
                    }
                }
                else
                {
                    if (unitTarget->HasAura(79101, m_originalCasterGUID))
                        unitTarget->RemoveAurasDueToSpell(79101);
                    if (unitTarget->HasAura(79102, m_originalCasterGUID))
                        unitTarget->RemoveAurasDueToSpell(79102);
        ...
        case 31789:                                 // Righteous Defense (step 1)
        {
            ...
            {
                Unit* attacker = monster::Containers::SelectRandomContainerElement(attackers);
    ...
    m_caster->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_SPELL, m_spellInfo->Id, m_caster, unitTarget, effIndex);
...
void Spell::EffectJumpDest(SpellEffIndex effIndex)
{
    ...
    if (effectHandleMode == SPELL_EFFECT_HANDLE_LAUNCH)
    {
        ...
        CalculateJumpSpeeds(effIndex, m_caster->GetExactDist2d(x, y), speedXY, speedZ);
		CalculateJumpSpeeds(effIndex, m_caster->GetExactDist2d(x, y), speedXY, speedZ);
		// Mainaz: Just so we can handle special cases
		switch (m_spellInfo->Id)
		{
			// Mainaz: Feral cat form charge
		case 49376:
			m_caster->GetMotionMaster()->MoveJumpOrientation(x, y, z, speedXY, speedZ, 0, ObjectAccessor::GetUnit(*m_caster, m_caster->GetUInt64Value(UNIT_FIELD_TARGET)));
			break;
		default:
...
void Spell::EffectTeleportUnits(SpellEffIndex /*effIndex*/)
{
    ...
    switch (m_spellInfo->Id)
    {
        ...
        case 96683:
        {
            ...
            break;
            case 99566: // Meteor burn (Firelands)
            m_caster->NearTeleportTo(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ() - 3.00f, m_caster->GetOrientation(), false);
            return;
        }
    }
...
void Spell::EffectSendEvent(SpellEffIndex effIndex)
{
    ...
    m_caster->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_EVENT, m_spellInfo->Effects[effIndex].MiscValue, m_caster, target);
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
    ...
    if (m_spellInfo->Id == 45051)
    {
        ...
        if (!avalibleElixirs.empty())
        {
            ...
            m_caster->CastSpell(unitTarget, monster::Containers::SelectRandomContainerElement(avalibleElixirs), true, m_CastItem);
        }
    }
}
    ...
    if (gameObjTarget)
    {
        ...
        switch (gameObjTarget->GetGoType())
        {
            ...
            case GAMEOBJECT_TYPE_BUTTON:
                ...
                player->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT, gameObjTarget->GetDBTableGUIDLow(), gameObjTarget, player);
                return;
            ...
            case GAMEOBJECT_TYPE_CHEST:
                ...
                if (gameObjTarget->GetGOInfo()->chest.eventId)
                {
                    ...
                    player->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_GAMEOBJECT, gameObjTarget->GetGOInfo()->chest.eventId, player, gameObjTarget);
                }
...

                {
                    ...
                    for (uint32 l : Entries)
                    {
                        if (gameObjTarget->GetEntry() == l)
                            player->GrantArcheologyXp(false);
                    }
                }
...
void Spell::EffectTameCreature(SpellEffIndex /*effIndex*/)
{
    ...
    if (m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        ...
        m_caster->ToPlayer()->RemovePet(pet, newslot);

        uint32 spellId = 0;
        
        switch (newslot)
        {
            case 0: spellId = 883; break;
            case 1: spellId = 83242; break;
            case 2: spellId = 83243; break;
            case 3: spellId = 83244; break;
            case 4: spellId = 83245; break;
        }

        m_caster->CastSpell(m_caster, spellId, true);

        if (m_caster->ToPlayer()->GetPet())
            m_caster->ToPlayer()->GetPet()->SetFullHealth();
    }
}
...
void Spell::EffectStuck(SpellEffIndex /*effIndex*/)
{
    ...
    if (!hearthStone || player->HasSpellCooldown(8690 /*Spell Hearthstone*/))
    {
...
void Spell::EffectForceDeselect(SpellEffIndex /*effIndex*/)
{
    ...
    monster::AnyUnfriendlyUnitInObjectRangeCheck u_check(m_caster, m_caster, m_caster->GetMap()->GetVisibilityRange());
    monster::UnitListSearcher<monster::AnyUnfriendlyUnitInObjectRangeCheck> searcher(m_caster, targets, u_check);
...
void Spell::EffectCharge(SpellEffIndex /*effIndex*/)
{
    ...
    ///< Quadral: Save our selected unit to use in Pointmovementgenerator
    m_caster->saveSelectedUnit(unitTarget);
    ...
    if (effectHandleMode == SPELL_EFFECT_HANDLE_HIT_TARGET)
    {
        ...
        if (!m_spellInfo->IsPositive() && m_caster->GetTypeId() == TYPEID_PLAYER)
		{
			...
			Unit* selectedUnit = ObjectAccessor::GetUnit(*m_caster, playerCaster->GetSelection());
            m_caster->Attack(selectedUnit, true);
...
void Spell::EffectLeapBack(SpellEffIndex effIndex)
{
    ...
    // old unitTarget->JumpTo(speedxy, speedz, m_spellInfo->SpellIconID != 539 || m_spellInfo->SpellIconID != 4777, true);
    // https://en.wikipedia.org/wiki/Operators_in_C_and_C%2B%2B
    unitTarget->JumpTo(speedxy, speedz, m_spellInfo->SpellIconID != 539 && m_spellInfo->SpellIconID != 4777, true);

    //1891: Disengage, avoids going forward instead of backward.
    //m_caster->JumpTo(speedxy, speedz, m_spellInfo->SpellIconID != 539);
    // Breaks tons of spells. Todo if Disengage is still going forward: make a proper fix that doesn't affect other shit.
}
...
void Spell::SummonGuardian(uint32 i, uint32 entry, SummonPropertiesEntry const* properties, uint32 numGuardians)
{
    ...
    for (uint32 count = 0; count < numGuardians; ++count)
    {
		...
		// Mainaz: Paladin's guardian should have different weapons depending on the caster's spec
		if (summon->GetEntry() == 46499)
		{
			uint32 spec = m_caster->ToPlayer()->GetPrimaryTalentTree(m_caster->ToPlayer()->GetActiveSpec());
			switch (spec)
			{
			case TALENT_TREE_PALADIN_RETRIBUTION:
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, 62902); // Main hand
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, -1); // Set the offhand to -1
				break;
			case TALENT_TREE_PALADIN_PROTECTION:
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, -1);
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, 62904);
				break;
			case TALENT_TREE_PALADIN_HOLY:
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID, 62903);
				summon->SetUInt32Value(UNIT_VIRTUAL_ITEM_SLOT_ID + 1, -1);
				break;
			default:
				break;
			}
