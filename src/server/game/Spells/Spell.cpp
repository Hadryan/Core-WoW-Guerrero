...
void Spell::SelectImplicitConeTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    {
        monster::WorldObjectSpellConeTargetCheck check(DegToRad(coneAngle), radius, m_caster, m_spellInfo, selectionType, condList);
        monster::WorldObjectListSearcher<monster::WorldObjectSpellConeTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
        SearchTargets<monster::WorldObjectListSearcher<monster::WorldObjectSpellConeTargetCheck> >(searcher, containerTypeMask, m_caster, m_caster, radius);
...
void Spell::SelectImplicitAreaTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType, uint32 effMask)
{
    ...
    if (!unitTargets.empty())
    {
        ...
        if (maxSize && power != -1)
        {
            if (Powers(power) == POWER_HEALTH)
            {
                ...
                {
                    unitTargets.sort(monster::HealthPctOrderPred());
            ...
            else
            {
                ...
                if (unitTargets.size() > maxSize)
                {
                    unitTargets.sort(monster::PowerPctOrderPred((Powers)power));
                    unitTargets.resize(maxSize);
                }
            }
        }
        ...
        // Other special target selection goes here
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            monster::Containers::RandomResizeList(unitTargets, maxTargets);
    ...
    {
        if (uint32 maxTargets = m_spellValue->MaxAffectedTargets)
            monster::Containers::RandomResizeList(gObjTargets, maxTargets);
...
void Spell::SelectImplicitTrajTargets()
{
    ...
    std::list<WorldObject*> targets;
    monster::WorldObjectSpellTrajTargetCheck check(dist2d, m_targets.GetSrcPos(), m_caster, m_spellInfo);
    monster::WorldObjectListSearcher<monster::WorldObjectSpellTrajTargetCheck> searcher(m_caster, targets, check, GRID_MAP_TYPE_MASK_ALL);
    SearchTargets<monster::WorldObjectListSearcher<monster::WorldObjectSpellTrajTargetCheck> > (searcher, GRID_MAP_TYPE_MASK_ALL, m_caster, m_targets.GetSrcPos(), dist2d);
    if (targets.empty())
        return;

    targets.sort(monster::ObjectDistanceOrderPred(m_caster));
...
#define CHECK_DIST {\
    ...
    if (m_targets.GetSrcPos()->GetExactDist2d(m_targets.GetDstPos()) > bestDist)
    {
        ...
        CallScriptDestinationTargetSelectHandlers(dest, EFFECT_0);
...
void Spell::SearchTargets(SEARCHER& searcher, uint32 containerMask, Unit* referer, Position const* pos, float radius)
{
    ...
    if (searchInGrid || searchInWorld)
    {
        ...
        CellCoord p(monster::ComputeCellCoord(x, y));
        Cell cell(p);
        cell.SetNoCreate();
...
WorldObject* Spell::SearchNearbyTarget(float range, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return NULL;
    monster::WorldObjectSpellNearbyTargetCheck check(range, m_caster, m_spellInfo, selectionType, condList);
    monster::WorldObjectLastSearcher<monster::WorldObjectSpellNearbyTargetCheck> searcher(m_caster, target, check, containerTypeMask);
    SearchTargets<monster::WorldObjectLastSearcher<monster::WorldObjectSpellNearbyTargetCheck> > (searcher, containerTypeMask, m_caster, m_caster, range);
    return target;
}
...
void Spell::SearchAreaTargets(std::list<WorldObject*>& targets, float range, Position const* position, Unit* referer, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectionType, ConditionList* condList)
{
    ...
    if (!containerTypeMask)
        return;
    monster::WorldObjectSpellAreaTargetCheck check(range, position, m_caster, referer, m_spellInfo, selectionType, condList);
    monster::WorldObjectListSearcher<monster::WorldObjectSpellAreaTargetCheck> searcher(m_caster, targets, check, containerTypeMask);
    SearchTargets<monster::WorldObjectListSearcher<monster::WorldObjectSpellAreaTargetCheck> > (searcher, containerTypeMask, m_caster, position, range);
}
...
void Spell::FilterTargets()
{
    for (std::list<TargetInfo>::iterator iterList = m_UniqueTargetInfo.begin(); iterList != m_UniqueTargetInfo.end();)
    {
		...
		// Quest item spell 'Burn Corpse'
		if (m_spellInfo->Id == 85555)
		{
			if (!target->ToCreature() || target->ToCreature()->GetEntry() != 45851)
				return;
		}
...
void Spell::DoAllEffectOnTarget(TargetInfo* target)
{
    ...
    if (missInfo != SPELL_MISS_EVADE && !m_caster->IsFriendlyTo(unit) && (!m_spellInfo->IsPositive() || m_spellInfo->HasEffect(SPELL_EFFECT_DISPEL)))
    {
        ...
        /// Hackfix for freeze, should be initial aggro of combat too
...
SpellResults::SpellPrepareResults Spell::prepare(SpellCastTargets const* targets, AuraEffect const* triggeredByAura, uint32 p_GcdAtCast)
{
    if (m_CastItem)
    ...
    // by default creature should`ve facing to victim
    if (!m_spellInfo->IsChanneled() && !triggeredByAura && m_caster->ToCreature() && m_caster->getVictim() &&
        (!targets->GetObjectTargetGUID() || targets->GetObjectTargetGUID() == m_caster->getVictim()->GetGUID() || targets->GetObjectTargetGUID() == m_caster->ToCreature()->GetGUID()))
        m_caster->SetFacingTo(m_caster->GetAngle(m_caster->getVictim()));
    ...
    if (Player::QueueSystemEnabled() && p_GcdAtCast && p_GcdAtCast <= MAX_SPELL_QUEUE_GCD)
    {
        ...
        return SpellResults::SpellPrepareResults::Queued;
    }
    else
    ...
    if (!(_triggeredCastFlags & TRIGGERED_IGNORE_CAST_IN_PROGRESS) && m_caster->IsNonMeleeSpellCasted(false, true, true, m_spellInfo->IsAutoshot()) && m_cast_count)
    {
        ...
        finish(false);
        return SpellResults::SpellPrepareResults::Failed;
    }
    ...
    return SpellResults::SpellPrepareResults::Ok;
}
        ...
        case SPELL_STATE_CASTING:
        {
            for (std::list<TargetInfo>::const_iterator ihit = m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
            {
                if ((*ihit).missCondition == SPELL_MISS_NONE)
                {
                    ...
                    {
                        // Mind flay specific (maybe not?) case, if aura cancelled with 1 tick remaining, carry out the remaining tick to the beginning of the new spell
                        if (m_spellInfo->Id == 15407)
                        {
                            if (AuraEffect const* aurEff = unit->GetAuraEffect(SPELL_AURA_PERIODIC_DAMAGE, SPELLFAMILY_PRIEST, 0, 0, 1088, m_caster->GetGUID()))
                            {
                                if (Player* player = m_caster->ToPlayer())
                                {
                                    player->SetMindFlayRecastTimer(200);
                                    player->SetMindFlayPeriodic(aurEff->GetPeriodicTimer());
                                }
                            }
                        }
...
void Spell::cast(bool skipCheck)
{
        ...
        if (GetSpellInfo()->DmgClass != SPELL_DAMAGE_CLASS_NONE)
        {
            ...
            if (m_targets.GetTargetMask() & TARGET_FLAG_UNIT)
                for (Unit::ControlList::iterator itr = playerCaster->m_Controlled.begin(); itr != playerCaster->m_Controlled.end(); ++itr)
                    if ((*itr)->IsAIEnabled && (*itr)->isGuardian() && (*itr)->ToCreature()->HasReactState(REACT_ASSIST) && GetSpellInfo()->HasEffect(SPELL_EFFECT_SCHOOL_DAMAGE))
                        if (WorldObject* objTarget = m_targets.GetObjectTarget())
                            if (Unit* unitTarget = objTarget->ToUnit())
                                if ((*itr)->IsValidAttackTarget(unitTarget))
                                    (*itr)->ToCreature()->AI()->AttackStart(unitTarget, true);
        }
    }
	...
	case 15090: // Dispel Magic
	case 33076: // Prayer of Mending
	case 40440: // Divine Blessing
		if (m_caster->GetTypeId() == TYPEID_PLAYER) // Heart of Unliving
		{
			if (m_caster->HasAura(109812)) // Raid Finder Version
				m_caster->AddAura(109811, m_caster);
			if (m_caster->HasAura(107963)) // Normal Version
				m_caster->AddAura(107962, m_caster);
			if (m_caster->HasAura(109814)) // Heroic Version
				m_caster->AddAura(109813, m_caster);
			break;
		}
...
void Spell::update(uint32 difftime)
{
    ...
    case SPELL_STATE_QUEUED:
    {
        if (m_caster->ToPlayer()->GetQueuedSpell() == this)
        {
            ...
            l_Spell->m_glyphIndex = m_glyphIndex;
            cancel();
    ...
    if (powerType == POWER_RUNES)
    {
        if (m_caster->GetTypeId() == TYPEID_PLAYER)
            if (uint64 targetGUID = m_targets.GetUnitTargetGUID())
                for (std::list<TargetInfo>::iterator ihit= m_UniqueTargetInfo.begin(); ihit != m_UniqueTargetInfo.end(); ++ihit)
                    if (ihit->targetGUID == targetGUID)
						// Mainaz: If the target is invulnerable return
                        if (ihit->missCondition == SPELL_MISS_DODGE || ihit->missCondition == SPELL_MISS_PARRY || m_targets.GetUnitTarget() && m_targets.GetUnitTarget()->IsImmunedToDamage(m_spellInfo))
                            return;
...
SpellCastResult Spell::CheckCast(bool strict)
{
            ...
            case SPELL_EFFECT_DUMMY:
            {
                ...
                case 83967:
                {
                    ...
                    if (pMap->IsDungeon())
                    {
                        ...
                        if (!m_caster->FindNearestCreature(53488, 10.0f))
                            return SPELL_FAILED_SPELL_UNAVAILABLE;
                    }
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            case SPELL_AURA_MOUNTED:
            {
                ...
                // @hack for poseidon
                if (m_caster->IsInWater() && !allowInWaterMount && m_spellInfo->Effects[i].MiscValue != 53270 && !(m_spellInfo->Id == 98718 || m_spellInfo->Id == 75207 || m_spellInfo->Id != 64731)) // hackfix for poseidon + vashj'ir + sea turtle
    ...
    // Mainaz: The OR check is for skullbash, which is handled from spelldruid
    if (m_spellInfo->AttributesEx7 & SPELL_ATTR7_HAS_CHARGE_EFFECT && m_caster->HasUnitState(UNIT_STATE_ROOT) && m_spellInfo->SpellIconID != 3201)
        return SPELL_FAILED_ROOTED;
    ...
    if (m_spellInfo->RequiresSpellFocus)
    {
        CellCoord p(monster::ComputeCellCoord(m_caster->GetPositionX(), m_caster->GetPositionY()));
        Cell cell(p);

        GameObject* ok = NULL;
        monster::GameObjectFocusCheck go_check(m_caster, m_spellInfo->RequiresSpellFocus);
        monster::GameObjectSearcher<monster::GameObjectFocusCheck> checker(m_caster, ok, go_check);

        TypeContainerVisitor<monster::GameObjectSearcher<monster::GameObjectFocusCheck>, GridTypeMapContainer > object_checker(checker);
    ...
    // Mainaz: Don't allow players to eat/drink or both while hexed
    ...
    for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
    {
        ...
        else if (m_spellInfo->Effects[j].IsAreaAuraEffect())
        {
            ...
            if (target->isPet() || target->isGuardian())
            {
                ...
                {
                    if (Player* plrOwner = target->GetOwner()->ToPlayer())
                        if (Group* group = plrOwner->GetGroup())
                        {
                            // Iterate through group - If we find a member that has not the aura we allow to auto cast it
                        ...
                        {
                            // If we don't have a group just check owner
                            if (plrOwner->HasAuraEffect(m_spellInfo->Id, j))
                                return false;
                        }
                }
                default:
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (m_spellInfo->RangeEntry)
    {
        ...
        if (m_spellInfo->RangeEntry->ID == 1 || m_spellInfo->Id == 89091 || m_spellInfo->Id == 89087)
            return SPELL_CAST_OK;
    ...
    {
        /// Bonus range from Leeway mechanic
        max_range += m_caster->GetLeewayBonusRange(target);
...
SpellCastResult Spell::CheckPower()
{
    ...
    // Dont check holy power cost if player has cost mod and current holy power is 0
    // Holy power calculations are done in handle_finishing_phase just like combo points and its usage is controlled by the client
    // Adding this check here to prevent any possible WPE exploits in the future, though unlikely to happen.
    if (m_spellInfo->PowerType == POWER_HOLY_POWER)
        if (m_caster)
            if (m_spellInfo->PowerType == POWER_HOLY_POWER && m_caster->HasAuraTypeWithMiscvalue(SPELL_AURA_ADD_PCT_MODIFIER, SPELLMOD_COST) 
                || m_caster->HasAuraTypeWithMiscvalue(SPELL_AURA_ADD_PCT_MODIFIER, SPELLMOD_COST) && m_caster->GetPower(POWER_HOLY_POWER) == 0)
                skipPowerCheck = true;
        ...
        switch (_targetSelectionType)
        {
            ...
            case TARGET_CHECK_PARTY:
                ...
                if (!_referer->IsInPartyWith(unitTarget))
                    return false;
                if (unitTarget->GetGUID() == _caster->GetGUID() && _spellInfo->Id == 15290) // Vampiric embrace, prevent effect 0 and effect 1 hitting the caster altogether
                    return false;
                break;
            case TARGET_CHECK_RAID_CLASS:
...
bool WorldObjectSpellAreaTargetCheck::operator()(WorldObject* target)
{
    ...
    if (!target->IsWithinDist3d(_position, new_dist) && !(target->ToGameObject() && target->ToGameObject()->IsInRange(_position->GetPositionX(), _position->GetPositionY(), _position->GetPositionZ(), new_dist)))
        return false;
