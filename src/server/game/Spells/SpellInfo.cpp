...
SpellImplicitTargetInfo::StaticData  SpellImplicitTargetInfo::_data[TOTAL_SPELL_TARGETS] =
{
    ...
    {TARGET_OBJECT_TYPE_UNIT, TARGET_REFERENCE_TYPE_CASTER, TARGET_SELECT_CATEGORY_DEFAULT, TARGET_CHECK_DEFAULT,  TARGET_DIR_NONE },        // 120 TARGET_UNK_120
...
{
    return !(Attributes & SPELL_ATTR0_CANT_USED_IN_COMBAT) || Id == 90813 || Id == 90820 || Id == 90822 || Id == 90823; // Some Twilight Highlands spells from Wonderman, needs to be use-able in combat for quest to work.
}
...
int32 SpellInfo::CalcPowerCost(Unit const* caster, SpellSchoolMask schoolMask, Spell* spell, bool baseCost) const
{
    ...
    if (ManaCostPercentage)
    {
        switch (PowerType)
        {
            ...
            case POWER_RUNIC_POWER:
                ...
                break;
            // This check is utterly retarded because client already handles the calcpowercost.
            // Leave this here just so that stuff like divine purpose dont get messed over with a "not enough holy power" error.
            case POWER_HOLY_POWER: 
                return caster->GetPower(POWER_HOLY_POWER);
            default:
    ...
    if (PowerType != POWER_HOLY_POWER) // Done already in HandleHolyPower
        if (Player* modOwner = caster->GetSpellModOwner()) // This is bad, we are dropping charges in a CALCULATION?
