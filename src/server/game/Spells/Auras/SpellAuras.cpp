        ...
        else
        {
            ...
            switch (GetSpellInfo()->Effects[effIndex].Effect)
            {
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_RAID:
                {
                    ...
                    if (!isolated)
                    {
                        monster::AnyGroupedUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius, GetSpellInfo()->Effects[effIndex].Effect == SPELL_EFFECT_APPLY_AREA_AURA_RAID);
                        monster::UnitListSearcher<monster::AnyGroupedUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                ...
                case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
                {
                    ...
                    if (!isolated)
                    {
                        monster::AnyFriendlyUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius);
                        monster::UnitListSearcher<monster::AnyFriendlyUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                        GetUnitOwner()->VisitNearbyObject(radius, searcher);
                    }
                    break;
                }
                ...
                {
                    monster::AnyAoETargetUnitInObjectRangeCheck u_check(GetUnitOwner(), GetUnitOwner(), radius); // No GetCharmer in searcher
                    monster::UnitListSearcher<monster::AnyAoETargetUnitInObjectRangeCheck> searcher(GetUnitOwner(), targetList, u_check);
                    GetUnitOwner()->VisitNearbyObject(radius, searcher);
                    break;
                }
...
void DynObjAura::FillTargetMap(std::map<Unit*, uint8> & targets, Unit* /*caster*/)
{
    ...
    for (uint8 effIndex = 0; effIndex < MAX_SPELL_EFFECTS; ++effIndex)
    {
        ...
        {
            monster::AnyFriendlyUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius,  GetSpellInfo()->Effects[effIndex].ImplicitTargetConditions);
            monster::UnitListSearcher<monster::AnyFriendlyUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
        else
        {
            monster::AnyAoETargetUnitInObjectRangeCheck u_check(GetDynobjOwner(), dynObjOwnerCaster, radius, GetSpellInfo()->Effects[effIndex].ImplicitTargetConditions);
            monster::UnitListSearcher<monster::AnyAoETargetUnitInObjectRangeCheck> searcher(GetDynobjOwner(), targetList, u_check);
            GetDynobjOwner()->VisitNearbyObject(radius, searcher);
        }
