...
// such checks will speedup monster change amount/send for client operations
        ...
        if (caster)
        {
            ...
            if (m_spellInfo->IsChanneled())
            {
                if (m_spellInfo->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION)
                {
                    ...
                    if (m_spellInfo->Id == 64843 || m_spellInfo->Id == 64901 || m_spellInfo->Id == 1120)
                    {
                        int32 totalTicks = m_spellInfo->GetDuration() / m_amplitude;
                        m_amplitude = int32(m_amplitude * caster->GetFloatValue(UNIT_MOD_CAST_SPEED) - (sWorld->GetUpdateTime() / totalTicks));
                    }
                }
            }
...
void AuraEffect::Update(uint32 diff, Unit* caster)
{
    if (m_isPeriodic && (GetBase()->GetDuration() >= 0 || GetBase()->IsPassive() || GetBase()->IsPermanent()))
    {
        if (m_extraTick)
        {
            ++m_tickNumber;
            m_extraTick = false;
            // Make it tick faster if its a channeled spell, channel bar might end before world update tick happens
            // Need to double check for SPELL_ATTR5_START_PERIODIC_AT_APPLY here, mindflay tick carryover also counts as extraTick.
            if (m_spellInfo->IsChanneled() && m_spellInfo->AttributesEx5 & SPELL_ATTR5_START_PERIODIC_AT_APPLY)
                if (GetTotalTicks() > 0) // Don't care about infinite ticking spells.
                    m_periodicTimer -= diff / GetTotalTicks();
...
void AuraEffect::HandleShapeshiftBoosts(Unit* target, bool apply) const
{
    ...
    if (apply)
    {
        ...
        if (target->GetTypeId() == TYPEID_PLAYER)
        {
            ...
            {
            case FORM_CAT:
                //Mainaz: Heart of the wild attack power boost
                ...
                break;
            case FORM_BEAR:
                //Mainaz: Heart of the Wild stamina boost
    ...
    {
        //Mainaz: Heart of the Wild boosts remove
        if (AuraEffect* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_ATTACK_POWER_PCT, SPELLFAMILY_DRUID, 240, EFFECT_1))
            aurEff->ChangeAmount(0);
        if (AuraEffect* aurEff = target->GetAuraEffect(SPELL_AURA_MOD_PERCENT_STAT, SPELLFAMILY_DRUID, 240, EFFECT_2))
            aurEff->ChangeAmount(0);
...
void AuraEffect::HandleModStealth(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
		// Set delay used to emulate retail stealth
		target->SetTickCountOnBeginStealth();
		sWorld->m_unitsRequiringVisibilityUpdates.push_back(target);
...
void AuraEffect::HandleFeignDeath(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
    ...
    if (apply)
    {
        ...
        */
        ...
        UnitList targets;
        monster::AnyUnfriendlyUnitInObjectRangeCheck u_check(target, target, target->GetMap()->GetVisibilityRange());
        monster::UnitListSearcher<monster::AnyUnfriendlyUnitInObjectRangeCheck> searcher(target, targets, u_check);
...
void AuraEffect::HandleModPowerCostPCT(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
	...
	// Mainaz: Special exception
	if (m_spellInfo->Id == 82365 && !target->GetPower(POWER_MANA))
		return;
            ...
            case 22842:
                {
					...
					// Mainaz: Frenzied regeneration should be able to crit, if it crits just double the normal amount
					float crit_chance = 0.0f;
					crit_chance += target->GetUnitCriticalChance(BASE_ATTACK, target);
					if (roll_chance_f(crit_chance))
					{
						regen *= 2.0f;
						caster->HealBySpell(target, m_spellInfo, regen, true);
					}
					else
            ...
            case 51690:
                {
                    ...
                    UnitList targets;
                    {
                        ...
                        CellCoord p(monster::ComputeCellCoord(target->GetPositionX(), target->GetPositionY()));
                        Cell cell(p);

                        monster::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck u_check(target, radius);
                        monster::UnitListSearcher<monster::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck> checker(target, targets, u_check);

                        TypeContainerVisitor<monster::UnitListSearcher<monster::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, GridTypeMapContainer > grid_object_checker(checker);
                        TypeContainerVisitor<monster::UnitListSearcher<monster::AnyUnfriendlyAttackableVisibleUnitInObjectRangeCheck>, WorldTypeMapContainer > world_object_checker(checker);
                    ...
                    Unit* spellTarget = monster::Containers::SelectRandomContainerElement(targets);
...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* target, Unit* caster) const
{
    ...
    if (caster->isAlive())
    {
        // DealDamage returns 0 for Training Dummies so we set new_damage to real damage done
        if (target->ToCreature() && target->ToCreature()->GetScriptName() == "npc_training_dummy")
            new_damage = damage;
