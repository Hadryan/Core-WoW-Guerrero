...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    if (mincountOrRef > 0)                                  // item (quest or non-quest) entry, maybe grouped
    {
        ...
        if (itemid < 0)
        {
            CurrencyTypesEntry const* currency = sCurrencyTypesStore.LookupEntry(abs(itemid));
            if (!currency)
            {
                TC_LOG_ERROR("sql.sql", "Table '%s' entry %d item %d (currency): currency entry not listed in `CurrencyTypes.dbc` - skipped", store.GetName(), entry, itemid);
                return false;
            }
        }
        else
...
void Loot::AddCurrency(int32 currencyId, LootStoreItem const& item)
{
    CurrencyTypesEntry const* entry = sCurrencyTypesStore.LookupEntry(abs(currencyId));
    
    if (!entry)
        return;

    uint32 count = urand(item.mincountOrRef, item.maxcount);
    currencies.push_back(std::make_pair(abs(currencyId), count));
}
...
void Loot::FillNotNormalLootFor(Player* player, bool presentAtLooting)
{
    ...
    // Process currency
    if (!currencies.empty())
        for (std::list<std::pair<int,int>>::iterator iter = currencies.begin(); iter != currencies.end(); iter++)
            player->ModifyCurrency(iter->first, iter->second);
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot& loot, uint16 lootMode) const
{
    ...
    if (!possibleLoot.empty())                              // If nothing selected yet - an item is taken from equal-chanced part
        return monster::Containers::SelectRandomContainerElement(possibleLoot);
...
void LootTemplate::Process(Loot& loot, bool rate, uint16 lootMode, uint8 groupId) const
{
    ...
    for (LootStoreItemList::const_iterator i = Entries.begin(); i != Entries.end(); ++i)
    {
        ...
        if (item->itemid < 0)                                   // Currency case, doesnt need to be added to the loot item list
        {
            loot.AddCurrency(item->itemid, *item);
            continue;
        }
