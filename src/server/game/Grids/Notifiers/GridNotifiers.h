...
#ifndef monster_GRIDNOTIFIERS_H
#define monster_GRIDNOTIFIERS_H
...
namespace monster
{
    ...
    */
    ...
    {
        public:
            explicit NearestHostileUnitCheck(Unit const* p_Unit, float dist = 0.f, bool playerOnly = false) : me(p_Unit), i_playerOnly(playerOnly)
            {
    ...
    {
        public:
            explicit NearestHostileUnitInAttackDistanceCheck(Unit const* p_Unit, float dist = 0) : me(p_Unit)
            {
                m_range = (dist == 0 ? 9999 : dist);
            ...
            bool operator()(Unit* u)
            {
                ...
                {
                    if (Creature const* l_Creature = me->ToCreature())
                    {
                        if (!l_Creature->canStartAttack(u, false))
                            return false;
                    }
                    else
                        if (!me->IsValidAttackTarget(u))
    ...
    {
        public:
            explicit NearestHostileUnitInAggroRangeCheck(Unit const* p_Unit, bool useLOS = false) : _me(p_Unit), _useLOS(useLOS)
            {
            }
            bool operator()(Unit* u)
            {
                ...
                if (!u->IsWithinDistInMap(_me, _me->ToCreature() ? _me->ToCreature()->GetAggroRange(u) : 150.0f)) ///< TODO CHECK VALIDITY ON 50.0f
                    return false;
