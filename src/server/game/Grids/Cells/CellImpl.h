...
#ifndef monster_CELLIMPL_H
#define monster_CELLIMPL_H
...
inline CellArea Cell::CalculateCellArea(float x, float y, float radius)
{
    if (radius <= 0.0f)
    {
        CellCoord center = monster::ComputeCellCoord(x, y).normalize();
        return CellArea(center, center);
    }

    CellCoord centerX = monster::ComputeCellCoord(x - radius, y - radius).normalize();
    CellCoord centerY = monster::ComputeCellCoord(x + radius, y + radius).normalize();
...
{
    CellCoord p(monster::ComputeCellCoord(center_obj->GetPositionX(), center_obj->GetPositionY()));
    Cell cell(p);
    if (dont_load)
        cell.SetNoCreate();
