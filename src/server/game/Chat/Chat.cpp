...
{
    const char *format = GetmonsterString(entry);
    char str[1024];
    va_list ap;
    va_start(ap, entry);
...
const char *ChatHandler::GetmonsterString(int32 entry) const
{
    return m_session->GetmonsterString(entry);
}
...
{
    SendSysMessage(GetmonsterString(entry));
}
        ...
        if ((table[i].Handler)(this, table[i].Name[0] != '\0' ? text : oldtext))
        {
            if (!AccountMgr::IsPlayerAccount(table[i].SecurityLevel))
            {
                ...
                if (m_session)
                {
                    ...
                    if (sWorld->getBoolConfig(CONFIG_DISCORD_LOG))
...
GameObject* ChatHandler::GetNearbyGameObject()
{
    ...
    GameObject* obj = NULL;
    monster::NearestGameObjectCheck check(*pl);
    monster::GameObjectLastSearcher<monster::NearestGameObjectCheck> searcher(pl, obj, check);
...
GameObject* ChatHandler::GetObjectGlobalyWithGuidOrNearWithDbGuid(uint32 lowguid, uint32 entry)
{
    ...
    if (!obj && sObjectMgr->GetGOData(lowguid))                   // guid is DB guid of object
    {
        ...
        CellCoord p(monster::ComputeCellCoord(pl->GetPositionX(), pl->GetPositionY()));
        Cell cell(p);

        monster::GameObjectWithDbGUIDCheck go_check(*pl, lowguid);
        monster::GameObjectSearcher<monster::GameObjectWithDbGUIDCheck> checker(pl, obj, go_check);

        TypeContainerVisitor<monster::GameObjectSearcher<monster::GameObjectWithDbGUIDCheck>, GridTypeMapContainer > object_checker(checker);
...
const char *CliHandler::GetmonsterString(int32 entry) const
{
...
{
    return GetmonsterString(LANG_CONSOLE_COMMAND);
}
