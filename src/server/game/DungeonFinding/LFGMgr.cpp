            ...
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL && joinData.result == LFG_JOIN_OK; itr = itr->next())
            {
                if (Player* plrg = itr->getSource())
                {
					...
					if (!plrg->HaveBot())
						continue;
					//add npcbots
					for (uint8 i = 0; i != plrg->GetMaxNpcBots(); ++i)
					{
						NpcBotMap const* map = plrg->GetBotMap(i);
						if (!grp->IsMember(plrg->GetBotMap(i)->_Guid()))
							continue;

						if (Creature* bot = ObjectAccessor::GetCreatureOrPetOrVehicle(*plrg, plrg->GetBotMap(i)->_Guid()))
						{
							++memberCount;
							players.insert(plrg->GetBotMap(i)->_Guid());
						}
					}
				}
			}
    ...
    if (grp)                                               // Begin rolecheck
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* plrg = itr->getSource())
            {
				...
				for (uint8 i = 0; i != plrg->GetMaxNpcBots(); ++i)
				{
					if (players.find(plrg->GetBotMap(i)->_Guid()) == players.end() || !grp->IsMember(plrg->GetBotMap(i)->_Guid()))
                        continue;

					if (Creature* bot = ObjectAccessor::GetCreatureOrPetOrVehicle(*plrg, plrg->GetBotMap(i)->_Guid()))
                    {
                            uint64 bguid = bot->GetGUID();
                            ...
                            if (!debugNames.empty())
                                debugNames.append(", ");
                            debugNames.append(bot->GetName().c_str());

                            uint8 broles = 0;
							if (bot->GetBotClass() == CLASS_PALADIN || bot->GetBotClass() == CLASS_WARRIOR || bot->GetBotClass() == CLASS_DEATH_KNIGHT) //BOT_ROLE_TANK
							{
								broles |= PLAYER_ROLE_TANK;
								ss << bot->GetName().c_str() << " has chosen: Tank" << "\n";
							}
							if (bot->GetBotClass() == CLASS_PRIEST || bot->GetBotClass() == CLASS_DRUID || bot->GetBotClass() == CLASS_SHAMAN) //BOT_ROLE_HEAL
							{
								broles |= PLAYER_ROLE_HEALER;
								ss << bot->GetName().c_str() << " has chosen: Healer" << "\n";
							}
							if (bot->GetBotClass() == CLASS_ROGUE || bot->GetBotClass() == CLASS_WARLOCK || bot->GetBotClass() == CLASS_ROGUE || bot->GetBotClass() == CLASS_MAGE || bot->GetBotClass() == CLASS_HUNTER) //BOT_ROLE_DPS
							{
								broles |= PLAYER_ROLE_DAMAGE;
								ss << bot->GetName().c_str() << " has chosen: Damage" << "\n";
							}
...
void LFGMgr::UpdateRoleCheck(uint64 gguid, uint64 guid /* = 0 */, uint8 roles /* = PLAYER_ROLE_NONE */)
{
    ...
    for (LfgRolesMap::const_iterator it = roleCheck.roles.begin(); it != roleCheck.roles.end(); ++it)
    {
        ...
        if (sendRoleChosen && guid != roleCheck.leader && guid < 200000)
    ...
    for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
    {
        ...
        */
        ...
        if (lockData != LFG_LOCKSTATUS_NONE)
        {
            ...
            dataSet.locktyp = lockData; //@ToDo: Rewrite this to tc, lock[dungeon->Entry()] = lockData;
                                        //@ Related compile error / issue: typedef std::map<uint32, LockData> LfgLockMap;
