        ...
        switch (type)
        {
            case DTT_CHARACTER:
            {
                ...
                if (name == "")
                {
                    ...
                    if (result)
                    {
                        loginFlags = uint32(atol(getnth(line, 37).c_str()));
                        ...
                        if (!changenth(line, 37, logFlags))    // characters.at_login set to "rename on login"
                            ROLLBACK(DUMP_FILE_BROKEN);
                    }
                }
                ...
                if (!changenth(line, 60, null))             // characters.deleteInfos_Account
                    ROLLBACK(DUMP_FILE_BROKEN);
                if (!changenth(line, 61, null))             // characters.deleteInfos_Name
                    ROLLBACK(DUMP_FILE_BROKEN);
                if (!changenth(line, 62, null))             // characters.deleteDate
                    ROLLBACK(DUMP_FILE_BROKEN);
                break;
            }
            case DTT_CHAR_TABLE:
