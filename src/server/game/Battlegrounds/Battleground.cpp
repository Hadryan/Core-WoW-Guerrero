...
inline void Battleground::_ProcessJoin(uint32 diff)
{
	...
	if (m_ResetStatTimer > 5000)
	{
		...
		for (BattlegroundPlayerMap::const_iterator itr = GetPlayers().begin(); itr != GetPlayers().end(); ++itr)
			if (Player* player = ObjectAccessor::FindPlayer(itr->first))
			{
				for (uint32 i = uint32(PET_SLOT_HUNTER_FIRST); i < PET_SLOT_HUNTER_LAST; i++)
					if (PetData* pet_data = player->GetPetDatabySlot(i))
					{
						pet_data->m_spellscooldown.clear();
						// Aps: Making health and mana high enough to ensure they are maxed out
						pet_data->curhealth = 200000;
						pet_data->curmana = 200000;
					}
