...
void PetAI::UpdateAI(const uint32 diff)
{
	...
	{
		// Aps: Make sure pet isn't performing a motion action before returning to its owner
		bool valid_motion = !me->GetMotionMaster()->GetMotionSlot(MOTION_SLOT_CONTROLLED);

		if (me->HasReactState(REACT_AGGRESSIVE) || me->GetCharmInfo()->IsAtStay() && valid_motion)
		{
			...
			if (nextTarget)
			{
				SetIsEngageBySpell(false);
				AttackStart(nextTarget);
			}
			else
				HandleReturnMovement();
		}
		else if (valid_motion)
			HandleReturnMovement();
	}
	...
	if (!me->HasUnitState(UNIT_STATE_CASTING))
	{
		...
		{
			owner->ToPlayer()->GetSession()->HandlePetActionHelper(me, me->GetGUID(), abs(me->GetCharmInfo()->GetForcedSpell()), ACT_ENABLED, me->GetCharmInfo()->GetForcedTarget(), NULL, NULL, NULL);
		...
		for (uint8 i = 0; i < me->GetPetAutoSpellSize(); ++i)
		{
			...
			{
				if (spellInfo->CanBeUsedInCombat() && me->isInCombat())
				{

					// Check if we're commanded to attack
...
void PetAI::AttackStart(Unit* target)
{
	...
	// Only chase if not commanded to stay or if stay but commanded to attack and last attack action isnt a spell on passive reaction state
	DoAttack(target, ((!me->GetCharmInfo()->HasCommandState(COMMAND_STAY) && !me->GetCharmInfo()->HasCommandState(COMMAND_MOVE_TO))
		|| me->GetCharmInfo()->IsCommandAttack() && !IsEngageBySpell()));
}
...
void PetAI::HandleReturnMovement()
{
	...
	{
		if (!me->GetCharmInfo()->IsFollowing() && !me->GetCharmInfo()->IsReturning() && !me->GetTransGUID())
		{
			ClearCharmInfoFlags();
			me->GetCharmInfo()->SetIsReturning(true);
			me->GetMotionMaster()->Clear();
