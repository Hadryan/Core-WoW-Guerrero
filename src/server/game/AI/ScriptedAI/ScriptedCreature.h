...
class SummonList : public std::list<uint64>
{
    public:
        ...
        template <class Predicate> void DoAction(int32 info, Predicate& predicate, uint16 max = 0)
        {
            ...
            monster::Containers::RandomResizeList<uint64, Predicate>(listCopy, predicate, max);
...
class BossAI : public ScriptedAI
{
    public:
        ...
        void JustReachedHome() { ScriptedAI::JustReachedHome(); _JustReachedHome(); }
...
struct SmartCreatureAI : public ScriptedAI
{
    SmartCreatureAI(Creature* creature) : ScriptedAI(creature), summons(me), isCaster(false) { }

    virtual uint64 GetLowestFriendGUID() { return 0; }

    virtual float GetFloatData() { return 0.0f; }
    virtual void SetFloatData(float data) { }
