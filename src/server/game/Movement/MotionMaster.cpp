...
void MotionMaster::MoveRandom(float spawndist, bool p_ShouldRun /*= false*/, bool p_SkipWalkChance /*= false*/)
{
    if (_owner->GetTypeId() == TYPEID_UNIT)
    {
        TC_LOG_DEBUG("misc", "Creature (GUID: %u) start moving random", _owner->GetGUIDLow());
        Mutate(new RandomMovementGenerator<Creature>(spawndist, p_ShouldRun, p_SkipWalkChance), MOTION_SLOT_IDLE);
    }
}
...
void MotionMaster::MovePoint(uint32 id, float x, float y, float z, bool generatePath, float speed, bool fixedOrientation, bool isCyclic, bool isCatMull)
{
    if (_owner->GetTypeId() == TYPEID_PLAYER)
    {
...
void MotionMaster::MoveLand(uint32 id, Position const& pos)
{
    ...
    _owner->SetIsHomePosLandNeeded(false);
}
...
void MotionMaster::MoveTakeoff(uint32 id, Position const& pos)
{
    ...
    init.Launch();
    Mutate(new EffectMovementGenerator(id), MOTION_SLOT_ACTIVE);
    _owner->SetIsHomePosLandNeeded(true);
}
...
void MotionMaster::MoveJumpOrientation(float x, float y, float z, float speedXY, float speedZ, uint32 id, Unit const* target)
{
	if (speedXY <= 0.1f)
		return;
	float moveTimeHalf = speedZ / Movement::gravity;
	...
	init.SetParabolic(max_height, 0);
	init.SetVelocity(speedXY);
	// Mainaz: This make the caster face his target after the movejump, (feral charge etc..)
	if (target)
...
void MotionMaster::MoveCharge(PathGenerator path, float speed, uint32 id, Unit const* target /*= nullptr*/)
{
...
void MotionMaster::MoveCustomPath(uint32 pathId, bool repeat, bool isCyclic, bool isCatmullRom, CustomWaypointPath* path)
{
	if (!path)
		return;
	Mutate(new CustomWaypointMovementGenerator<Creature>(pathId, path, repeat, isCyclic, isCatmullRom), MOTION_SLOT_ACTIVE);
}
...
MovementGeneratorType MotionMaster::GetCurrentMovementGeneratorType() const
{
   ...
   /// IF IT STILL CRASHES PORT FROM TC: https://forum.pandawow.ru/showthread.php?t=234751
   return top() ? top()->GetMovementGeneratorType() : IDLE_MOTION_TYPE;
}
...
void MotionMaster::MoveAroundPoint(Position const& centerPos, float radius, bool clockwise, uint8 numberOfPoints, float z, bool temp)
{
	if (!temp)
	{
		Mutate(new CircleMovementGenerator<Creature>(centerPos, radius, clockwise, numberOfPoints), MOTION_SLOT_CONTROLLED);
		return;
	}
...
void MotionMaster::MoveSmoothPath(uint32 pointId, std::vector<Position> const& p_PathPoints, bool walk /*= false*/, bool fly /*= false*/, bool p_Cyclic, bool p_CatMull)
{
    Movement::MoveSplineInit init(_owner);
    if (fly)
    {
    ...
    uint32 pathSize = p_PathPoints.size();

    Movement::PointsArray path;

    /// First point is cleared if cyclic or catmull
    if (p_Cyclic || p_CatMull)
    {
    ...
    for (auto & l_Pos : p_PathPoints)
        path.push_back(G3D::Vector3(l_Pos.GetPositionX(), l_Pos.GetPositionY(), l_Pos.GetPositionZ()));
