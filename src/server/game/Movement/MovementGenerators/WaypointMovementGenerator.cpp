...
void WaypointMovementGenerator<Creature>::OnArrived(Creature* creature)
{
    ...
    if (i_path->at(i_currentNode)->event_id && urand(0, 99) < i_path->at(i_currentNode)->event_chance)
    {
        ...
        creature->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_WAYPOINT, i_path->at(i_currentNode)->event_id, creature, nullptr);
    }
...
void FlightPathMovementGenerator::DoFinalize(Player* player)
{
    ...
    if (player->m_taxi.empty())
    {
        ...
        player->StopMoving();
        float mapHeight = player->TeleportTo(i_path[GetCurrentNode()]->MapID, i_path[GetCurrentNode()]->LocX, i_path[GetCurrentNode()]->LocY, player->GetFloorZ(), player->GetOrientation());
...
void FlightPathMovementGenerator::DoEventIfAny(Player* player, TaxiPathNodeEntry const* node, bool departure)
{
    if (uint32 eventid = departure ? node->DepartureEventID : node->ArrivalEventID)
    {
        ...
        player->GetMap()->ScriptsStart(SmartScriptType::SMART_SCRIPT_TYPE_WAYPOINT, eventid, player, player);
    }
}
